...
/**
 * Simple file logging <tt>Handler</tt>. Extends java.util.logging.FileHandler
 * and adds the special component to the file pattern - %s which is replaced at
 * runtime with sip-communicator's home directory. If the pattern option is
 * missing creates log directory in sip-communicator's home directory. If the
...
public class FileHandler extends java.util.logging.FileHandler {
	...
	/**
	 * Specifies a pattern for generating the output file name. A pattern
	 * consists of a string that includes the special components that will be
	 * replaced at runtime such as : %t, %h, %g, %u. Also adds the special
	 * component : %s sip-communicator's home directory, typically -
	...
	/**
	 * Substitute %s in the pattern and creates the directory if it doesn't
	 * exist.
	 * 
	 ...
	 * 
	 * @param pattern
	 *            the directory we'd like to check.
	 */
