...
public class VideoContainer extends TransparentPanel {
	...
	/**
	 * The <tt>Component</tt> to be displayed by this <tt>VideoContainer</tt> at
	 * {@link VideoLayout#CENTER_REMOTE} when no other <tt>Component</tt> has
	 ...
	 *            the component to be displayed when no remote video is
	 *            available
	 * @param conference
	 *            <tt>true</tt> to dedicate the new instance to a telephony
	 *            conferencing user interface; otherwise, <tt>false</tt>
	 */
	...
	public void add(Component comp, Object constraints, int index) {
		enterAddOrRemove();
		try {
			if (VideoLayout.CENTER_REMOTE.equals(constraints) && (noVideoComponent != null) && !noVideoComponent.equals(comp) || (comp.equals(noVideoComponent) && noVideoComponent.getParent() != null)) {
	 ...
	 *            notification such as the <tt>Component</tt> that has been
	 *            added or removed
	 */
	 ...
	 *            the notification such as the name of the property whose value
	 *            changed and the <tt>Component</tt> which fired the
	 *            notification
	 */
	...
	public void remove(Component comp) {
		...
		try {
			...
			for (Component c : components) {
				if (!c.equals(noVideoComponent) && VideoLayout.CENTER_REMOTE.equals(videoLayout.getComponentConstraints(c))) {
					hasComponentsAtCenterRemote = true;
					break;
				}
			}

			if (!hasComponentsAtCenterRemote && (noVideoComponent != null) && !noVideoComponent.equals(comp)) {
