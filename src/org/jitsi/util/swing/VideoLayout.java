...
public class VideoLayout extends FitLayout {
	 ...
	 * 
	 * @param conference
	 *            <tt>true</tt> if the new instance will be dedicated to a
	 *            conference; otherwise, <tt>false</tt>
	 */
	 ...
	 * 
	 * @param size
	 *            the <tt>Dimension</tt> whose aspect ratio is to be compared to
	 *            the aspect ratio of <tt>width</tt> and <tt>height</tt>
	 * @param width
	 *            the width which defines in combination with <tt>height</tt>
	 *            the aspect ratio to be compared to the aspect ratio of
	 *            <tt>size</tt>
	 * @param height
	 *            the height which defines in combination with <tt>width</tt>
	 *            the aspect ratio to be compared to the aspect ratio of
	 *            <tt>size</tt>
	 ...
	 * 
	 * @param remotes
	 *            the remote visual/video <tt>Component</tt>s to be displayed in
	 *            a grid
	 ...
	 * 
	 * @param c
	 *            the component for which constraints we're looking for
	 ...
	 * 
	 * @param parent
	 *            the <tt>Container</tt> to lay out
	 */
	@Override
	public void layoutContainer(Container parent) {
		...
		if (!conference && (remoteCount == 1)) {
			...
			/*
			 * HGAP is the horizontal gap between the Components being laid out
			 * by this VideoLayout so the number of HGAPs will be with one less
			 * than the number of columns and that horizontal space cannot be
			 * allocated to the bounds of the Components.
			 */
			...
			for (int i = 0; i < remoteCount; i++) {
				...
				super.layoutComponent(remotes.get(i), bounds, Component.CENTER_ALIGNMENT, Component.CENTER_ALIGNMENT);
			}
		}
		...
		if (local == null) {
			...
			if (!remotes.contains(local)) {
				...
				super.layoutComponent(local, new Rectangle(localX, localY, width, height), alignmentX, Component.BOTTOM_ALIGNMENT);
			}
	 ...
	 * 
	 * @param parent
	 *            the container which preferred layout size we're looking for
	...
	public Dimension preferredLayoutSize(Container parent) {
		...
		if (!conference && (remoteCount == 1)) {
			...
			prefLayoutSize = new Dimension(prefLayoutWidth + columnsMinus1 * HGAP, prefLayoutHeight);
		} else
			prefLayoutSize = null;
