...
public class FileAccessServiceImpl implements FileAccessService {
	...
	public File getDefaultDownloadDirectory() throws IOException {
		...
		if (OSUtils.IS_WINDOWS) {
			if (getMajorOSVersion() < 6) {
				...
				int hResult = Shell32.INSTANCE.SHGetFolderPath(null, Shell32.CSIDL_MYDOCUMENTS, null, Shell32.SHGFP_TYPE_CURRENT, pszPath);
				...
				g.data2 = 0x123F;
				g.data3 = 0x4565;
				g.data4 = new byte[] { (byte) 0x91, 0x64, 0x39, (byte) 0xC4, (byte) 0x92, 0x5E, 0x46, 0x7B };

				PointerByReference pszPath = new PointerByReference();
				int hResult = Shell32.INSTANCE.SHGetKnownFolderPath(g, Shell32.KF_FLAG_INIT | Shell32.KF_FLAG_CREATE, null, pszPath);
	...
	private synchronized void initialize() {
		...
		profileDirLocation = cfg != null ? "" : getSystemProperty(ConfigurationService.PNAME_SC_HOME_DIR_LOCATION);
		if (profileDirLocation == null) {
			throw new IllegalStateException(ConfigurationService.PNAME_SC_HOME_DIR_LOCATION);
		}

		scHomeDirName = cfg != null ? "" : getSystemProperty(ConfigurationService.PNAME_SC_HOME_DIR_NAME);
