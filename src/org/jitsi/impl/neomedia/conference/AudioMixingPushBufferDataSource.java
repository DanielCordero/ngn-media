...
public class AudioMixingPushBufferDataSource extends PushBufferDataSource implements CaptureDevice, MuteDataSource, InbandDTMFDataSource {
	...
	/**
	 * The indicator which determines whether this <tt>DataSource</tt> is set to
	 * transmit "silence" instead of the actual media.
	 */
	 ...
	 */
	private final LinkedList<DTMFInbandTone> tones = new LinkedList<DTMFInbandTone>();
	 ...
	 * 
	 * @param audioMixer
	 *            the <tt>AudioMixer</tt> performing audio mixing, managing the
	 *            input <tt>DataSource</tt>s and pushing the data of the new
	 ...
	 * 
	 * @param inDataSource
	 *            a <tt>DataSource</tt> to be added for mixing to the
	 *            <tt>AudioMixer</tt> associate with this instance and to not
	 *            have its audio contributions included in the mixing output
	 ...
	 * 
	 * @throws IOException
	 *             if the <tt>AudioMixer</tt> fails to connect
	 */
	@Override
	 ...
	 *         being made available by this <tt>DataSource</tt> i.e. the
	 ...
	 *            this instance to be retrieved
	 * @return an <tt>Object</tt> which represents the control of this instance
	 *         with the specified type if such a control is available;
	 ...
	 * 
	 * @param sampleRate
	 *            The sampling frequency (codec clock rate) in Hz of the stream
	 *            which will encapsulate this signal.
	 * @param sampleSizeInBits
	 *            The size of each sample (8 for a byte, 16 for a short and 32
	 *            for an int)
	 ...
	 *         from the associated <tt>AudioMixer</tt> and mixes its inputs if
	 *         this <tt>DataSource</tt> is connected; otherwise, an empty array
	 */
	@Override
			...
			if (audioMixerOutStream != null) {
				...
				if (started)
					try {
						...
						logger.error("Failed to start " + outStream.getClass().getSimpleName() + " with hashCode " + outStream.hashCode(), ioex);
					}
			}
		}
	 ...
	 * 
	 * @param mute
	 *            <tt>true</tt> to mute this <tt>DataSource</tt>; otherwise,
	 *            <tt>false</tt>
	 */
	 ...
	 * 
	 * @param inDataSource
	 *            the <tt>DataSource</tt> that was updated.
	 */
