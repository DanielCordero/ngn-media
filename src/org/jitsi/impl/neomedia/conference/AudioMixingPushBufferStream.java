...
public class AudioMixingPushBufferStream extends ControlsAdapter implements PushBufferStream {
	 ...
	 * 
	 * @param outFormat
	 *            the <tt>AudioFormat</tt> of which to get the maximum possible
	 *            value for an audio sample
	 * @return the maximum possible value for an audio sample of the specified
	 *         <tt>AudioFormat</tt>
	 * @throws UnsupportedFormatException
	 *             if the specified <tt>outFormat</tt> is not supported by the
	...
	/**
	 * Initializes a new <tt>AudioMixingPushBufferStream</tt> mixing the input
	 * data of a specific <tt>AudioMixerPushBufferStream</tt> and excluding from
	 * the mix the audio contributions of a specific
	 * <tt>AudioMixingPushBufferDataSource</tt>.
	 * 
	 * @param audioMixerStream
	 *            the <tt>AudioMixerPushBufferStream</tt> reading data from
	 *            input <tt>DataSource</tt>s and to push it to the new
	 *            <tt>AudioMixingPushBufferStream</tt>
	 * @param dataSource
	 *            the <tt>AudioMixingPushBufferDataSource</tt> which has
	 *            requested the initialization of the new instance and which
	...
	public boolean endOfStream() {
		/*
		 * TODO If the inSamples haven't been consumed yet, don't report the end
		 * of this stream even if the wrapped stream has reached its end.
		 */
	 ...
	 *         owns this instance and defines the input data which is to not be
	 *         mixed in the output of this <tt>PushBufferStream</tt>
	 */
	 ...
	 * 
	 * @param inSamples
	 *            the collection of audio sample sets to be mixed into one audio
	 *            sample set in the sense of audio mixing
	 * @param outFormat
	 *            the <tt>AudioFormat</tt> in which the resulting mix audio
	 *            sample set is to be produced. The <tt>format</tt> property of
	 *            the specified <tt>outBuffer</tt> is expected to be set to the
	 *            same value but it is provided as a method argument in order to
	 *            avoid casting from <tt>Format</tt> to <tt>AudioFormat</tt>.
	 * @param outSampleCount
	 *            the size of the resulting mix audio sample set to be produced
	...
	private short[] mix(short[][] inSamples, AudioFormat outFormat, int outSampleCount) {
		...
		for (short[] inStreamSamples : inSamples) {
			...
			for (int i = 0; i < inStreamSampleCount; i++) {
				...
				outSamples[i] = (short) (inStreamSample + outSample - Math.round(inStreamSample * (outSample / maxOutSample)));
			}
		}
		return outSamples;
	}
	...
	/**
	 * Implements {@link PushBufferStream#read(Buffer)}. If <tt>inSamples</tt>
	 * are available, mixes them and writes the mix to the specified
	 * <tt>Buffer</tt> performing the necessary data type conversions.
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> to receive the data read from this
	 *            instance
	 * @throws IOException
	 *             if anything wrong happens while reading from this instance
	 */
	 ...
	 * 
	 * @param inSamples
	 *            the collection of audio sample sets to be mixed by this stream
	 *            when data is read from it
	 * @param maxInSampleCount
	 *            the maximum number of per-stream audio samples available
	 *            through <tt>inSamples</tt>
	 * @param timeStamp
	 *            the time stamp of <tt>inSamples</tt> to be reported in the
	 *            specified <tt>Buffer</tt> when data is read from this instance
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if starting the pushing of data out of this stream fails
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if stopping the pushing of data out of this stream fails
	 */
