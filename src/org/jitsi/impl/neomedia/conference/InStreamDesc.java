...
class InStreamDesc {
	 ...
	 * 
	 * @param inStream
	 *            a <tt>SourceStream</tt> for which additional information is to
	 ...
	 * 
	 * @param create
	 *            the indicator which determines whether the <tt>Buffer</tt> is
	 *            to be created in case it does not exist
	 ...
	 *         contribution of the <tt>SourceStream</tt> described by this
	 ...
	 * @param buffer
	 *            the <tt>Buffer</tt> into which media data is to be read from
	 *            the <tt>SourceStream</tt> described by this instance
	 */
	...
	public void setInStream(SourceStream inStream) {
		if (this.inStream != inStream) {
			...
			/*
			 * Since the inStream has changed, one may argue that the Buffer of
			 * the old value is not optimal for the new value.
			 */
			setBuffer(null);
		}
	}
