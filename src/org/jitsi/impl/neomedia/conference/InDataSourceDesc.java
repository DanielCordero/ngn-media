...
class InDataSourceDesc {
	 ...
	 * element type. Explicitly defined in order to avoid unnecessary
	 * allocations.
	 */
	private static final SourceStream[] EMPTY_STREAMS = new SourceStream[0];

	/**
	 * The <tt>Logger</tt> used by the <tt>InDataSourceDesc</tt> class and its
	...
	/**
	 * Initializes a new <tt>InDataSourceDesc</tt> instance which is to describe
	 * additional information about a specific input <tt>DataSource</tt> of an
	 * <tt>AudioMixer</tt>. Associates the specified <tt>DataSource</tt> with
	 * the <tt>AudioMixingPushBufferDataSource</tt> in which the mix
	 * contributions of the specified input <tt>DataSource</tt> are to not be
	 * included.
	 * 
	 * @param inDataSource
	 *            a <tt>DataSource</tt> for which additional information is to
	 *            be described by the new instance
	 * @param outDataSource
	 *            the <tt>AudioMixingPushBufferDataSource</tt> in which the mix
	 *            contributions of <tt>inDataSource</tt> are to not be included
	 */
	 ...
	 * 
	 * @param audioMixer
	 *            the <tt>AudioMixer</tt> requesting the effective input
	 *            <tt>DataSource</tt> described by this instance to be connected
	 * @throws IOException
	 *             if anything wrong happens while connecting the effective
	...
	synchronized void connect(final AudioMixer audioMixer) throws IOException {
		final DataSource effectiveInDataSource = (transcodingDataSource == null) ? inDataSource : transcodingDataSource;
				...
				connectThread = new Thread() {
					...
					public void run() {
						try {
							...
							logger.error("Failed to connect to inDataSource " + MediaStreamImpl.toString(inDataSource), ioex);
						} finally {
							synchronized (InDataSourceDesc.this) {
	 ...
	 *            <tt>DataSource</tt> described by this instance are to be
	 *            transcoded
	 * @return <tt>true</tt> if a new transcoding <tt>DataSource</tt> has been
	 *         created for the input <tt>DataSource</tt> described by this
	 ...
	 * 
	 * @param controlType
	 *            a <tt>String</tt> value which specifies the type of the
	 ...
	 *         with the specified <tt>controlType</tt> if such a control exists;
	...
	public synchronized DataSource getEffectiveInDataSource() {
		return (transcodingDataSource == null) ? inDataSource : (connected ? transcodingDataSource : null);
	}
	 ...
	 *            the <tt>DataSource</tt> which transcodes the tracks of the
	 *            input <tt>DataSource</tt> described by this instance in the
	 ...
	 * 
	 * @throws IOException
	 *             if starting the effective input <tt>DataSource</tt> described
	 *             by this instance fails
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if stopping the effective input <tt>DataSource</tt> described
	 *             by this instance fails
	 */
