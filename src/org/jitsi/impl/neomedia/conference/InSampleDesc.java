...
/**
 * Describes a specific set of audio samples read from a specific set of input
 * streams specified by their <tt>InStreamDesc</tt>s.
 * <p>
...
class InSampleDesc {
	 ...
	 * <tt>Buffer</tt>s of the <tt>AudioMixingPushBufferStream</tt>s when mixes
	 * are read from them.
	 */
	private long timeStamp = Buffer.TIME_UNKNOWN;

	/**
	 * Initializes a new <tt>InSampleDesc</tt> instance which is to describe a
	 * specific set of audio samples read from a specific set of input streams
	 * specified by their <tt>InStreamDesc</tt>s.
	 * 
	 * @param inSamples
	 *            the set of audio samples read from <tt>inStreams</tt>
	 * @param inStreams
	 *            the set of input streams from which <tt>inSamples</tt> were
	 *            read
	 * @param format
	 ...
	 *         <tt>Buffer</tt>s of the <tt>AudioMixingPushBufferStream</tt>s
	 *         when mixes are read from them
	 */
	...
	public void setTimeStamp(long timeStamp) {
		if (this.timeStamp == Buffer.TIME_UNKNOWN) {
			...
			/*
			 * Setting the timeStamp more than once does not make sense because
			 * the inStreams will report different timeStamps so only one should
			 * be picked up where the very reading from inStreams takes place.
			 */
