 ...
 * {@link #addInDataSource(DataSource)} in the form of input <tt>DataSource</tt>
 * s giving access to one or more input <tt>SourceStreams</tt>.
 * </p>
 * <p>
...
public class AudioMixer {
	 ...
	 */
	private static final AudioFormat DEFAULT_OUTPUT_FORMAT = new AudioFormat(AudioFormat.LINEAR, 8000, 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED);
	...
	/**
	 * Gets the <tt>Format</tt> in which a specific <tt>DataSource</tt> provides
	 * stream data.
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> for which the <tt>Format</tt> in which
	 *            it provides stream data is to be determined
	 * @return the <tt>Format</tt> in which the specified <tt>dataSource</tt>
	 *         provides stream data if it was determined; otherwise,
	 *         <tt>null</tt>
	 */
	 ...
	 * 
	 * @param stream
	 *            the <tt>SourceStream</tt> for which the <tt>Format</tt> in
	 ...
	 * <tt>inDataSources</tt> excluding <tt>captureDevice</tt> and is thus meant
	 * for playback on the local peer in a call.
	 */
	 ...
	 * instance pushes audio sample data to <tt>AudioMixingPushBufferStream</tt>
	 * s to be mixed.
	 */
	 ...
	 *            <tt>AudioMixingPushBufferDataSource</tt>s created by the new
	 *            instance and its first input <tt>DataSource</tt>
	 */
	 ...
	 * 
	 * @param inDataSource
	 *            a new <tt>DataSource</tt> to input audio to this instance
	 */
	 ...
	 *            a new <tt>DataSource</tt> to input audio to this instance
	 * @param outDataSource
	 *            the <tt>AudioMixingPushBufferDataSource</tt> to not include
	 *            the audio contributions of <tt>inDataSource</tt> in the mix it
	 *            outputs
	 */
	void addInDataSource(DataSource inDataSource, AudioMixingPushBufferDataSource outDataSource) {
		...
		synchronized (inDataSources) {
			...
			if (added) {
				...
				/*
				 * If the other inDataSources have been started, start the new
				 * one as well.
				 */
				if (started > 0) {
					try {
	 ...
	 * connected. The first of the many <tt>AudioMixingPushBufferDataSource</tt>
	 * s reading from this <tt>AudioMixer</tt> which gets connected causes it to
	 * connect to the input <tt>DataSource</tt>s it manages.
	 * 
	 * @throws IOException
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> to connect to
	 * @param inDataSource
	 *            the <tt>DataSource</tt> which is the cause for
	 ...
	 * 
	 * @param inDataSource
	 *            the <tt>InDataSourceDesc</tt> of the input <tt>DataSource</tt>
	 *            which has finished its connecting procedure
	 * @throws IOException
	 ...
	 * 
	 * @param inStream
	 *            the input <tt>SourceStream</tt> to be described by the new
	 *            instance
	...
	/**
	 * Creates a new <tt>AudioMixingPushBufferDataSource</tt> which gives access
	 * to a single audio stream representing the mix of the audio streams input
	 * into this <tt>AudioMixer</tt> through its input <tt>DataSource</tt>s. The
	 * returned <tt>AudioMixingPushBufferDataSource</tt> can also be used to
	 * include new input <tt>DataSources</tt> in this <tt>AudioMixer</tt> but
	 ...
	 *         to a single audio stream representing the mix of the audio
	 *         streams input into this <tt>AudioMixer</tt> through its input
	 *         <tt>DataSource</tt>s
	 */
	 ...
	 * specific input <tt>DataSource</tt> into a specific output <tt>Format</tt>
	 * .
	 * 
	 * @param inDataSourceDesc
	 *            the <tt>InDataSourceDesc</tt> describing the input
	 *            <tt>DataSource</tt> to be transcoded into the specified output
	 *            <tt>Format</tt> and to receive the transcoding
	 *            <tt>DataSource</tt>
	 * @param outFormat
	 *            the <tt>Format</tt> in which the tracks of the input
	 ...
	 * @throws IOException
	 *             if an error occurs while creating the transcoding
	 ...
	 * <tt>AudioMixer</tt> which gets disconnected causes it to disconnect from
	 * the input <tt>DataSource</tt>s it manages.
	 */
	void disconnect() {
		synchronized (inDataSources) {
			if (connected <= 0)
			...
			if (connected == 0) {
				...
				/*
				 * XXX Make the outStream to release the inStreams. Otherwise,
				 * the PushBufferStream ones which have been wrapped into
				 * CachingPushBufferStream may remain waiting.
				 */
	 ...
	 *         its <tt>AudioMixingPushBufferDataSource</tt>s if such a control
	 *         is available for the <tt>CaptureDevice</tt> of this instance;
	 *         otherwise, <tt>null</tt>
	 */
	BufferControl getBufferControl() {
		if ((bufferControl == null) && (captureDevice instanceof Controls)) {
			BufferControl captureDeviceBufferControl = (BufferControl) ((Controls) captureDevice).getControl(BufferControl.class.getName());
	...
	/**
	 * Gets the <tt>CaptureDeviceInfo</tt> of the <tt>CaptureDevice</tt> this
	 ...
	 * <tt>InStreamDesc</tt>s which describes a specific <tt>SourceStream</tt>.
	 * If such an <tt>InStreamDesc</tt> does not exist, returns <tt>null</tt>.
	 * 
	 * @param inStream
	 *            the <tt>SourceStream</tt> to locate an <tt>InStreamDesc</tt>
	 *            for in <tt>existingInStreamDescs</tt>
	 * @param existingInStreamDescs
	 *            the list of existing <tt>InStreamDesc</tt>s in which an
	 *            <tt>InStreamDesc</tt> for <tt>inStream</tt> is to be located
	 * @return an <tt>InStreamDesc</tt> from <tt>existingInStreamDescs</tt>
	 *         which describes <tt>inStream</tt> if such an
	 *         <tt>InStreamDesc</tt> exists; otherwise, <tt>null</tt>
	 */
	...
	/**
	 * Gets the <tt>SourceStream</tt>s (in the form of <tt>InStreamDesc</tt>) of
	 * a specific <tt>DataSource</tt> (provided in the form of
	 ...
	 * @param existingInStreams
	 *            the <tt>InStreamDesc</tt> instances which already exist and
	 *            which are used to avoid creating multiple
	 *            <tt>InStreamDesc</tt>s for input <tt>SourceStream</tt>s which
	 *            already have ones
	 * @param inStreams
	 *            the <tt>List</tt> of <tt>InStreamDesc</tt> in which the
	 *            discovered <tt>SourceStream</tt>s are to be returned
		...
		if ((inFormat != null) && !matches(inFormat, outFormat)) {
			if (inDataSource instanceof PushDataSource) {
				for (PushSourceStream inStream : ((PushDataSource) inDataSource).getStreams()) {
					...
					if (inStreamDesc == null)
						inStreamDesc = createInStreamDesc(new PushBufferStreamAdapter(inStream, inFormat), inDataSourceDesc);
	...
	/**
	 * Gets the <tt>SourceStream</tt>s (in the form of <tt>InStreamDesc</tt>) of
	 * the <tt>DataSource</tt>s from which this <tt>AudioMixer</tt> reads data
	 * which produce data in a specific <tt>AudioFormat</tt>. When an input
	 ...
	 *            <tt>SourceStream</tt>s are to produce data
	 * @param existingInStreams
	 *            the <tt>SourceStream</tt>s which are already known to this
	 *            <tt>AudioMixer</tt>
	 * @return a new collection of <tt>SourceStream</tt>s (in the form of
	 *         <tt>InStreamDesc</tt>) retrieved from the input
	 *         <tt>DataSource</tt>s of this <tt>AudioMixer</tt> and producing
	 *         data in the specified <tt>AudioFormat</tt>
	 * @throws IOException
	 *             if anything wrong goes while retrieving the input
	...
	private Collection<InStreamDesc> getInStreamsFromInDataSources(AudioFormat outFormat, InStreamDesc[] existingInStreams) throws IOException {
		...
		synchronized (inDataSources) {
			for (InDataSourceDesc inDataSourceDesc : inDataSources) {
				boolean got = getInStreamsFromInDataSource(inDataSourceDesc, outFormat, existingInStreams, inStreams);
	 ...
	 *         of all input <tt>DataSource</tt>s excluding the
	 *         <tt>CaptureDevice</tt> of this <tt>AudioMixer</tt> and is thus
	 ...
	 *         of this <tt>AudioMixer</tt> can produce data and which is to be
	 *         the output <tt>Format</tt> of this <tt>AudioMixer</tt>
	 */
	 ...
	 *         input <tt>DataSource</tt>s of this <tt>AudioMixer</tt> and pushes
	 *         it to output <tt>AudioMixingPushBufferStream</tt>s for audio
	 *         mixing
	 */
	AudioMixerPushBufferStream getOutStream() {
		synchronized (inDataSources) {
			AudioFormat outFormat = (outStream == null) ? getOutFormatFromInDataSources() : outStream.getFormat();
			...
			try {
				inStreams = getInStreamsFromInDataSources(outFormat, (outStream == null) ? null : outStream.getInStreams());
			} catch (IOException ioex) {
				throw new UndeclaredThrowableException(ioex);
			}
	 ...
	 * <tt>TranscodingDataSource</tt>. Currently this is only used when the
	 * <tt>MediaStream</tt> needs access to the codec chain used to playback one
	 * of it's <tt>ReceiveStream</tt>s.
	 * 
	 * @param inDataSource
	 *            the <tt>DataSource</tt> to search for.
	 * @return The <tt>TranscodingDataSource</tt> associated with
	 *         <tt>inDataSource</tt>, if we can find one, <tt>null</tt>
	 *         otherwise.
	 */
	 ...
	 * <tt>Format</tt> in the sense of JMF <tt>Format</tt> matching. Since this
	 * <tt>AudioMixer</tt> and the audio mixing functionality related to it can
	 * handle varying characteristics of a certain output <tt>Format</tt>, the
	 * only requirement for the specified <tt>Format</tt>s to match is for both
	 ...
	 * @param pattern
	 *            the <tt>Format</tt> against which the specified <tt>input</tt>
	 *            is to be matched
	...
	/**
	 * Reads media from a specific <tt>PushBufferStream</tt> which belongs to a
	 * specific <tt>DataSource</tt> into a specific output <tt>Buffer</tt>.
	 * Allows extenders to tap into the reading and monitor and customize it.
	 * 
	 * @param stream
	 *            the <tt>PushBufferStream</tt> to read media from and known to
	 *            belong to the specified <tt>DataSOurce</tt>
	 * @param buffer
	 *            the output <tt>Buffer</tt> in which the media read from the
	 *            specified <tt>stream</tt> is to be written so that it gets
	 *            returned to the caller
	 * @param dataSource
	 *            the <tt>DataSource</tt> from which <tt>stream</tt> originated
	 * @throws IOException
	 *             if anything wrong happens while reading from the specified
	 *             <tt>stream</tt>
	 */
	 ...
	 *            the <tt>DataSourceFilter</tt> which selects the
	 *            <tt>DataSource</tt>s to be removed from the list of input
	 *            <tt>DataSource</tt>s of this <tt>AudioMixer</tt> from which it
	 ...
	 *            <tt>DataSource</tt>s of this <tt>AudioMixer</tt> are to be
	 ...
	 * 
	 * @param outStream
	 *            the <tt>AudioMixerPushBufferStream</tt> which requests this
	 *            <tt>AudioMixer</tt> to start. If <tt>outStream</tt> is the
	 *            current one and only <tt>AudioMixerPushBufferStream</tt> of
	 *            this <tt>AudioMixer</tt>, this <tt>AudioMixer</tt> starts if
	 *            it hasn't started yet. Otherwise, the request is ignored.
	 * @param generation
	 *            a value generated by <tt>outStream</tt> indicating the order
	 *            of the invocations of the <tt>start</tt> and <tt>stop</tt>
	 *            methods performed by <tt>outStream</tt> allowing it to execute
	 *            the said methods outside synchronized blocks for the purposes
	 *            of reducing deadlock risks
	 * @throws IOException
	 *             if any of the input <tt>DataSource</tt>s of this
	 *             <tt>AudioMixer</tt> throws such an exception while attempting
	 *             to start it
	 */
	 ...
	 * @param outStream
	 *            the <tt>AudioMixerPushBufferStream</tt> which requests this
	 *            <tt>AudioMixer</tt> to stop. If <tt>outStream</tt> is the
	 *            current one and only <tt>AudioMixerPushBufferStream</tt> of
	 *            this <tt>AudioMixer</tt>, this <tt>AudioMixer</tt> stops.
