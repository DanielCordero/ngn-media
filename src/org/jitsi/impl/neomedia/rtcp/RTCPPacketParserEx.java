 ...
 */
public class RTCPPacketParserEx extends net.sf.fmj.media.rtp.RTCPPacketParser {
	/**
	...
	@Override
	protected RTCPPacket parse(RTCPCompoundPacket base, int firstbyte /*
																	 * without
																	 * version
																	 * /padding
																	 */, int type, int length /*
																							 ...
																							 * actual
																							 * bytes
																							 */, DataInputStream in) throws BadFormatException, IOException {
		if (type == RTCPFBPacket.RTPFB || type == RTCPFBPacket.PSFB) {
			...
			if (type == RTCPFBPacket.RTPFB) {
				return parseRTCPFBPacket(base, firstbyte, RTCPFBPacket.RTPFB, length, in, senderSSRC, sourceSSRC);
			} else {
				switch (firstbyte) {
				case RTCPREMBPacket.FMT: // REMB
					...
					remb.mantissa = ((buf[0] & 0x3) << 16) & 0xFF0000 | (buf[1] << 8) & 0x00FF00 | buf[2] & 0x0000FF;
					...
					return remb;
				default:
					return parseRTCPFBPacket(base, firstbyte, RTCPFBPacket.PSFB, length, in, senderSSRC, sourceSSRC);
				}
			}
		} else
			return null;
	...
	private RTCPFBPacket parseRTCPFBPacket(RTCPCompoundPacket base, int firstbyte, int type, int length /*
																										 * in
																										 * actual
																										 * bytes
																										 */, DataInputStream in, long senderSSRC, long sourceSSRC) throws IOException {
			...
			case RTCPFBPacket.RTPFB:
				...
				case 4: /*
					...
					for (int i = 0, end = fcilen - 8; i < end; i += 8) {
						...
						= (((b4 & 0x1) << 16) & 0xFF0000) | ((fb.fci[i + 5] << 8) & 0x00FF00) | (b6 & 0x0000FF);
						int measuredOverhead /* 9 bits */
						= (((b6 & 0x1) << 8) & 0xFF00) | (fb.fci[i + 7] & 0x00FF);
