...
public class FeedbackCacheProcessor {
	...
	private Map<Integer, Map<Integer, FeedbackData>> getReverseFeedbackMapMap() {
		Map<Integer, Map<Integer, FeedbackData>> reverseFeedbackMapMap = new HashMap<Integer, Map<Integer, FeedbackData>>();

		if (feedbackCache != null && feedbackCache.size() != 0) {
			for (Map.Entry<Integer, FeedbackCacheEntry> entry : feedbackCache.entrySet()) {
				...
				if (item.reports != null && item.reports.length != 0) {
					for (RTCPReportBlock b : item.reports) {
						...
						// <dest, feedback>
						Map<Integer, FeedbackData> reverseFeedbackMap = new HashMap<Integer, FeedbackData>();
				...
				if (item.remb != null && item.remb.dest != null && item.remb.dest.length != 0) {
					for (long destl : item.remb.dest) {
						...
						if (reverseFeedbackMapMap.containsKey(dest)) {
							reverseFeedbackMap = reverseFeedbackMapMap.get(dest);
						} else {
							reverseFeedbackMap = new HashMap<Integer, FeedbackData>();
	...
	public Map<Integer, FeedbackData> getReverseFeedbackMap() {
		...
		Map<Integer, Map<Integer, FeedbackData>> reverseFeedbackMapMap = getReverseFeedbackMapMap();
		...
		for (Map.Entry<Integer, Map<Integer, FeedbackData>> entry : reverseFeedbackMapMap.entrySet()) {
			...
			int idxNearestBest = (int) Math.ceil((p / 100.0) * scoresMap.size()) - 1;

			FeedbackData feedback = new ArrayList<FeedbackData>(scoresMap.values()).get(idxNearestBest);
	...
	public RTCPPacket[] makeReports(int localSSRC) {
		Map<Integer, FeedbackData> reverseFeedback = getReverseFeedbackMap();

		if (reverseFeedback == null || reverseFeedback.size() == 0)
			return null;

		List<RTCPReportBlock> reportBlocks = new ArrayList<RTCPReportBlock>(reverseFeedback.size());
		List<RTCPREMBPacket> rembs = new ArrayList<RTCPREMBPacket>(reverseFeedback.size());
		...
		for (Map.Entry<Integer, FeedbackData> entry : reverseFeedback.entrySet()) {
			...
			if (feedback.remb != null) {
				RTCPREMBPacket remb = new RTCPREMBPacket(localSSRC, 0L, feedback.remb.exp, feedback.remb.mantissa, new long[] { dest & 0xFFFFFFFFL });
