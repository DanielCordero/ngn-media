...
public class ReceiverReporting {
	...
	/**
	 * The cache processor that will be making the RTCP reports coming from the
	 * bridge.
	 */
	...
	public RTCPPacket[] makeReports() {
		...
		RTCPPacket[] bestReceiverFeedback = feedbackCacheProcessor.makeReports(localSSRC);
		if (bestReceiverFeedback != null && bestReceiverFeedback.length != 0) {
			for (RTCPPacket packet : bestReceiverFeedback) {
				if (packet.type == RTCPFBPacket.PSFB && packet instanceof RTCPFBPacket && ((RTCPFBPacket) packet).fmt == RTCPREMBPacket.FMT) {
					packets.add(packet);
				}
			}
		}
	...
	public Map<Integer, RTCPReportBlock> makeReceiverReportsMap(long time) {
		...
		for (Enumeration<SSRCInfo> elements = rtcpTransmitter.cache.cache.elements(); elements.hasMoreElements();) {
	...
	private RTCPReportBlock[] makeReceiverReports(long time) {
		Collection<RTCPReportBlock> reports = makeReceiverReportsMap(time).values();
