...
/**
 * A depacketizer from VP8. See
 * {@link "http://tools.ietf.org/html/draft-ietf-payload-vp8-11"}
 * 
 * @author Boris Grozev
 */
public class DePacketizer extends AbstractCodec2 {
	...
	/**
	 * Stores the RTP payloads (VP8 payload descriptor stripped) from RTP
	 * packets belonging to a single VP8 compressed frame.
	 */
	 ...
	 * frame, parts of which are currently stored in <tt>data</tt>, or -1 if the
	 * <tt>PictureID</tt> field is not in use or <tt>data</tt> is empty.
	 */
	private int pictureId = -1;

	/**
	 * Stores the RTP timestamp of the packets stored in <tt>data</tt>, or -1 if
	 * they don't have a timestamp set.
	 */
	 ...
	 */
	public DePacketizer() {
		super("VP8 RTP DePacketizer", VideoFormat.class, new VideoFormat[] { new VideoFormat(VideoFormat.VP8) });
		inputFormats = new VideoFormat[] { new VideoFormat(VideoFormat.VP8_RTP) };
	}
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if (!empty) {
			...
			// frame.
			if ((inPictureId != -1 && pictureId != -1 && inPictureId != pictureId) | (timestamp != -1 && inRtpTimestamp != -1 && inRtpTimestamp != timestamp)) {
				if (seqNumComparator.compare(inSeq, firstSeq) != 1) // inSeq <=
																	// firstSeq
				{
				...
				} else // inSeq > firstSeq (and also presumably isSeq > lastSeq)
				{
					...
					// not-well-received PictureID by sending a RTCP SLI or
					// NACK.
					reinit();
				}
			}
		...
		if (firstSeq == -1 || (seqNumComparator.compare(firstSeq, inSeq) == 1))
			firstSeq = inSeq;
		if (lastSeq == -1 || (seqNumComparator.compare(inSeq, lastSeq) == 1))
			lastSeq = inSeq;
	 ...
	 * {@link "http://tools.ietf.org/html/draft-ietf-payload-vp8-10"}
	 */
	static class VP8PayloadDescriptor {
		...
		/**
		 * Returns a simple Payload Descriptor, with PartID = 0, the 'start of
		 * partition' bit set according to <tt>startOfPartition</tt>, and all
		 * other bits set to 0.
		 * 
		 * @param startOfPartition
		 *            whether to 'start of partition' bit should be set
		 * @return a simple Payload Descriptor, with PartID = 0, the 'start of
		 *         partition' bit set according to <tt>startOfPartition</tt>,
		...
		/**
		 * The size in bytes of the Payload Descriptor at offset <tt>offset</tt>
		 * in <tt>input</tt>. The size is between 1 and 6.
		 * 
		 * @param input
		 *            input
		 * @param offset
		 ...
		 *         <tt>offset</tt> in <tt>input</tt>, or -1 if the input is not
		 *         a valid VP8 Payload Descriptor. The size is between 1 and 6.
		 */
		...
		private static int getPictureId(byte[] input, int offset) {
			...
			if ((input[offset] & X_BIT) == 0 || (input[offset + 1] & I_BIT) == 0)
				return -1;
		 ...
		 * @param offset
		 *            offset
		 * @return <tt>true</tt> if the '<tt>start of partition</tt>' bit is
		 ...
		 *         is set and the <tt>PID</tt> fields has value 0 in the VP8
		 *         Payload Descriptor at offset <tt>offset</tt> in
		 *         <tt>input</tt>.
		 */
		...
		/**
		 * Returns the value of the <tt>PID</tt> (partition ID) field of the VP8
		 * Payload Descriptor at offset <tt>offset</tt> in <tt>input</tt>.
		 * 
		 * @param input
		 * @param offset
		 * @return the value of the <tt>PID</tt> (partition ID) field of the VP8
		 *         Payload Descriptor at offset <tt>offset</tt> in
		 *         <tt>input</tt>.
		 */
