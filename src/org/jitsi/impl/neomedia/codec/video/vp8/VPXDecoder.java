...
public class VPXDecoder extends AbstractCodec2 {
	...
	/**
	 * The <tt>Logger</tt> used by the <tt>VPXDecoder</tt> class for logging
	 * output.
	 */
	 ...
	 */
	private static final VideoFormat[] SUPPORTED_OUTPUT_FORMATS = new VideoFormat[] { new AVFrameFormat(FFmpeg.PIX_FMT_YUV420P) };

	/**
	 * Pointer to a native vpx_codec_dec_cfg structure containing the decoder
	 * configuration
	 */
	private long cfg = 0;
	...
	/**
	 * The last known width of the video output by this <tt>VPXDecoder</tt>.
	 * Used to detect changes in the output size.
	 */
	private int height;

	/**
	 * Pointer to a native vpx_image structure, containing a decoded frame. When
	 * doProcess() is called, this is either 0 or it has the address of the next
	 * unprocessed image from the decoder
	 */
	private long img = 0;
	...
	/**
	 * The last known height of the video output by this <tt>VPXDecoder</tt>.
	 * Used to detect changes in the output size.
	 */
	private int width;
	...
	public VPXDecoder() {
		...
		inputFormat = null;
		outputFormat = null;
		inputFormats = new VideoFormat[] { new VideoFormat(VideoFormat.VP8) };
	}
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		if (leftoverFrames) {
			 ...
			 */
			updateOutputFormat(VPX.img_get_d_w(img), VPX.img_get_d_h(img), ((VideoFormat) inputBuffer.getFormat()).getFrameRate());
			...
			int ret = VPX.codec_decode(context, buf, buf_offset, buf_size, 0, 0);
			if (ret != VPX.CODEC_OK) {
				if (logger.isDebugEnabled())
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AVFrameFormat(inputVideoFormat.getSize(), inputVideoFormat.getFrameRate(), FFmpeg.PIX_FMT_YUV420P) };
	}

	/**
	 * Allocates a new AVFrame and set its data fields to the data fields from
	 * the <tt>vpx_image_t</tt> pointed to by <tt>img</tt>. Also set its
	 * 'linesize' according to <tt>img</tt>.
	 * 
	 * @param img
	 *            pointer to a <tt>vpx_image_t</tt> whose data will be used
	 * 
	...
	private AVFrame makeAVFrame(long img) {
		...
		FFmpeg.avframe_set_data(avframe.getPtr(), p0, p1 - p0, p2 - p1);
	...
	private void updateOutputFormat(int width, int height, float frameRate) {
		if ((width > 0) && (height > 0) && ((this.width != width) || (this.height != height))) {
			this.width = width;
			this.height = height;
			outputFormat = new AVFrameFormat(new Dimension(width, height), frameRate, FFmpeg.PIX_FMT_YUV420P);
		}
	}
}
