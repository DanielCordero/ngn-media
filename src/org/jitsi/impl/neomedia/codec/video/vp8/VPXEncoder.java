...
public class VPXEncoder extends AbstractCodec2 {
	 ...
	 */
	private static final VideoFormat[] SUPPORTED_OUTPUT_FORMATS = new VideoFormat[] { new VideoFormat(VideoFormat.VP8) };

	/**
	 * Pointer to a native vpx_codec_dec_cfg structure containing encoder
	 * configuration
	 */
	private long cfg = 0;
	 ...
	 * {@inheritDoc}
	 * 
	 * Encodes the frame in <tt>inputBuffer</tt> (in <tt>YUVFormat</tt>) into a
	 * VP8 frame (in <tt>outputBuffer</tt>)
	 * 
	 * @param inputBuffer
	 *            input <tt>Buffer</tt>
	 * @param outputBuffer
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (leftoverPackets) {
			...
			int result = VPX.codec_encode(context, img, (byte[]) inputBuffer.getData(), offsetY, offsetU, offsetV, frameCount, // pts
					1, // duration
					0, // flags
					VPX.DL_REALTIME);
			...
			if (pkt != 0 && VPX.codec_cx_pkt_get_kind(pkt) == VPX.CODEC_CX_FRAME_PKT) {
				int size = VPX.codec_cx_pkt_get_size(pkt);
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new VideoFormat[] { new VideoFormat(VideoFormat.VP8, inputVideoFormat.getSize(),
