...
public class JNIDecoder extends AbstractCodec {
	 ...
	 */
	private static final VideoFormat[] DEFAULT_OUTPUT_FORMATS = new VideoFormat[] { new AVFrameFormat(FFmpeg.PIX_FMT_YUV420P) };
	...
	public JNIDecoder() {
		...
		/*
		 * Explicitly state both ParameterizedVideoFormat (to receive any format
		 * parameters which may be of concern to this JNIDecoder) and
		 * VideoFormat (to make sure that nothing breaks because of equality
		 * and/or matching tests involving ParameterizedVideoFormat).
		 */
		(VideoFormat)(new RTPMediaFormat(new VideoFormat(VideoFormat.H264)).getFormat()), new VideoFormat(VideoFormat.H264) };
	...
	public boolean checkFormat(Format format) {
		return format.getEncoding().equals(VideoFormat.H264_RTP);
	}
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AVFrameFormat(inputVideoFormat.getSize(), ensureFrameRate(inputVideoFormat.getFrameRate()), FFmpeg.PIX_FMT_YUV420P) };
	}
	...
	private void handleFmtps() {
		try {
			...
			if (f instanceof VideoFormat && f.getRtpFormat() != null) {
				String spropParameterSets = f.getRtpFormat().getFormatSettings().containsKey(RTPMediaFormat.H264_SPROP_PARAMETER_SETS_FMTP)? f.getRtpFormat().getFormatSettings().get(RTPMediaFormat.H264_SPROP_PARAMETER_SETS_FMTP):null;
				...
				if (spropParameterSets != null) {
					...
					if (nals.size() != 0) {
						// Add padding because it seems to be required by
						// FFmpeg.
						 ...
						 * sprop-parameter-sets in the NAL unit stream to
						 * precede any other NAL units in decoding order.
						 */
						FFmpeg.avcodec_decode_video(avctx, avframe.getPtr(), got_picture, nals.toByteArray(), nals.size());
					}
				}
			}

			/*
			 * Because the handling of format parameter is new at the time of
			 * this writing and it currently handles only the format parameter
