...
public class DePacketizer extends AbstractCodec2 {
	 ...
	 */
	private static final long TIME_FROM_KEY_FRAME_TO_REQUEST_KEY_FRAME = ((JNIEncoder.DEFAULT_KEYINT * 4L) / (JNIEncoder.DEFAULT_FRAME_RATE * 3L)) * 1000L;
	 ...
	 */
	public DePacketizer() {
		super("H264 DePacketizer", VideoFormat.class, new VideoFormat[] { new VideoFormat(VideoFormat.H264) });

		List<Format> inputFormats = new ArrayList<Format>();

		inputFormats.add(new VideoFormat(VideoFormat.H264_RTP));
		/*
	 ...
	 * 
	 * @param in
	 *            the payload of the RTP packet from which a FU-A fragment of a
	 *            NAL unit is to be extracted
	 * @param inOffset
	 *            the offset in <tt>in</tt> at which the payload begins
	 * @param inLength
	 *            the length of the payload in <tt>in</tt> beginning at
	 *            <tt>inOffset</tt>
	 * @param outBuffer
	 *            the <tt>Buffer</tt> which is to receive the extracted FU-A
	 *            fragment of a NAL unit
	...
	private int dePacketizeFUA(byte[] in, int inOffset, int inLength, Buffer outBuffer) {
		...
		byte[] out = validateByteArraySize(outBuffer, outBuffer.getOffset() + newOutLength + outputPaddingSize, true);
	 ...
	 *            <tt>inOffset</tt>
	 * @param outBuffer
	 *            the <tt>Buffer</tt> which is to receive the extracted NAL unit
	 * @return the flags such as <tt>BUFFER_PROCESSED_OK</tt> and
	 *         <tt>OUTPUT_BUFFER_NOT_FILLED</tt> to be returned by
	 *         {@link #process(Buffer, Buffer)}
	 */
	private int dePacketizeSingleNALUnitPacket(int nal_unit_type, byte[] in, int inOffset, int inLength, Buffer outBuffer) {
		...
		byte[] out = validateByteArraySize(outBuffer, outOffset + newOutLength + outputPaddingSize, true);
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		case 5 /* Coded slice of an IDR picture */:
			...
			/*
			 * While it seems natural to not request a key frame in the presence
			 * of 5, 7 and 8 often seem to be followed by 5 so do not request a
			 * key frame if either 7 or 8 is present.
			 */
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		if ((matchingOutputFormats != null) && (matchingOutputFormats.length != 0) && (inputFormat instanceof VideoFormat && inputFormat.getRtpFormat() != null)) {
			Map<String, String> fmtps = inputFormat.getRtpFormat().getFormatSettings();

			if (fmtps != null) {
				fmtps.remove(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP);
				if (!fmtps.isEmpty()) {
					for (int i = 0; i < matchingOutputFormats.length; i++) 
					{
						if (matchingOutputFormats[i].getRtpFormat() != null)
						{
						    matchingOutputFormats[i].getRtpFormat().setFormatSettings(fmtps);
						}
					}
				}
			}
	 ...
	 * {@link #keyFrameControl}.
	 * 
	 * @param requestKeyFrame
	 *            <tt>true</tt> if this <tt>DePacketizer</tt> is to request a
	 *            key frame from the remote peer associated with
	 *            {@link #keyFrameControl}
	 */
