...
public class JNIEncoder extends AbstractCodec implements RTCPFeedbackMessageListener {
	 ...
	 */
	public static final String[] AVAILABLE_PRESETS = { "ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow" };
	...
	public static Map<String,String> makeFormatSettings(String key,String value)
	{
		Map<String,String> ftpms = new HashMap<String,String>();
		ftpms.put(key, value);
		return ftpms;
	}
	 ...
	 * as output.
	 */
	static final  Format[] SUPPORTED_OUTPUT_FORMATS = { new RTPMediaFormat(new VideoFormat(VideoFormat.H264), RTPMediaType.VIDEO,RTPMediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, makeFormatSettings(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, "0"),null).getFormat(), new RTPMediaFormat(new VideoFormat(VideoFormat.H264), RTPMediaType.VIDEO,RTPMediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, makeFormatSettings(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, "1"),null).getFormat() };
	...
	/**
	 * The <tt>KeyFrameControl</tt> used by this <tt>JNIEncoder</tt> to control
	...
	private Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		for (int index = packetizationModes.length - 1; index >= 0; index--) {
			Map<String,String> ftpms = new HashMap<String,String>();
			ftpms.put(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, packetizationModes[index]);
			
			VideoFormat vff = new VideoFormat(VideoFormat.H264);
			vff.setSize(size);
			vff.setFrameRate(frameRate);
			RTPMediaFormat rmf = new RTPMediaFormat(vff);
			rmf.setFormatSettings(ftpms);
			matchingOutputFormats[index] = rmf.getFormat();
		}
	...
	public Format[] getSupportedOutputFormats(Format in) {
		...
		else if (!(in instanceof VideoFormat) || (null == AbstractCodec2.matches(in, inputFormats)))
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		if (additionalCodecSettings != null) {
			for (Map.Entry<String, String> e : additionalCodecSettings.entrySet()) {
				...
				if ("h264.intrarefresh".equals(k)) {
					...
					if (BASELINE_PROFILE.equals(v) || HIGH_PROFILE.equals(v) || MAIN_PROFILE.equals(v))
						profile = v;
				}
			}
		}
		...
		/*
		 * XXX crf=0 means lossless coding which is not supported by the
		 * baseline and main profiles. Consequently, we cannot specify it
		 * because we specify either the baseline or the main profile.
		 * Otherwise, x264 will detect the inconsistency in the specified
		 * parameters/options and FFmpeg will fail.
		 */
		// "crf" /* constant quality mode, constant ratefactor */, "0",
				"intra-refresh", intraRefresh ? "1" : "0", "keyint", Integer.toString(keyint), "partitions", "b8x8,i4x4,p8x8", "preset", preset, "thread_type", "slice", "tune", "zerolatency") < 0) {
	 ...
	 * 
	 * @param ev
	 *            an <tt>RTCPFeedbackMessageEvent</tt> which specifies the
	 *            details of the notification event such as the feedback message
	 *            type and the payload type
	 */
	@Override
	 ...
	 * 
	 * @param additionalCodecSettings
	 *            the additional settings to be set on this <tt>Codec</tt>
	 */
	 ...
	 * 
	 * @param format
	 *            the <tt>Format</tt> of media data to set on this
	 *            <tt>Codec</tt>
	 * @return the <tt>Format</tt> of media data set on this <tt>Codec</tt> or
	 *         <tt>null</tt> if the specified <tt>format</tt> is not supported
	 *         by this <tt>Codec</tt>
	 */
	@Override
	public Format setInputFormat(Format format) {
		// mismatch input format
		if (!(format instanceof VideoFormat) || (null == AbstractCodec2.matches(format, inputFormats)))
			return null;
	 ...
	 * 
	 * @param format
	 *            the <tt>Format</tt> in which this <tt>Codec</tt> is to output
	 *            media data
	 * @return the <tt>Format</tt> in which this <tt>Codec</tt> is currently
	 *         configured to output media data or <tt>null</tt> if
	 *         <tt>format</tt> was found to be incompatible with this
	 *         <tt>Codec</tt>
	 */
	@Override
	public Format setOutputFormat(Format format) {
		...
		if (format instanceof VideoFormat && format.getRtpFormat() != null)
			fmtps = ((RTPMediaFormat) format.getRtpFormat()).getFormatSettings();
		if (fmtps == null)
			fmtps = new HashMap<String, String>();
		if (packetizationMode != null) {
			fmtps.put(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, packetizationMode);
		}

		VideoFormat vf = new VideoFormat(videoFormat.getEncoding());
		vf.setSize(size);
		vf.setFrameRate(videoFormat.getFrameRate());		
		RTPMediaFormat rmf = new RTPMediaFormat(vf);
		rmf.setFormatSettings(fmtps);
		outputFormat = rmf.getFormat();
	 ...
	 * 
	 * @param packetizationMode
	 *            the packetization mode to be used for the H.264 RTP payload
	 *            output by this <tt>JNIEncoder</tt> and the associated
	 *            packetizer
	 */
