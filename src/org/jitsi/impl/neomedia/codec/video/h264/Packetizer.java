...
public class Packetizer extends AbstractPacketizer {
	 ...
	 * as output.
	 */
	static final  Format[] SUPPORTED_OUTPUT_FORMATS = { new RTPMediaFormat(new VideoFormat(VideoFormat.H264_RTP), RTPMediaType.VIDEO,RTPMediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, makeFormatSettings(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, "0"),null).getFormat(), new RTPMediaFormat(new VideoFormat(VideoFormat.H264_RTP), RTPMediaType.VIDEO,RTPMediaFormat.RTP_PAYLOAD_TYPE_UNKNOWN, makeFormatSettings(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, "1"),null).getFormat() };
	 ...
	 * 
	 * @param byteStream
	 *            the H.264 encoded byte stream composed of NAL units in which
	 *            the index of the beginning of the start_code_prefix_one_3bytes
	 *            of a NAL unit is to be found
	 * @param beginIndex
	 *            the inclusive index in <tt>byteStream</tt> at which the search
	 *            is to begin
	 * @param endIndex
	 *            the exclusive index in <tt>byteStream</tt> at which the search
	 *            is to end
	...
	private Format[] getMatchingOutputFormats(Format input) {
		...
		VideoFormat vf = new VideoFormat(VideoFormat.H264_RTP);
		vf.setSize(size);
		vf.setFrameRate(frameRate);
		RTPMediaFormat rmf = new RTPMediaFormat(vf);
		rmf.setFormatSettings(makeFormatSettings(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, packetizationMode));
		return new Format[] { rmf.getFormat() };
	}
	 ...
	 * 
	 * @param format
	 *            the <tt>Format</tt> which assigns a value to the
	...
	private String getPacketizationMode(Format format) {
		...
		if (format instanceof VideoFormat && format.getRtpFormat() != null)
			packetizationMode = format.getRtpFormat().getFormatSettings().containsKey(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP)?format.getRtpFormat().getFormatSettings().get(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP):null;
	 ...
	 *             if something goes wrong during initialization of the
	 *             Packetizer.
	 */
	@Override
	 ...
	 *            be packetized
	 * @param nalOffset
	 *            the offset in <tt>nal</tt> at which the NAL unit of H.264
	 *            encoded data to be packetized begins
	 * @param nalLength
	 *            the length in <tt>nal</tt> beginning at <tt>nalOffset</tt> of
	 *            the NAL unit of H.264 encoded data to be packetized
	...
	public int process(Buffer inBuffer, Buffer outBuffer) {
		...
		if (nals.size() > 0) {
			...
			if (nals.size() > 0)
				 ...
				 * NAL at a time, do NOT always mark it i.e. the NALs with a
				 * value for nal_unit_type which signals that they cannot be the
				 * last NALs in an access unit should probably NOT be marked
				 * anyway.
				 */
				if (nal.length > 0) {
	...
	public Format setOutputFormat(Format format) {
		...
		if (format instanceof VideoFormat && format.getRtpFormat() != null)
			fmtps = format.getRtpFormat().getFormatSettings();
		if (fmtps == null)
			fmtps = new HashMap<String, String>();
		if (fmtps.get(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP) == null) {
			fmtps.put(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP, getPacketizationMode(inputFormat));
		}
