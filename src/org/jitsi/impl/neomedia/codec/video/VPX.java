...
/**
 * A wrapper for the libvpx native library. See
 * {@link "http://www.webmproject.org/docs/"}
 * 
 * @author Boris Grozev
 */
public class VPX {
	/**
	 * Operation completed without error. Corresponds to <tt>VPX_CODEC_OK</tt>
	 * from <tt>vpx/vpx_codec.h</tt>
	 */
	public static final int CODEC_OK = 0;

	/**
	 * An iterator reached the end of list. Corresponds to
	 * <tt>VPX_CODEC_LIST_END</tt> from <tt>vpx/vpx_codec.h</tt>
	 */
	public static final int CODEC_LIST_END = 9;

	/**
	 * Use eXternal Memory Allocation mode flag Corresponds to
	 * <tt>VPX_CODEC_USE_XMA</tt> from <tt>vpx/vpx_codec.h</tt>
	 */
	 ...
	 * 
	 * To set this option for an encoder, enable this bit in the value passed to
	 * <tt>vpx_enc_cft_set_error_resilient</tt> for the encoder's configuration.
	 * 
	...
	/**
	 * I420 format constant Corresponds to <tt>VPX_IMG_FMT_I420</tt> from
	 * <tt>vpx/vpx_image.h</tt>
	 */
	public static final int IMG_FMT_I420 = 258;

	/**
	 * Variable Bitrate mode. Corresponds to <tt>VPX_VBR</tt> from
	 * <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int RC_MODE_VBR = 0;

	/**
	 * Constant Bitrate mode. Corresponds to <tt>VPX_CBR</tt> from
	 * <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int RC_MODE_CBR = 1;

	/**
	 * Constant Quality mode. Corresponds to <tt>VPX_CQ</tt> from
	 * <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int RC_MODE_CQ = 2;

	/**
	 * Encoder determines optimal placement automatically. Corresponds to
	 * <tt>VPX_KF_AUTO</tt> from in <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int KF_MODE_AUTO = 1;

	/**
	 * Encoder does not place keyframes. Corresponds to <tt>VPX_KF_DISABLED</tt>
	 * from <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int KF_MODE_DISABLED = 1;

	/**
	 * Process and return as soon as possible ('realtime' deadline) Corresponds
	 * to <tt>VPX_DL_REALTIME</tt> from <tt>vpx/vpx_encoder.h</tt>
	 */
	public static final int DL_REALTIME = 1;

	/**
	 * Compressed video frame packet type. Corresponds to
	 * <tt>VPX_CODEC_CX_FRAME_PKT</tt> from <tt>vpx/vpx_encoder.h</tt>
	 */
	 ...
	 * 
	 * @param context
	 *            Pointer to a pre-allocated <tt>vpx_codec_ctx_t</tt>.
	 * @param iface
	 *            Interface to be used. Has to be one of the
	 *            <tt>VPX.INTERFACE_*</tt> constants.
	 * @param cfg
	 *            Pointer to a pre-allocated <tt>vpx_codec_dec_cfg_t</tt>, may
	 *            be 0.
	 * @param flags
	 *            Flags.
	 * 
	 ...
	 * @param buf_offset
	 *            Offset into <tt>buf</tt> where the encoded frame begins.
	 * @param buf_size
	 *            Size of the encoded frame.
	 * @param user_priv
	 *            Application specific data to associate with this frame.
	 * @param deadline
	 *            Soft deadline the decoder should attempt to meet, in
	 *            microseconds. Set to zero for unlimited.
	 * 
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The
	 *         error code can be converted to a <tt>String</tt> with
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_decode(long context, byte[] buf, int buf_offset, int buf_size, long user_priv, long deadline);

	/**
	 * Gets the next frame available to display from the decoder context
	 * <tt>context</tt>. The list of available frames becomes valid upon
	 * completion of the <tt>codec_decode</tt> call, and remains valid until the
	 * next call to <tt>codec_decode</tt>.
	 * 
	 * @param context
	 *            The decoder context to use.
	 * @param iter
	 *            Iterator storage, initialized by setting its first element to
	 *            0.
	 * 
	 ...
	 * 
	 * @param context
	 *            Pointer to the <tt>vpx_codec_ctx_t</tt> context to destroy.
	 * 
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The
	 *         error code can be converted to a <tt>String</tt> with
	 *         {@link VPX#codec_err_to_string(int)}
	 ...
	 * stream. In cases where the stream is not available at allocation time, a
	 * fixed size must be requested. The codec will not be able to operate on
	 * streams larger than the size used at allocation time.
	 * 
	 * @param context
	 *            The codec context to use.
	 * @param mmap
	 *            Pointer to a <tt>vpx_codec_mmap_t</tt> to populate
	 * @param iter
	 *            Iterator storage, initialized by setting its first element to
	 *            0.
	 * 
	 ...
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_get_mem_map(long context, long mmap[], long[] iter);
	 ...
	 *            The codec context to use.
	 * @param mmap
	 *            Pointer to the first memory map entry in the list.
	 * @param count
	 *            Number of entries being set at this time.
	 * 
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The
	 *         error code can be converted to a <tt>String</tt> with
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_set_mem_map(long context, long mmap, int count);
	 ...
	 *            <tt>VPX.INTERFACE_*</tt> constants.
	 * @param cfg
	 *            Pointer to a pre-allocated <tt>vpx_codec_enc_cfg_t</tt>, may
	 *            be 0.
	 * @param flags
	 *            Flags.
	 * 
	 ...
	 * 
	 * @param context
	 *            Pointer to the codec context on which to set the confirutation
	 * @param cfg
	 *            Pointer to a <tt>vpx_codec_enc_cfg_t</tt> to set.
	 * 
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The
	 *         error code can be converted to a <tt>String</tt> with
	 *         {@link VPX#codec_err_to_string(int)}
	 ...
	 * 
	 * @param context
	 *            Pointer to the codec context to use.
	 * @param img
	 *            Pointer to a <tt>vpx_image_t</tt> describing the raw frame
	 * @param buf
	 *            Contains the raw frame
	 * @param offset0
	 *            Offset of the first plane
	 * @param offset1
	 *            Offset of the second plane
	 * @param offset2
	 ...
	 * @param flags
	 *            Flags to use for encoding this frame.
	 * @param deadline
	 ...
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_encode(long context, long img, byte[] buf, int offset0, int offset1, int offset2, long pts, long duration, long flags, long deadline);

	/**
	 * Encoded data iterator. Iterates over a list of data packets to be passed
	 * from the encoder to the application. The kind of a packet can be
	 * determined using {@link VPX#codec_cx_pkt_get_kind} Packets of kind
	 * <tt>CODEC_CX_FRAME_PKT</tt> should be passed to the application's muxer.
	 * 
	 * @param context
	 *            The codec context to use.
	 * @param iter
	 ...
	 * 
	 * @param pkt
	 *            Pointer to a <tt>vpx_codec_cx_pkt_t</tt>.
	 * 
	 ...
	 * 
	 * @param pkt
	 *            Pointer to the <tt>vpx_codec_cx_pkt_t</tt>.
	 ...
	 * 
	 * @param img
	 *            Pointer to a <tt>vpx_image_t</tt>.
	 * 
	 ...
	 *            Height of the image.
	 * @param align
	 *            Alignment, in bytes, of each row in the image.
	 * @param data
	 *            Storage to use for the image
	 */
	 ...
	 * 
	 * @param cfg
	 *            Pointer to a <tt>vpx_codec_dec_cfg_t</tt>.
	 * @param value
	 *            The value to set.
	 */
	 ...
	 * 
	 * @param iface
	 *            Interface. Should be one of the <tt>INTERFACE_*</tt> constants
	 * @param cfg
	...
	/**
	 * Sets the <tt>rc_min_quantizer</tt> field of a
	 * <tt>vpx_codec_enc_cfg_t</tt>.
	 * 
	 * @param cfg
	 *            Pointer to a <tt>vpx_codec_enc_cfg_t</tt>.
	...
	/**
	 * Sets the <tt>rc_max_quantizer</tt> field of a
	 * <tt>vpx_codec_enc_cfg_t</tt>.
	 * 
	 * @param cfg
	 *            Pointer to a <tt>vpx_codec_enc_cfg_t</tt>.
	...
	/**
	 * Sets the <tt>rc_undershoot_pct</tt> field of a
	 * <tt>vpx_codec_enc_cfg_t</tt>.
	 * 
	 * @param cfg
	 *            Pointer to a <tt>vpx_codec_enc_cfg_t</tt>.
	 ...
	 * 
	 * @param stream_info
	 *            Pointer to a <tt>vpx_codec_stream_info_t</tt>.
	 * 
	 ...
	 * context is not necessary. Can be used to determine if the bitstream is of
	 * the proper format, and to extract information from the stream.
	 * 
	 * @param iface
	 *            Interface, should be one of the <tt>INTERFACE_*</tt>
	 *            constants.
	 * @param buf
	 *            Buffer containing a compressed frame.
	 * @param buf_offset
	 *            Offset into <tt>buf</tt> where the compressed frame begins.
	 * @param buf_size
	 *            Size of the compressed frame.
	 * @param si_ptr
	 *            Pointer to a <tt>vpx_codec_stream_info_t</tt> which will be
	 *            filled with information about the compressed frame.
	 * 
	 * @return <tt>CODEC_OK</tt> on success, or an error code otherwise. The
	 *         error code can be converted to a <tt>String</tt> with
	 *         {@link VPX#codec_err_to_string(int)}
	 */
	public static native int codec_peek_stream_info(int iface, byte[] buf, int buf_offset, int buf_size, long si_ptr);

	/**
	 * Gets the <tt>sz</tt> field of a <tt>vpx_codec_mmap_t</tt>.
	 * 
	 * @param mmap
	 *            Pointer to a <tt>vpx_codec_mmap_t</tt>
	 ...
	 */
	public static native int codec_err_to_string(int err, byte[] buf, int buf_size);
	...
	static class StreamInfo {
		 ...
		 *            constants.
		 * @param buf
		 *            Buffer containing a compressed frame to parse.
		 * @param buf_offset
		 *            Offset into buffer where the compressed frame begins.
		 * @param buf_size
		 *            Size of the compressed frame.
		 */
