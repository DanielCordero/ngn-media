...
/**
 * Implements a video <tt>Effect</tt> which horizontally flips <tt>AVFrame</tt>
 * s.
 * 
...
public class HFlip extends AbstractCodec2 implements Effect {
	 ...
	 *             if any of the required resource cannot be allocated
		...
		if (graph == 0) {
			...
			if (graphIsPending) {
				...
				if (graph == 0)
					...
					if (error == 0) {
						/*
						 * Unfortunately, the name of an AVFilterContext created
						 * by avfilter_graph_parse is not the name of the
						 * AVFilter.
						 */
