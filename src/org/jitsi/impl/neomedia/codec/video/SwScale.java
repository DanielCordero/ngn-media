...
public class SwScale extends AbstractCodec {
	 ...
	 * 
	 * @param format
	 *            the <tt>VideoFormat</tt> which is the template for the
	 *            <tt>VideoFormat</tt> to be returned
	 * @param size
	 *            the size i.e. width and height of the <tt>VideoFormat</tt> to
	 *            be returned
	...
	private static VideoFormat setSize(VideoFormat format, Dimension size) {
		...
		if (format instanceof RGBFormat) {
			...
			format = new RGBFormat(size,
			/* maxDataLength */Format.NOT_SPECIFIED, dataType, format.getFrameRate(), bitsPerPixel, rgbFormat.getRedMask(), rgbFormat.getGreenMask(), rgbFormat.getBlueMask(), pixelStride, ((pixelStride == Format.NOT_SPECIFIED) || (size == null)) ? Format.NOT_SPECIFIED : (pixelStride * size.width) /* lineStride */, rgbFormat.getFlipped(), rgbFormat.getEndian());
		} else if (format instanceof YUVFormat) {
			YUVFormat yuvFormat = (YUVFormat) format;

			format = new YUVFormat(size,
			/* maxDataLength */Format.NOT_SPECIFIED, format.getDataType(), format.getFrameRate(), yuvFormat.getYuvType(),
			/* strideY */Format.NOT_SPECIFIED,
			/* strideUV */Format.NOT_SPECIFIED, 0,
	 ...
	 */
	private VideoFormat[] supportedOutputFormats = new VideoFormat[] { new RGBFormat(), new YUVFormat(YUVFormat.YUV_420) };
	 ...
	 * 
	 * @param fixOddYuv420Size
	 *            <tt>true</tt> to have the new instance keep the width and
	 *            height of YUV 420 output even; otherwise, <tt>false</tt>
	 */
	 ...
	 *            <tt>true</tt> to have the new instance preserve the aspect
	 *            ratio of the video frames provided to it as input to be
	...
	public SwScale(boolean fixOddYuv420Size, boolean preserveAspectRatio) {
		...
		inputFormats = new Format[] { new AVFrameFormat(), new RGBFormat(), new YUVFormat(YUVFormat.YUV_420) };
	 ...
	 * 
	 * @param input
	 *            input format to get supported output ones for
	...
	public Format[] getSupportedOutputFormats(Format input) {
		...
		/*
		 * if size is set for element 0 (YUVFormat), it is also set for element
		 * 1 (RGBFormat) and so on...
		 */
		Dimension size = supportedOutputFormats[0].getSize();

		/*
		 * no specified size set so return the same size as input in output
		 * format supported
		 */
		...
		return new Format[] { new RGBFormat(size,
		/* maxDataLength */Format.NOT_SPECIFIED,
		/* dataType */null, frameRate, 32,
		/* red */Format.NOT_SPECIFIED,
		/* green */Format.NOT_SPECIFIED,
		/* blue */Format.NOT_SPECIFIED), new YUVFormat(size,
		/* maxDataLength */Format.NOT_SPECIFIED,
		/* dataType */null, frameRate, YUVFormat.YUV_420,
		/* strideY */Format.NOT_SPECIFIED,
	 ...
	 * 
	 * @param in
	 *            the input size which defines the aspect ratio
	 * @param out
	 *            the output size which defines the rectangle into which the
	...
	private Dimension preserveAspectRatio(Dimension in, Dimension out) {
		 ...
		 */
		if ((preserveAspectRatioCachedIn != null) && (preserveAspectRatioCachedOut != null) && (preserveAspectRatioCachedIn.height == inHeight) && (preserveAspectRatioCachedIn.width == inWidth) && (preserveAspectRatioCachedOut.height == outHeight) && (preserveAspectRatioCachedOut.width == outWidth) && (preserveAspectRatioCachedRet != null))
		...
		if (scale) {
			 ...
			 */
			if ((Math.abs(retHeight - outHeight) > 1) || (Math.abs(retWidth - outWidth) > 1)) {
	 ...
	 * 
	 * @param in
	 *            the input <tt>Buffer</tt> to process (from)
	 * @param out
	 *            the output <tt>Buffer</tt> to process into
	...
	public int process(Buffer in, Buffer out) {
		...
		if (src instanceof AVFrame) {
			...
			srcFmt = (inFormat instanceof YUVFormat) ? FFmpeg.PIX_FMT_YUV420P : getFFmpegPixelFormat((RGBFormat) inFormat);
			srcPicture = 0;
		}

		swsContext = FFmpeg.sws_getCachedContext(swsContext, inWidth, inHeight, srcFmt, outWidth, outHeight, dstFmt, FFmpeg.SWS_BICUBIC);

		if (srcPicture == 0) {
			FFmpeg.sws_scale(swsContext, src, srcFmt, inWidth, inHeight, 0, inHeight, dst, dstFmt, outWidth, outHeight);
		} else {
			FFmpeg.sws_scale(swsContext, srcPicture, 0, inHeight, dst, dstFmt, outWidth, outHeight);
		}
			...
			if (YUVFormat.YUV_420 == yuvFormat.getYuvType()) {
				...
				if ((size != null) && (size.width > 2) && (size.height > 2)) {
					...
					if ((width != size.width) || (height != size.height)) {
						format = new YUVFormat(new Dimension(width, height),
						/* maxDataLength */Format.NOT_SPECIFIED, yuvFormat.getDataType(), yuvFormat.getFrameRate(), yuvFormat.getYuvType(),
						/* strideY */Format.NOT_SPECIFIED,
						/* strideUV */Format.NOT_SPECIFIED, 0,
						/* offsetU */Format.NOT_SPECIFIED,
	 ...
	 * 
	 * @param size
	 *            the size i.e. width and height to be set on the current
	 ...
	 * 
	 * @param size
	 *            the size to set as the output size
	 */
