...
public class AVFrameFormat extends VideoFormat {
	 ...
	 * 
	 * @param size
	 *            the <tt>Dimension</tt> of the new instance
	 * @param frameRate
	 *            the frame rate of the new instance
	 * @param pixFmt
	 *            the FFmpeg colorspace to be represented by the new instance
	 * @param deviceSystemPixFmt
	 *            the <tt>DeviceSystem</tt>-specific colorspace to be
	...
	public AVFrameFormat(Dimension size, float frameRate, int pixFmt, int deviceSystemPixFmt) {
		...
		if ((pixFmt == NOT_SPECIFIED) && (deviceSystemPixFmt != NOT_SPECIFIED)) {
			logger.warn("Specifying a device system-specific pixel format 0x" + Long.toHexString(deviceSystemPixFmt & 0xffffffffL) + " without a matching FFmpeg pixel format may" + " eventually lead to a failure.", new Throwable());
		}
	 ...
	 * 
	 * @param f
	 *            the <tt>Format</tt> the properties of which are to be copied
	 *            into this instance
	 */
	@Override
