...
public class Packetizer extends AbstractPacketizer {
	 ...
	 */
	private static final Format[] DEFAULT_OUTPUT_FORMATS = { new VideoFormat(VideoFormat.H263P_RTP) };
	 ...
	 * H.263+ encoded data into RTP packets in accord with RFC 4529
	 * "RTP Payload Format for ITU-T Rec. H.263 Video".
	 */
	public Packetizer() {
		inputFormats = new Format[] { new VideoFormat(VideoFormat.H263P) };
	...
	private Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new VideoFormat(VideoFormat.H263P_RTP, inputVideoFormat.getSize(),
		/* maxDataLength */Format.NOT_SPECIFIED, Format.byteArray, inputVideoFormat.getFrameRate()) };
	}
	 ...
	 * 
	 * @param data
	 *            the bytes which contain the H.263+ encoded data to be
	 *            packetized
	 * @param offset
	 *            the offset of H.263+ encoded data to be packetized begins
	 * @param length
	 *            the length of the H.263+ encoded data starting at offset
