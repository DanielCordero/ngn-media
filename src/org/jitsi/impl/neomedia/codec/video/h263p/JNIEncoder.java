...
public class JNIEncoder extends AbstractCodec {
	 ...
	 */
	private static final Format[] DEFAULT_OUTPUT_FORMATS = { new VideoFormat(VideoFormat.H263P) };
	...
	private Format[] getMatchingOutputFormats(Format in) {
		...
		return new VideoFormat[] { new VideoFormat(VideoFormat.H263P, videoIn.getSize(), Format.NOT_SPECIFIED, Format.byteArray, videoIn.getFrameRate()) };
	}
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		FFmpeg.avcodeccontext_set_bit_rate_tolerance(avcontext, bitRate / (frameRate - 1));
		...
		FFmpeg.avcodeccontext_add_flags(avcontext, FFmpeg.CODEC_FLAG_AC_PRED);
		FFmpeg.avcodeccontext_add_flags(avcontext, FFmpeg.CODEC_FLAG_H263P_UMV);
		FFmpeg.avcodeccontext_add_flags(avcontext, FFmpeg.CODEC_FLAG_H263P_SLICE_STRUCT);
	...
	public synchronized int process(Buffer inBuffer, Buffer outBuffer) {
		...
		// encode data
		int outputLength = FFmpeg.avcodec_encode_video(avcontext, out, out.length, avFrame);
	...
	public Format setOutputFormat(Format format) {
		...
		/* maxDataLength */Format.NOT_SPECIFIED, Format.byteArray, videoFormat.getFrameRate());
