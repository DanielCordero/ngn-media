...
public class DePacketizer extends AbstractCodec2 {
	 ...
	 */
	public DePacketizer() {
		super("H263+ DePacketizer", VideoFormat.class, new VideoFormat[] { new VideoFormat(VideoFormat.H263P) });

		inputFormats = new VideoFormat[] { new VideoFormat(VideoFormat.H263P_RTP) };
	}
	...
	protected int doProcess(Buffer inBuffer, Buffer outBuffer) {
		...
		if ((lastSequenceNumber != -1) && ((sequenceNumber - lastSequenceNumber) != 1)) {
			...
			/*
			 * maybe we lost a frame somewhere or the sequence number reach its
			 * maximum number
			 */
		...
		byte out[] = validateByteArraySize(outBuffer, outOffset + dataLength + outputPaddingSize, true);
	 ...
	 * 
	 * @param out
	 *            the buffer in which <tt>outputPaddingSize</tt> number of bytes
	 *            are to be written
	 * @param outOffset
	 ...
	 * 
	 * @param outBuffer
	 *            the output <tt>Buffer</tt> to be reset
