...
public class JNIDecoder extends AbstractCodec {
	 ...
	 */
	public JNIDecoder() {
		inputFormats = new VideoFormat[] { new VideoFormat(VideoFormat.H263P) };
	...
	public boolean checkFormat(Format format) {
		return format.getEncoding().equals(VideoFormat.H263P_RTP);
	}
	...
	public synchronized int process(Buffer in, Buffer out) {
		...
		FFmpeg.avcodec_decode_video(avcontext, avframe, got_picture, (byte[]) in.getData(), in.getLength());
		...
		if (!(outData instanceof AVFrame) || (((AVFrame) outData).getPtr() != avframe)) {
