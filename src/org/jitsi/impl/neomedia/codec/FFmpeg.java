...
public class FFmpeg {
	...
	/**
	 * BGR32 format handled in endian specific manner. It is stored as ABGR on
	 * big-endian and RGBA on little-endian.
	 */
	public static final int PIX_FMT_BGR32;

	/**
	 * BGR32_1 format handled in endian specific manner. It is stored as BGRA on
	 * big-endian and ARGB on little-endian.
	 */
	...
	/**
	 * RGB24 format handled in endian specific manner. It is stored as RGB on
	 * big-endian and BGR on little-endian.
	 */
	...
	/**
	 * RGB32 format handled in endian specific manner. It is stored as ARGB on
	 * big-endian and BGRA on little-endian.
	 */
	public static final int PIX_FMT_RGB32;

	/**
	 * RGB32_1 format handled in endian specific manner. It is stored as RGBA on
	 * big-endian and ABGR on little-endian.
	 */
	 ...
	 *            frame decoded
	 * @param got_picture
	 *            if the decoding has produced a valid picture
	 * @param buf
	 *            the input buffer
	 * @param buf_size
	 *            input buffer size
	 ...
	 */
	public static native int avcodec_decode_video(long ctx, long frame, boolean[] got_picture, byte[] buf, int buf_size);
	 ...
	 * @param samples
	 *            the input buffer containing the samples. The number of samples
	 *            read from this buffer is <tt>frame_size</tt>*<tt>channels</tt>
	 *            , both of which are defined in <tt>ctx</tt>. For PCM audio the
	 *            number of samples read from samples is equal to
	 *            <tt>buf_size</tt>*<tt>input_sample_size</tt>/
	 *            <tt>output_sample_size</tt>.
	 ...
	 *         number of bytes used to encode the data read from the input
	 *         buffer
	 */
	public static native int avcodec_encode_audio(long ctx, byte[] buf, int buf_offset, int buf_size, byte[] samples, int samples_offset);
	 ...
	 * 
	 * @param id
	 *            <tt>CodecID</tt> of the requested encoder
	 ...
	 * 
	 * @param frame
	 *            an <tt>AVFrame *</tt> value which points to the
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> which will be set up to use the
	 *            specified <tt>AVCodec</tt>
	 * @param codec
	 *            the <tt>AVCodec</tt> to use within the <tt>AVCodecContext</tt>
	 * @param options
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to get the samples per packet of
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the average bit rate of
	 * @param bit_rate
	 *            the average bit rate to be set to the specified
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the bit rate of
	 * @param bit_rate_tolerance
	 *            bit rate tolerance
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the number of channels of
	 * @param channels
	 *            the number of channels to set to the specified
	 *            <tt>AVCodecContext</tt>
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the <tt>gop_size</tt> of
	 * @param gop_size
	 *            the maximum number of pictures in a group of pictures i.e. the
	 *            maximum interval between keyframes
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the minimum GOP size of
	 * @param keyint_min
	 *            the minimum GOP size to set on <tt>ctx</tt>
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the maximum B frames of
	 * @param max_b_frames
	 *            maximum B frames
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the pixel format of
	 * @param pix_fmt
	 *            pixel format
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the RTP payload size of
	 * @param rtp_payload_size
	 *            RTP payload size
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the samples per second of
	 * @param sample_rate
	 *            the samples per second to set to the specified
	 *            <tt>AVCodecContext</tt>
	 */
	 ...
	 * 
	 * @param ctx
	 *            the <tt>AVCodecContext</tt> to set the number of thread of
	 * @param thread_count
	 *            number of thread to set
	 */
	 ...
	 * 
	 * @param graph
	 *            a pointer to the <tt>AVFilterGraph</tt> instance to check the
	 *            validity of and configure
	 * @param log_ctx
	 *            the <tt>AVClass</tt> context to be used for logging
	 ...
	 * 
	 * @param graph
	 *            a pointer to the <tt>AVFilterGraph</tt> instance to free
	 */
	 ...
	 * 
	 * @param graph
	 *            a pointer to the <tt>AVFilterGraph</tt> instance where the
	 *            <tt>AVFilterContext</tt> instance with the specified name is
	 *            to be found
	 * @param name
	 *            the name of the <tt>AVFilterContext</tt> instance which is to
	 ...
	 * 
	 * @param graph
	 *            a pointer to the <tt>AVFilterGraph</tt> instance where to link
	 *            the parsed graph context
	 * @param filters
	 *            the <tt>String</tt> to be parsed
	 * @param inputs
	 *            a pointer to a linked list to the inputs of the graph if any;
	 *            otherwise, <tt>0</tt>
	 * @param outputs
	 *            a pointer to a linked list to the outputs of the graph if any;
	 *            otherwise, <tt>0</tt>
	 * @param log_ctx
	 *            the <tt>AVClass</tt> context to be used for logging
	 * @return <tt>0</tt> on success; a negative <tt>AVERROR</tt> on error
	 */
	public static native int avfilter_graph_parse(long graph, String filters, long inputs, long outputs, long log_ctx);
	 ...
	 * 
	 * @param ref
	 *            a pointer to the <tt>AVFilterBufferRef</tt> instance to remove
	 */
	...
	public static native long get_filtered_video_frame(long input, int width, int height, int pixFmt, long buffer, long ffsink, long output);
	 ...
	 */
	public static native long sws_getCachedContext(long ctx, int srcW, int srcH, int srcFormat, int dstW, int dstH, int dstFormat, int flags);
	 ...
	 *            SwsContext native pointer
	 * @param src
	 *            source image (native pointer)
	 * @param srcSliceY
	 *            slice Y of source image
	 * @param srcSliceH
	 *            slice H of source image
	 * @param dst
	 *            destination image (java type)
	 * @param dstFormat
	 *            destination format
	 * @param dstW
	 *            width of destination image
	 ...
	 * @return 0 if success, -1 otherwise
	 */
	public static native int sws_scale(long ctx, long src, int srcSliceY, int srcSliceH, Object dst, int dstFormat, int dstW, int dstH);
	 ...
	 * @return 0 if success, -1 otherwise
	 */
	public static native int sws_scale(long ctx, Object src, int srcFormat, int srcW, int srcH, int srcSliceY, int srcSliceH, Object dst, int dstFormat, int dstW, int dstH);
}
