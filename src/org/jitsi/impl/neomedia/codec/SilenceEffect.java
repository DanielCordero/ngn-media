 ...
 * monitoring the input <tt>Buffer</tt>s' timestamps and lengths, and inserts
 * silence to account for missing data.
 * 
 * @author Boris Grozev
 */
public class SilenceEffect extends AbstractCodec2 implements Effect {
	...
	/**
	 * The indicator which determines whether <tt>SilenceEffect</tt> instances
	 * are to perform the copying of the data from input <tt>Buffer</tt>s to
	 * output <tt>Buffer</tt>s themselves (e.g. using
	 ...
	 */
	public static final Format[] SUPPORTED_FORMATS = new Format[] { new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, 1, // channels
	...
	/**
	 * The clock rate for the timestamps of input <tt>Buffer</tt>s (i.e. the
	 * number of units which constitute one second).
	 */
	 ...
	 * 
	 * @param rtpClockRate
	 *            the clock rate that the RTP timestamps use.
	 */
	...
	protected void doClose() {
		if (logger.isInfoEnabled())
			logger.info("Closing SilenceEffect, inserted a total of " + totalSamplesInserted + " samples of silence.");
	}
	 ...
	 *         to <tt>outBuf</tt>, and <tt>INPUT_BUFFER_NOT_CONSUMED</tt> if
	 *         silence was inserted instead.
	 */
	@Override
	protected int doProcess(Buffer inBuf, Buffer outBuf) {
		...
		} else // timestamp != -1 && lastOutputTimestamp != -1
		{
			...
			if (diffSamples > MAX_SAMPLES_SILENCE) {
				logger.info("More than the maximum of " + MAX_SAMPLES_SILENCE + " samples of silence need to be inserted.");
		...
		if (useInput) {
			...
			lastOutputTimestamp = calculateTimestamp(lastOutputTimestamp, (inLen * 8) / sampleSizeInBits);
		}
	 ...
	 * 
	 * @param oldTimestamp
	 *            the timestamp to which to add.
	 * @param samplesToAdd
	 *            the number of samples to add.
	 * @return the timestamp obtained by adding <tt>samplesToAdd</tt> samples
	 *         (using a sample rate of <tt>this.sampleRate</tt> per second) to
	 *         timestamp (with a clock rate of <tt>this.clockRate</tt> per
	 *         second).
	 */
	 ...
	 * 
	 * @param buf
	 *            the <tt>Buffer</tt> to fill with silence
	 * @param samples
	 *            the number of samples of silence to fill.
