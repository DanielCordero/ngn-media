...
public abstract class AbstractCodec2 extends AbstractCodec {
	 ...
	 *            {@link Buffer#SEQUENCE_UNKNOWN}. May be equal to
	 *            <tt>seqNo</tt> for the purposes of Codec implementations which
	 *            repeatedly process one and the same input Buffer multiple
	 *            times.
	 * @param seqNo
	 *            the current sequence number. May be equal to
	 *            <tt>lastSeqNo</tt> for the purposes of Codec implementations
	 *            which repeatedly process one and the same input Buffer
	 *            multiple times.
	 * @return the number of sequences (between <tt>lastSeqNo</tt> and
	 *         <tt>seqNo</tt>) which have been lost i.e. which have not been
	 *         received
	 */
	 ...
	 * 
	 * @param seqNo
	 *            the sequence number to increment
	 * @return a sequence number which represents an increment over the
	 *         specified <tt>seqNo</tt> within the range of valid RTP sequence
	 *         number values
	 */
	...
	public static YUVFormat specialize(YUVFormat yuvFormat, Class<?> dataType) {
		...
		if ((offsetU == Format.NOT_SPECIFIED) && (strideY != Format.NOT_SPECIFIED) && (size != null))
			offsetU = offsetY + strideY * size.height;

		int offsetV = yuvFormat.getOffsetV();

		if ((offsetV == Format.NOT_SPECIFIED) && (offsetU != Format.NOT_SPECIFIED) && (strideUV != Format.NOT_SPECIFIED) && (size != null))
			offsetV = offsetU + strideUV * ((size.height + 1) / 2);

		int maxDataLength = ((strideY != Format.NOT_SPECIFIED) && (strideUV != Format.NOT_SPECIFIED)) && (size != null) ? (strideY * size.height + 2 * strideUV * ((size.height + 1) / 2) + FFmpeg.FF_INPUT_BUFFER_PADDING_SIZE) : Format.NOT_SPECIFIED;

		return new YUVFormat(size, maxDataLength, (dataType == null) ? yuvFormat.getDataType() : dataType, yuvFormat.getFrameRate(), YUVFormat.YUV_420, strideY, strideUV, offsetY, offsetU, offsetV);
	}
	 ...
	 * @param formatClass
	 *            the <tt>Class</tt> of input and output <tt>Format</tt>s
	 *            supported by the new instance
	 * @param supportedOutputFormats
	 *            the list of <tt>Format</tt>s supported by the new instance as
	 *            output
	 */
	 ...
	 *             if any of the resources that this <tt>Codec</tt> needs to
	 *             operate cannot be acquired
	 */
	 ...
	 * 
	 * @param inputFormat
	 *            the <tt>Format</tt> of the input for which the supported
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return (supportedOutputFormats == null) ? EMPTY_FORMATS : supportedOutputFormats.clone();
	}
	...
	public Format[] getSupportedOutputFormats(Format inputFormat) {
		...
		if (!formatClass.isInstance(inputFormat) || (matches(inputFormat, inputFormats) == null))
	 ...
	 * 
	 * @throws ResourceUnavailableException
	 *             if any of the resources that this <tt>PlugIn</tt> needs to
	 *             operate cannot be acquired
	...
	public int process(Buffer inBuf, Buffer outBuf) {
		 ...
		 */
		if ((((BUFFER_FLAG_FEC | BUFFER_FLAG_PLC) & features) == 0) && ((Buffer.FLAG_SILENCE & inBuf.getFlags()) != 0))
	...
	public Format setInputFormat(Format format) {
		if (!formatClass.isInstance(format) || (matches(format, inputFormats) == null))
			return null;
	...
	public Format setOutputFormat(Format format) {
		if (!formatClass.isInstance(format) || (matches(format, getMatchingOutputFormats(inputFormat)) == null))
			return null;
	 ...
	 * 
	 * @param outputBuffer
	 *            the output <tt>Buffer</tt> to update the properties of
	 * @param format
	 *            the <tt>Format</tt> to set on <tt>outputBuffer</tt>
	 * @param length
	 *            the length to set on <tt>outputBuffer</tt>
	 * @param offset
	 *            the offset to set on <tt>outputBuffer</tt>
	 */
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> whose <tt>data</tt> property value is to
	 *            be validated
	 * @param newSize
	 *            the minimum length of the array of <tt>byte</tt> which is to
	 *            be the value of the <tt>data</tt> property of <tt>buffer</tt>
	 * @param arraycopy
	 *            <tt>true</tt> to copy the bytes which are in the value of the
	 *            <tt>data</tt> property of <tt>buffer</tt> at the time of the
	 *            invocation of the method if the value of the <tt>data</tt>
	 *            property of <tt>buffer</tt> is an array of <tt>byte</tt> whose
	 *            length is less than <tt>newSize</tt>; otherwise,
	 *            <tt>false</tt>
	 * @return an array of <tt>byte</tt>s which is the value of the
	 *         <tt>data</tt> property of <tt>buffer</tt> and whose length is at
	 *         least <tt>newSize</tt> number of bytes
	 */
