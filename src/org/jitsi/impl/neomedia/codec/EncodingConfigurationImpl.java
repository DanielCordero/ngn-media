...
public class EncodingConfigurationImpl extends EncodingConfiguration {
	 ...
	 * some countries and is licensed by <a href="http://www.sipro.com">SIPRO
	 * Lab Telecom</a>.
	 */
	...
	private void initializeFormatPreferences() {
		...
		// video
		setEncodingPreference("H264", RTPMediaFormat.DEFAULT_CLOCK_RATE, 1100);

		setEncodingPreference("H263-1998", RTPMediaFormat.DEFAULT_CLOCK_RATE, 0);

		setEncodingPreference("VP8", RTPMediaFormat.DEFAULT_CLOCK_RATE, 0);
		/*
		 * setEncodingPreference( "H263",
		 * VideoMediaFormatImpl.DEFAULT_CLOCK_RATE, 1000);
		 */
		setEncodingPreference("JPEG", RTPMediaFormat.DEFAULT_CLOCK_RATE, 950);
		setEncodingPreference("H261", RTPMediaFormat.DEFAULT_CLOCK_RATE, 800);
		...
		setEncodingPreference(AudioFormat.TELEPHONE_EVENT, 8000, 1);
	}
	...
	protected void setEncodingPreference(String encoding, double clockRate, int pref) {
		...
		/*
		 * The key in encodingPreferences associated with a RTPMediaFormat is
		 * currently composed of the encoding and the clockRate only so it makes
		 * sense to ignore the format parameters.
		 */
		for (RTPMediaFormat mf : RTPMediaFormatUtils.getMediaFormats(encoding)) {
	 ...
	 */
	@Override
	public RTPMediaFormat[] getAllEncodings(RTPMediaType type) {
		return RTPMediaFormatUtils.getMediaFormats(type);
	}
	 ...
	 */
	@Override
	protected int compareEncodingPreferences(RTPMediaFormat enc1, RTPMediaFormat enc2) {
		...
		if (res == 0) {
			...
			if (res == 0) {
				...
				if (res == 0) {
					...
					if (((index1 = RTPMediaFormatUtils.getMediaFormatIndexWithSameMediaType(enc1)) != -1) && ((index2 = RTPMediaFormatUtils.getMediaFormatIndexWithSameMediaType(enc2)) != -1)) {
						res = (index1 - index2);
					}
					...
					if (res == 0) {
						 ...
						 */
						Map<String, String> fmtps1 = enc1.getFormatSettings();
						Map<String, String> fmtps2 = enc2.getFormatSettings();
