...
public class JNIEncoder extends AbstractCodec2 {
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 */
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.SPEEX_RTP) };
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray);
		}
	}
	...
	protected int doProcess(Buffer inputBuffer, Buffer outputBuffer) {
		...
		if (state == 0) {
			...
			if (state == 0)
				return BUFFER_PROCESSED_FAILED;
			if (Speex.speex_encoder_ctl(state, Speex.SPEEX_SET_QUALITY, 4) != 0)
				return BUFFER_PROCESSED_FAILED;
			if (Speex.speex_encoder_ctl(state, Speex.SPEEX_SET_SAMPLING_RATE, inputSampleRate) != 0)
				return BUFFER_PROCESSED_FAILED;
	...
	protected Format[] getMatchingOutputFormats(Format inputFormat) {
		...
		return new Format[] { new AudioFormat(AudioFormat.SPEEX_RTP, inputAudioFormat.getSampleRate(), Format.NOT_SPECIFIED, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray) };
	}
	...
	public Format setInputFormat(Format format) {
		...
		if (inputFormat != null) {
			...
			if ((outputSampleRate != inputSampleRate) || (outputChannels != inputChannels)) {
				setOutputFormat(new AudioFormat(AudioFormat.SPEEX_RTP, inputSampleRate, Format.NOT_SPECIFIED, inputChannels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED, Format.byteArray));
			}
		}
		return inputFormat;
	}
