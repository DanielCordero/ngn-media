 ...
 **/
public class JavaDecoder extends AbstractCodec {
	/**
	 * Default output formats.
	 */
	protected Format[] DEFAULT_OUTPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.LINEAR) };

	/**
	 * The input format used to initialize the decoder. If null decoder must be
	 * created.
	 */
	...
	public Format[] getSupportedOutputFormats(Format in) {
		...
		if (!(in instanceof AudioFormat) || (null == AbstractCodec2.matches(in, inputFormats)))
			return new Format[0];
	...
	private Format[] getMatchingOutputFormats(Format in) {
		...
		return new AudioFormat[] { new AudioFormat(AudioFormat.LINEAR, af.getSampleRate(), 16, af.getChannels(), AudioFormat.LITTLE_ENDIAN, // isBigEndian(),
