...
public class Opus {
	 ...
	 * packets with a single frame, that's a 1 byte TOC + the maximum frame
	 * size. See http://tools.ietf.org/html/rfc6716#section-3.2
	 */
	 ...
	 * 
	 * @param decoder
	 *            the <tt>OpusDecoder</tt> state to perform the decoding
	 * @param input
	 *            an array of <tt>byte</tt>s which represents the input payload
	 *            to decode. If <tt>null</tt>, indicates packet loss.
	 * @param inputOffset
	 *            the offset in <tt>input</tt> at which the payload to be
	 *            decoded begins
	 * @param inputLength
	 *            the length in bytes in <tt>input</tt> beginning at
	 *            <tt>inputOffset</tt> of the payload to be decoded
	 * @param output
	 *            an array of <tt>byte</tt>s into which the decoded signal is to
	 *            be output
	 * @param outputOffset
	 *            the offset in <tt>output</tt> at which the output of the
	 *            decoded signal is to begin
	 * @param outputFrameSize
	 *            the number of samples per channel <tt>output</tt> beginning at
	 *            <tt>outputOffset</tt> of the maximum space available for
	 *            output of the decoded signal
	 * @param decodeFEC
	 *            0 to decode the packet normally, 1 to decode the FEC data in
	 *            the packet
	 * @return the number of decoded samples written into <tt>output</tt>
	 *         (beginning at <tt>outputOffset</tt>)
	 */
	public static native int decode(long decoder, byte[] input, int inputOffset, int inputLength, byte[] output, int outputOffset, int outputFrameSize, int decodeFEC);

	/**
	 * Creates an OpusDecoder structure, returns a pointer to it or 0 on error.
	 * 
	 * @param Fs
	 *            Sample rate to decode to
	 * @param channels
	 *            number of channels to decode to(1/2)
	 * 
	 ...
	 * 
	 * @param decoder
	 *            Address of the structure (as returned from decoder_create)
	 */
	 ...
	 * @param offset
	 *            Offset into packet where the actual packet begins.
	 * @param length
	 ...
	 *            The encoder to use.
	 * @param input
	 *            Array containing PCM encoded input.
	 * @param inputOffset
	 *            Offset to use into the <tt>input</tt> array
	 * @param inputFrameSize
	 *            The number of samples per channel in <tt>input</tt>.
	 * @param output
	 *            Array where the encoded packet will be stored.
	 * @param outputOffset
	 * @param outputLength
	 *            The number of available bytes in <tt>output</tt>.
	 * 
	 * @return The number of bytes written in <tt>output</tt>, or a negative on
	 *         error.
	 */
	public static native int encode(long encoder, byte[] input, int inputOffset, int inputFrameSize, byte[] output, int outputOffset, int outputLength);
	 ...
	 * @param channels
	 *            number of channels in the input (1/2)
	 * 
	 ...
	 * 
	 * @param encoder
	 *            Address of the structure (as returned from encoder_create)
	 */
	 ...
	 *            The encoder to use
	 * @param bandwidth
	 *            The bandwidth to set, should be one of
	 ...
	 *            The encoder to use
	 * @param complexity
	 *            The complexity level, from 1 to 10
	 * 
	 * @return OPUS_OK on success
	 */
	 ...
	 *            The encoder to use
	 * @param dtx
	 *            0 to turn DTX off, non-zero to turn it on
	 * 
	 * @return OPUS_OK on success
	 */
	 ...
	 *            The encoder to use
	 * @param maxBandwidth
	 *            The maximum bandwidth to use, should be one of
	 *            <tt>BANDWIDTH_FULLBAND</tt>, <tt>BANDWIDTH_MEDIUMBAND</tt>,
	 *            <tt>BANDWIDTH_NARROWBAND</tt>,
	 *            <tt>BANDWIDTH_SUPERWIDEBAND</tt> or
	 *            <tt>BANDWIDTH_WIDEBAND</tt>
	 ...
	 *            The encoder to use
	 * @param vbr
	 *            0 to turn VBR off, non-zero to turn it on.
	 * 
	 * @return OPUS_OK on success
	 */
	 ...
	 *            The encoder to use
	 * @param use_cvbr
	 *            0 to turn VBR constraint off, non-zero to turn it on.
	 * 
	 * @return OPUS_OK on success
	 */
	 ...
	 *         , <tt>BANDWIDTH_WIDEBAND</tt>, or <tt>INVALID_PACKET</tt> on
	 *         error.
	 */
