...
public class JNIEncoder extends AbstractCodec2 implements FormatParametersAwareCodec, PacketLossAwareEncoder {
	/**
	 * The <tt>Logger</tt> used by this <tt>JNIEncoder</tt> instance for logging
	 * output.
	 */
	 ...
	 * <tt>JNIEncoder</tt> instances.
	 */
	private static final Format[] SUPPORTED_OUTPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.OPUS_RTP, 48000,
	...
	static {
		...
		for (int i = 0; i < supportedInputCount; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16, 1, AbstractAudioRenderer.NATIVE_AUDIO_FORMAT_ENDIAN, AudioFormat.SIGNED,
			/* frameSizeInBits */Format.NOT_SPECIFIED,
			/* frameRate */Format.NOT_SPECIFIED, Format.byteArray);
		}
		/*
	...
	protected void doOpen() throws ResourceUnavailableException {
		...
		* cfg.getInt(Constants.PROP_OPUS_BITRATE, 32);
		...
		if (logger.isDebugEnabled()) {
			...
			logger.debug("Encoder settings: audio bandwidth " + bw + ", bitrate " + Opus.encoder_get_bitrate(encoder) + ", DTX " + Opus.encoder_get_dtx(encoder) + ", FEC " + Opus.encoder_get_inband_fec(encoder));
		}
	}
	 ...
	 * 
	 * @param inBuffer
	 *            the <tt>Buffer</tt> from which the media to be encoded is to
	 *            be read
	 * @param outBuffer
	 *            the <tt>Buffer</tt> into which the encoded media is to be
	 *            written
	...
	public Format getOutputFormat() {
		...
		if ((f != null) && (f.getClass() == AudioFormat.class)) {
			...
			f = setOutputFormat(new AudioFormat(af.getEncoding(), af.getSampleRate(), af.getSampleSizeInBits(), af.getChannels(), af.getEndian(), af.getSigned(), af.getFrameSizeInBits(), af.getFrameRate(), af.getDataType()) {
				@Override
	 ...
	 * 
	 * @param percentage
	 *            the expected packet loss percentage to set
	 */
	@Override
	public void setExpectedPacketLoss(int percentage) {
		if (opened) {
			Opus.encoder_set_packet_loss_perc(encoder, (percentage > minPacketLoss) ? percentage : minPacketLoss);
			if (logger.isTraceEnabled()) {
				logger.trace("Updating expected packet loss: " + percentage + " (minimum " + minPacketLoss + ")");
			}
		}
	}
	...
	public void setFormatParameters(Map<String, String> fmtps) {
		 ...
		 * http://
		 * tools.ietf.org/html/draft-spittka-payload-rtp-opus-02#section-6.1
		 */
		...
		String s;
		boolean useFec = this.useFec && (((s = fmtps.get("useinbandfec")) == null) || s.equals("1"));
		Opus.encoder_set_inband_fec(encoder, useFec ? 1 : 0);
	}
