...
public class AudioMediaStreamImpl extends MediaStreamImpl implements AudioMediaStream, PropertyChangeListener {
	 ...
	 * parameters.
	 */
	new AudioFormat(AudioFormat.ALAW_RTP, 8000, 8, 1, Format.NOT_SPECIFIED, AudioFormat.SIGNED), new AudioFormat(AudioFormat.G722_RTP, 8000, Format.NOT_SPECIFIED /* sampleSizeInBits */, 1) };
	...
	/**
	 * The listener that gets notified of changes in the audio level of remote
	 ...
	 *            capture and playback of audio exchanged via the specified
	 ...
	 * 
	 * @param listener
	 *            the <tt>DTMFListener</tt> to register for notifications about
	 *            the remote party starting sending of DTM tones to this
	 *            <tt>AudioMediaStream</tt>
	 ...
	 * {@link MediaStreamImpl#addRTPExtension(byte, RTPExtension)} this method
	 * enables sending of CSRC audio levels. The reason we are doing this here
	 * rather than in the super class is that CSRC levels only make sense for
	 * audio streams so we don't want them enabled in any other type.
	 * 
	 * @param extensionID
	 *            the ID assigned to <tt>rtpExtension</tt> for the lifetime of
	 *            this stream.
	 * @param rtpExtension
	 *            the RTPExtension that is being added to this stream.
	 */
	@Override
	public void addRTPExtension(byte extensionID, RTPExtension rtpExtension) {
		...
		if ((csrcEngine != null) || (ssrcEngine != null)) {
			...
			if (csrcEngine != null) {
				csrcEngine.setCsrcAudioLevelExtensionID((csrcExtID == null) ? -1 : csrcExtID.byteValue(), csrcDir);
			}
			if (ssrcEngine != null) {
				ssrcEngine.setSsrcAudioLevelExtensionID((ssrcExtID == null) ? -1 : ssrcExtID.byteValue(), ssrcDir);
			}
		}
	}
	 ...
	 * 
	 * @param audioLevels
	 *            a array mapping CSRC IDs to audio levels in consecutive
	 *            elements.
	 */
	 ...
	 *            the new tone
	 * @param end
	 *            <tt>true</tt> if the tone is to be ended or <tt>false</tt> to
	 *            be started
	 */
	...
	/**
	 * Returns the <tt>MediaDeviceSession</tt> associated with this stream after
	 * first casting it to <tt>AudioMediaDeviceSession</tt> since this is, after
	 * all, an <tt>AudioMediaStreamImpl</tt>.
	 * 
	...
	/**
	 * The priority of the audio is 3, which is meant to be higher than other
	 * threads and higher than the video one.
	 * 
	 ...
	 * 
	 * @param ev
	 *            The event which may contain a audio system change event.
	 */
	 ...
	 * 
	 * @param rtpManager
	 *            the <tt>RTPManager</tt> to register
	...
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager) {
		...
		for (AudioFormat format : CUSTOM_CODEC_FORMATS) {
			 ...
			 */
			rtpManager.addFormat(format, RTPMediaFormatUtils.getRTPPayloadType(format.getEncoding(), format.getSampleRate()));
		}
	}
	 ...
	 * 
	 * @param listener
	 *            the <tt>DTMFListener</tt> to no longer be notified by this
	 *            <tt>AudioMediaStream</tt> about incoming DTMF tones
	 ...
	 * 
	 * @param listener
	 *            the <tt>CsrcAudioLevelListener</tt> that we'd like to register
	 *            or <tt>null</tt> if we'd like to stop receiving notifications.
	 */
	 ...
	 * 
	 * @param listener
	 *            the <tt>SimpleAudioLevelListener</tt> that we'd like to
	 ...
	 * @param listener
	 *            the <tt>SimpleAudioLevelListener</tt> that we'd like to
	 *            register or <tt>null</tt> if we want to stop stream audio
	 *            level measurements.
	 */
	 ...
	 *             if <tt>dtmfMethod</tt> is not one of
	 *             {@link DTMFMethod#INBAND_DTMF}, {@link DTMFMethod#RTP_DTMF},
	 *             and {@link DTMFMethod#SIP_INFO_DTMF}
	 * @see AudioMediaStream#startSendingDTMF(DTMFTone, DTMFMethod, int, int,
	 *      int)
	 */
