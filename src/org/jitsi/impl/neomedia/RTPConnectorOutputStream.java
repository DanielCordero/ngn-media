...
public abstract class RTPConnectorOutputStream implements OutputDataStream {
	 ...
	 */
	private final LinkedBlockingQueue<RawPacket> rawPacketPool = new LinkedBlockingQueue<RawPacket>();
	 ...
	 */
	private final LinkedBlockingQueue<RawPacket[]> rawPacketArrayPool = new LinkedBlockingQueue<RawPacket[]>();

	/**
	 * Used for debugging. As we don't log every packet we must count them and
	 * decide which to log.
	 */
	 ...
	 * 
	 * Allows extenders to intercept the array and possibly filter and/or modify
	 * it.
	 * 
	 * @param buffer
	 *            the packet data to be sent to the targets of this instance
	 * @param offset
	 *            the offset of the packet data in <tt>buffer</tt>
	 * @param length
	 *            the length of the packet data in <tt>buffer</tt>
	 ...
	 * 
	 * @param numOfPacket
	 *            the number of the <tt>RawPacket</tt> in the total number of
	 *            sent <tt>RawPacket</tt>s
	 * @return <tt>true</tt> if the <tt>RawPacket</tt> with the specified
	 *         <tt>numOfPacket</tt> is to be logged by
	 *         <tt>PacketLoggingService</tt>; otherwise, <tt>false</tt>
	 */
	 ...
	 * 
	 * @param packet
	 *            the RTP packet to be sent through the <tt>DatagramSocket</tt>
	 *            of this <tt>OutputDataSource</tt>
	 ...
	 * <tt>OutputDataStream</tt> through its <tt>DatagramSocket</tt> per a
	 * specific number of milliseconds.
	 * 
	 * @param maxPackets
	 *            the maximum number of RTP packets to be sent by this
	 *            <tt>OutputDataStream</tt> through its <tt>DatagramSocket</tt>
	 *            per the specified number of milliseconds; <tt>-1</tt> if no
	 *            maximum is to be set
	 * @param perMillis
	 *            the number of milliseconds per which <tt>maxPackets</tt> are
	 *            to be sent by this <tt>OutputDataStream</tt> through its
	...
	public void setMaxPacketsPerMillis(int maxPackets, long perMillis) {
		if (maxPacketsPerMillisPolicy == null) {
			if (maxPackets > 0) {
				...
				maxPacketsPerMillisPolicy = new MaxPacketsPerMillisPolicy(maxPackets, perMillis);
			}
		} else {
			maxPacketsPerMillisPolicy.setMaxPacketsPerMillis(maxPackets, perMillis);
		}
	}
	...
	private class MaxPacketsPerMillisPolicy {
		 ...
		 */
		private final ArrayBlockingQueue<RawPacket> packetQueue = new ArrayBlockingQueue<RawPacket>(MAX_PACKETS_PER_MILLIS_POLICY_PACKET_QUEUE_CAPACITY);
		 ...
		 * 
		 * @param maxPackets
		 *            the maximum number of RTP packets to be sent per
		 *            <tt>perMillis</tt> milliseconds through the
		 *            <tt>DatagramSocket</tt> of this <tt>OutputDataStream</tt>
		 * @param perMillis
		 *            the number of milliseconds per which a maximum of
		 ...
		 * @param maxPackets
		 *            the maximum number of RTP packets to be sent by this
		 *            <tt>OutputDataStream</tt> through its
		 *            <tt>DatagramSocket</tt> per the specified number of
		 *            milliseconds; <tt>-1</tt> if no maximum is to be set
		 * @param perMillis
		 *            the number of milliseconds per which <tt>maxPackets</tt>
		 *            are to be sent by this <tt>OutputDataStream</tt> through
		 *            its <tt>DatagramSocket</tt>
		 */
		public void setMaxPacketsPerMillis(int maxPackets, long perMillis) {
			if (maxPackets < 1) {
				this.maxPackets = -1;
		 ...
		 * 
		 * @param packet
		 *            the RTP packet to be queued for sending through the
		 *            <tt>DatagramSocket</tt> of this <tt>OutputDataStream</tt>
		 */
	...
	/**
	 * Enables or disables this <tt>RTPConnectorOutputStream</tt>. While the
	 * stream is disabled, it suppresses actually sending any packets via
	 * {@link #send(RawPacket)}.
	 * 
	 * @param enabled
