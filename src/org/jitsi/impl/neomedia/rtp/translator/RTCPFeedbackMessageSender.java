...
public class RTCPFeedbackMessageSender {
	 ...
	 * 
	 * @param rtpTranslator
	 *            the <tt>RTPTranslatorImpl</tt> through which the new instance
	 *            is to send RTCP feedback message packets and the SSRC of which
	...
	private int getNextSequenceNumber(int sourceSSRC, int targetSSRC) {
		synchronized (sequenceNumbers) {
			Long key = Long.valueOf(((sourceSSRC & 0xffffffffl) << 32) | (targetSSRC & 0xffffffffl));
	...
	public boolean sendFIR(int mediaSenderSSRC) {
		 ...
		 * RED).
		 */
		for (StreamRTPManager streamRTPManager : rtpTranslator.getStreamRTPManagers()) {
	 ...
	 * 
	 * @param destination
	 *            the <tt>MediaStream</tt> through which the FIR command is to
	 *            be sent
	 * @param mediaSenderSSRC
	 *            the SSRC of the media sender/source
	 * @return <tt>true</tt> if a FIR command was sent; otherwise,
	...
	public boolean sendFIR(MediaStream destination, int mediaSenderSSRC) {
		...
		RTCPFeedbackMessagePacket fir = new RTCPFeedbackMessagePacket(RTCPFeedbackMessageEvent.FMT_FIR, RTCPFeedbackMessageEvent.PT_PS, senderSSRC, 0xffffffffl & mediaSenderSSRC);

		fir.setSequenceNumber(getNextSequenceNumber((int) senderSSRC, mediaSenderSSRC));
	 ...
	 * 
	 * @param destination
	 *            the <tt>MediaStream</tt> through which the FIR commands are to
	 *            be sent
