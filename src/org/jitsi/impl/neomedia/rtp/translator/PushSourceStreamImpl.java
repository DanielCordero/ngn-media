...
class PushSourceStreamImpl implements PushSourceStream, Runnable, SourceTransferHandler {
	...
	/**
	 * The indicator which determines whether {@link #read(byte[], int, int)}
	 * read a <tt>SourcePacket</tt> from {@link #readQ} after a
	 * <tt>SourcePacket</tt> was written there.
	 */
	...
	/**
	 * The pool of <tt>SourcePacket</tt> instances to reduce their allocations
	...
	public PushSourceStreamImpl(RTPConnectorImpl connector, boolean data) {
		...
		readQCapacity = RTPConnectorOutputStream.MAX_PACKETS_PER_MILLIS_POLICY_PACKET_QUEUE_CAPACITY;
	 ...
	 * {@inheritDoc}
	 * 
	 * Implements {@link SourceTransferHandler#transferData(PushSourceStream)}.
	 * This instance sets itself as the <tt>transferHandler</tt> of all
	 * <tt>PushSourceStream</tt>s that get added to it (i.e. {@link #streams}).
	 * When either one of these pushes media data, this instance pushes that
	 * media data.
	 */
	@Override
