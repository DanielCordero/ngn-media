...
public class RTPTranslatorImpl extends AbstractRTPTranslator implements ReceiveStreamListener {
	...
	public static long getPayloadLengthAndOffsetIfRTP(byte[] buf, int off, int len) {
		...
		if (/* extension (X) */(b0 & 0x10) != 0) {
			if (payloadLen < /* defined by profile */2 + /* length */2) {
				...
				int extensionLen = readUnsignedShort(buf, payloadOff + 2);
	 ...
	 * 
	 * @param obj
	 *            the object which is the source of the log request
	 * @param methodName
	 *            the name of the method on <tt>obj</tt> which is the source of
	 *            the log request
	 * @param buffer
	 *            the <tt>byte</tt>s which (possibly) represent an RTCP packet
	 *            to be logged for debugging purposes
	 * @param offset
	 *            the position within <tt>buffer</tt> at which the valid data
	 *            begins
	 * @param length
	 *            the number of bytes in <tt>buffer</tt> which constitute the
	 *            valid data
	 */
		...
		if (length >= 8 /* BYE */) {
			...
			if (v == RTCPHeader.VERSION) {
				...
				if (pt == 203 /* BYE */) {
					...
					int rtcpLength = (readUnsignedShort(buffer, offset + 2) + 1) * 4;

					if (rtcpLength <= length) {
						int sc = b0 & 0x1f;
						int off = offset + 4;

						for (int i = 0, end = offset + length; (i < sc) && (off + 4 <= end); i++, off += 4) {
							int ssrc = readInt(buffer, off);

							logger.trace(obj.getClass().getName() + '.' + methodName + ": RTCP BYE SSRC/CSRC " + Long.toString(ssrc & 0xffffffffl));
						}
					}
				}
			}
	 ...
	 * 
	 * @param buf
	 *            the <tt>byte</tt> buffer to read an <tt>int</tt> from
	 * @param off
	 *            the zero-based offset in <tt>buf</tt> to start reading an
	 *            <tt>int</tt> from
	 ...
	 * 
	 * @param buf
	 *            the <tt>byte</tt> buffer to read a 16-bit unsigned value from
	 * @param off
	 *            the zero-based offset in <tt>buf</tt> to start reading a
	 ...
	 * <tt>RTCPReportBuilder</tt> of the active <tt>RTCPTerminationStrategy</tt>
	 * .
	 */
	private final DelegatingRTCPReportBuilder delegatingRTCPReportBuilder = new DelegatingRTCPReportBuilder();
	...
	/**
	 * An instance which can be used to send RTCP Feedback Messages, using as
	 * 'packet sender SSRC' the SSRC of (the <tt>RTPManager</tt> of) this
	 * <tt>RTPTranslator</tt>.
	 */
	 ...
	 *            the <tt>StreamRTPManager</tt> that is requesting the
	 *            association of <tt>format</tt> to <tt>payloadType</tt>
	 * @param format
	 *            the <tt>Format</tt> which is to be associated with the
	 *            specified RTP payload type (number)
	 * @param payloadType
	 *            the RTP payload type (number) to be associated with the
	 ...
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> which specifies the neomedia
	 *            <tt>MediaStream</tt> with which the
	 *            <tt>ReceiveStreamEvent</tt>s delivered to the specified
	 *            <tt>listener</tt> are to be related. In other words, a
	 *            <tt>ReceiveStreamEvent</tt> received by
	 *            <tt>RTPTranslatorImpl</tt> is first examined to determine
	 *            which <tt>StreamRTPManager</tt> it is related to and then it
	 *            is delivered to the <tt>ReceiveStreamListener</tt>s which have
	 *            been added to this <tt>RTPTranslatorImpl</tt> by that
	 *            <tt>StreamRTPManager</tt>.
	 * @param listener
	 *            the <tt>ReceiveStreamListener</tt> to be notified about
	 ...
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> which is requesting the addition
	 *            of the specified <tt>RemoteListener</tt>
	 * @param listener
	 *            the <tt>RemoteListener</tt> to be notified about
	 *            <tt>RemoteEvent</tt>s received by this
	 ...
	 * 
	 * @param sendStreamDesc
	 *            a <tt>SendStreamDesc</tt> instance that specifies the
	 *            <tt>SendStream</tt> to be closed
	 */
	void closeSendStream(SendStreamDesc sendStreamDesc) {
		...
		try {
			...
			if (sendStreams.contains(sendStreamDesc) && (sendStreamDesc.getSendStreamCount() < 1)) {
	 ...
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> which is requesting the creation
	 *            of a <tt>SendStream</tt>. Since multiple
	 *            <tt>StreamRTPManager</tt> may request the creation of a
	 *            <tt>SendStream</tt> from one and the same combination of
	 *            <tt>dataSource</tt> and <tt>streamIndex</tt>, the method may
	 *            not create a completely new <tt>SendStream</tt> but may return
	 *            a <tt>StreamRTPManager</tt>-specific view of an existing
	 *            <tt>SendStream</tt>.
	 * @param dataSource
	 *            the <tt>DataSource</tt> which provides the stream from which a
	 *            <tt>SendStream</tt> is to be created
	 * @param streamIndex
	 *            the zero-based position within the array/list of streams of
	 *            the specified <tt>dataSource</tt> of the stream from which a
	 *            <tt>SendStream</tt> is to be created
	 * @return a <tt>SendStream</tt> created from the specified
	 *         <tt>dataSource</tt> and <tt>streamIndex</tt>. The returned
	 *         <tt>SendStream</tt> implementation is a <tt>streamRTPManager</tt>
	 *         -dedicated view to an actual <tt>SendStream</tt> which may have
	 *         been created during a previous execution of the method
	 * @throws IOException
	 ...
	 * 
	 * @param streamDesc
	 *            a <tt>PushSourceStreamDesc</tt> which identifies the peer from
	 *            which an RTP or RTCP packet has been received
	 * @param buffer
	 *            the buffer which contains the bytes of the received RTP or
	 *            RTCP packet
	 * @param offset
	 *            the zero-based index in <tt>buffer</tt> at which the bytes of
	 *            the received RTP or RTCP packet begin
	 * @param length
	 *            the number of bytes in <tt>buffer</tt> beginning at
	 *            <tt>offset</tt> which represent the received RTP or RTCP
	 *            packet
	 ...
	 */
	int didRead(PushSourceStreamDesc streamDesc, byte[] buffer, int offset, int length) throws IOException {
		...
		try {
			...
			if (data) {
				...
				if (!streamRTPManagerDesc.streamRTPManager.getMediaStream().getDirection().allowsReceiving()) {
					// FIXME We are ignoring RTP packets received from peers who
					// we
					// do not want to receive from ONLY in the sense that we are
					// not
				...
				// Do the bytes in the specified buffer resemble (a header of)
				// an
				// RTP packet?
				if ((length >= RTPHeader.SIZE) && (/* v */((buffer[offset] & 0xc0) >>> 6) == RTPHeader.VERSION)) {
					int ssrc = readInt(buffer, offset + 8);

					if (!streamRTPManagerDesc.containsReceiveSSRC(ssrc)) {
						if (findStreamRTPManagerDescByReceiveSSRC(ssrc, streamRTPManagerDesc) == null) {
	 ...
	 * 
	 * @param receiveSSRC
	 *            the receive/remote SSRC to which the returned
	 *            <tt>StreamRTPManager</tt> is to be related
	 * @param exclusion
	 *            the <tt>StreamRTPManager</tt>, if any, to be excluded from the
	 *            search
	 ...
	 *            ignored
	 * @return the return value of the invocation of
	 *         <tt>RTPManager.getGlobalReceptionStats()</tt> on the
	 *         internal/underlying <tt>RTPManager</tt>
	 */
	 ...
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> to which the returned
	 *            <tt>ReceiveStream</tt>s are to be related
			...
			if (streamRTPManagerDesc != null) {
				...
				if (managerReceiveStreams != null) {
					...
					for (Object s : managerReceiveStreams) {
						...
						// FMJ stores the synchronization source (SSRC)
						// identifiers
	 ...
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> to which the returned
	 *            <tt>SendStream</tt>s are to be related
	...
	List<StreamRTPManager> getStreamRTPManagers() {
		List<StreamRTPManager> ret = new ArrayList<StreamRTPManager>(streamRTPManagers.size());
		...
		try {
			...
			if (this.connector == null) {
				...
				if (manager instanceof RTPSessionMgr) {
					((RTPSessionMgr) manager).setRTCPReportBuilder(this.delegatingRTCPReportBuilder);
				}
			...
			if ((connectorDesc == null) || (connectorDesc.connector != connector)) {
				if (connectorDesc != null)
					this.connector.removeConnector(connectorDesc);
				streamRTPManagerDesc.connectorDesc = connectorDesc = (connector == null) ? null : new RTPConnectorDesc(streamRTPManagerDesc, connector);
				if (connectorDesc != null)
	...
	private void onRTCPTerminationStrategyChanged() {
		RTCPTerminationStrategy rtcpTerminationStrategy = getRTCPTerminationStrategy();

		if (rtcpTerminationStrategy != null) {
			rtcpTerminationStrategy.setRTPTranslator(this);
			delegatingRTCPReportBuilder.setDelegate(rtcpTerminationStrategy.getRTCPReportBuilder());
		} else {
	 ...
	 * {@link #addReceiveStreamListener(StreamRTPManager, ReceiveStreamListener)}
	 * does not add equal <tt>ReceiveStreamListener</tt>s, a single removal is
	 * enough to reverse multiple additions of equal
	 * <tt>ReceiveStreamListener</tt>s.
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> which specifies the neomedia
	 *            <tt>MediaStream</tt> with which the
	 *            <tt>ReceiveStreamEvent</tt>s delivered to the specified
	 *            <tt>listener</tt> are to be related
	 * @param listener
	 *            the <tt>ReceiveStreamListener</tt> to no longer be notified
	 *            about <tt>ReceiveStreamEvent</tt>s related to the specified
	 *            <tt>streamRTPManager</tt>
	 */
	 ...
	 * <tt>RemoteEvent</tt>s received by this <tt>RTPTranslatorImpl</tt>. Though
	 * the request is being made by a specific <tt>StreamRTPManager</tt>, the
	 * addition of the specified <tt>listener</tt> and the deliveries of the
	 * <tt>RemoteEvent</tt>s are performed irrespective of any
	 * <tt>StreamRTPManager</tt> so the removal follows the same logic.
	 * 
	 * @param streamRTPManager
	 *            the <tt>StreamRTPManager</tt> which is requesting the removal
	 *            of the specified <tt>RemoteListener</tt>
	 * @param listener
	 *            the <tt>RemoteListener</tt> to no longer be notified about
	 *            <tt>RemoteEvent</tt>s received by this
	 *            <tt>RTPTranslatorImpl</tt>
	 */
	 ...
	 * inspect and modify RTCP traffic between multiple <tt>MediaStream</tt>s.
	 * 
	 * @param rtcpTerminationStrategy
	 *            the <tt>RTCPTerminationStrategy</tt> which is to inspect and
	 *            modify RTCP traffic between multiple <tt>MediaStream</tt>s.
	 */
	 ...
	 * 
	 * @param ssrcFactory
	 *            the <tt>SSRCFactory</tt> which is to generate new
	 *            synchronization source (SSRC) identifiers or <tt>null</tt> if
	 *            this <tt>MediaStream</tt> is to employ internal logic to
	 ...
	 * 
	 * @param event
	 *            a <tt>ReceiveStreamEvent</tt> which contains the specifics of
	 *            the event this <tt>ReceiveStreamListener</tt> is being
	 *            notified about
	...
	public void update(ReceiveStreamEvent event) {
		...
		if (event != null) {
			...
			if (receiveStream != null) {
				 ...
				 */
				int receiveSSRC = (int) receiveStream.getSSRC();
				StreamRTPManagerDesc streamRTPManagerDesc = findStreamRTPManagerDescByReceiveSSRC(receiveSSRC, null);
	 ...
	 *            <tt>destination</tt>
	 * @param offset
	 *            the offset in <tt>buffer</tt> at which the bytes to be written
	 *            begin
	 * @param length
	 *            the number of bytes in <tt>buffer</tt> beginning at
	...
	/**
	 * Writes an <tt>RTCPFeedbackMessage</tt> into a destination identified by a
	 * specific <tt>MediaStream</tt>.
	 * 
