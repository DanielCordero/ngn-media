 ...
 */
public class RemoteBitrateEstimatorSingleStream implements CallStatsObserver, RecurringProcessible, RemoteBitrateEstimator {
	...
	private final RateStatistics incomingBitrate = new RateStatistics(500, 8000F);
	...
	public Collection<Integer> getSsrcs() {
		synchronized (critSect) {
			if (ssrcs == null) {
				ssrcs = Collections.unmodifiableCollection(new ArrayList<Integer>(overuseDetectors.keySet()));
			}
			return ssrcs;
		}
	}
	...
	public long getTimeUntilNextProcess() {
		return (lastProcessTime < 0L) ? 0L : lastProcessTime + kProcessIntervalMs - System.currentTimeMillis();
	}
	...
	public void incomingPacket(long arrivalTimeMs, int payloadSize, int ssrc, long rtpTimestamp) {
		...
		synchronized (critSect) {
			...
			if (overuseDetector == null) {
				...
				// TODO(holmer): If the channel changes SSRC the old SSRC will
				// still
				// be around in this map until the channel is deleted. This is
				// OK
			...
			if (overuseDetector.getState() == BandwidthUsage.kBwOverusing) {
				...
				if (priorState != BandwidthUsage.kBwOverusing || remoteRate.isTimeToReduceFurther(nowMs, incomingBitrate)) {
					// The first overuse should immediately trigger a new
					// estimate.
	...
	public void incomingPacket(long arrivalTimeMs, int payloadSize, RTPPacket header) {
		...
		long rtpTimestamp = header.timestamp + getExtensionTransmissionTimeOffset(header);
	...
	private void updateEstimate(long nowMs) {
		synchronized (critSect) {
			...
			for (Iterator<OveruseDetector> it = overuseDetectors.values().iterator(); it.hasNext();) {
				...
				if (packetTimeMs >= 0L && nowMs - packetTimeMs > kStreamTimeOutMs) {
					...
					// Make sure that we trigger an over-use if any of the
					// over-use
