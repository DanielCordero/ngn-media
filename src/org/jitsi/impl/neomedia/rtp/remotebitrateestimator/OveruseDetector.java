...
public class OveruseDetector {
	private static class FrameSample {
		 ...
		 * 
		 * @param source
		 *            the <tt>FrameSample</tt> the values of the fields of which
	 ...
	 * 
	 * @param matrix
	 *            the <tt>double</tt> two-dimensional matrix to create and
	...
	public void update(int packetSize, long timestampMs, long rtpTimestamp, long arrivalTimeMs) {
		...
		if (currentFrame.timestamp == -1L) {
			...
			if (prevFrame.completeTimeMs >= 0L) // This is our second frame.
			{
				...
				/* tsDelta */timeDeltas[1], currentFrame.size, prevFrame.size);
			}
			prevFrame.copy(currentFrame);
	...
	private void updateKalman(long tDelta, double tsDelta, long frameSize, long prevFrameSize) {
		...
		if ((hypothesis == BandwidthUsage.kBwOverusing && offset < prevOffset) || (hypothesis == BandwidthUsage.kBwUnderusing && offset > prevOffset)) {
		...
		boolean stableState = (Math.min(numOfDeltas, 60) * Math.abs(offset) < threshold);
		...
		updateNoiseEstimate(residualForUpdateNoiseEstimate, minFramePeriod, stableState);
