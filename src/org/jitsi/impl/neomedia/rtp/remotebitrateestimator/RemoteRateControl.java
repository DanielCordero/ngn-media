...
public class RemoteRateControl {
	...
	private final RateControlInput currentInput = new RateControlInput(BandwidthUsage.kBwNormal, 0L, 0D);
	...
	private long changeBitRate(long currentBitRate, long incomingBitRate, double delayFactor, long nowMs) {
		...
		case kRcIncrease: {
			if (avgMaxBitRate >= 0F) {
				if (incomingBitRateKbps > avgMaxBitRate + 3F * stdMaxBitRate) {
					...
					avgMaxBitRate = -1F;
				} else if (incomingBitRateKbps > avgMaxBitRate + 2.5F * stdMaxBitRate) {
					changeRegion(RateControlRegion.kRcAboveMax);
				}
			}

			long responseTime = ((long) (avgChangePeriod + 0.5F)) + rtt + 300L;
			double alpha = getRateIncreaseFactor(nowMs, lastBitRateChange, responseTime, delayFactor);
			...
			if (incomingBitRate < minConfiguredBitRate) {
				...
				if (currentBitRate > this.currentBitRate) {
					...
					if (rateControlRegion != RateControlRegion.kRcMaxUnknown) {
						currentBitRate = (long) (beta * avgMaxBitRate * 1000F + 0.5F);
					}
		...
		if (!recovery && (incomingBitRate > 100000L || currentBitRate > 150000L) && currentBitRate > 1.5 * incomingBitRate) {
	...
	private double getRateIncreaseFactor(long nowMs, long lastMs, long reactionTimeMs, double noiseVar) {
		...
		double alpha = 1.005D + B / (1D + Math.exp(b * (d * reactionTimeMs - (c1 * noiseVar + c2))));
	...
	public RateControlRegion update(RateControlInput input, long nowMs) {
		...
		if (!initializedBitRate) {
			...
			} else if (nowMs - timeFirstIncomingEstimate > 500L && input.incomingBitRate > 0L) {
	...
	public long updateBandwidthEstimate(long nowMs) {
		currentBitRate = changeBitRate(currentBitRate, currentInput.incomingBitRate, currentInput.noiseVar, nowMs);
		return currentBitRate;
	}
	...
	private void updateMaxBitRateEstimate(float incomingBitRateKbps) {
		...
		if (avgMaxBitRate == -1F) {
			...
			avgMaxBitRate = (1 - alpha) * avgMaxBitRate + alpha * incomingBitRateKbps;
		}
		...
		varMaxBitRate = (1 - alpha) * varMaxBitRate + alpha * (avgMaxBitRate - incomingBitRateKbps) * (avgMaxBitRate - incomingBitRateKbps) / norm;
