 ...
 * to: 1. Have a score at least {@link #ACTIVE_COEF} times as much as the score
 * of the currently active stream. 2. Have score at least
 * {@link #MIN_NEW_ACTIVE_SCORE}. 3. Have had its audio level updated in the
 * last {@link #MAX_NEW_ACTIVE_SILENT_INTERVAL} milliseconds. 4. Have updated
 * its audio level at least {@link #MIN_NEW_ACTIVE_SIZE} times.
 * </p>
 * <p>
 ...
 * thorough tests in a conference. Some justification for the rules: 1. Helps to
 * avoid often changing the active when there are two streams with similar
 * levels. 2. This is to prevent switching the active stream away during times
 * of &quot;silence&quot;. Without this threshold we observed the following:
 ...
 * higher noise becomes active. 3. This is for the case when someone quits the
 * conference shouting ;) 4. This is because of the way we compute scores.
 * Just-added streams might have an uncharacteristically high score.
 * </p>
 * 
 * @author Boris Grozev
 */
public class BasicActiveSpeakerDetector extends AbstractActiveSpeakerDetector {
	...
	private void updateActive() {
		...
		for (History history : histories) {
			if (history.lastUpdate != -1 && history.lastUpdate + MAX_NEW_ACTIVE_SILENT_INTERVAL < System.currentTimeMillis()) // rule
																																// 4
																																// in
																																// class
																																// javadoc
			...
			if (history.score > active.score * ACTIVE_COEF // rule 1
					&& history.score > newActive.score // highest score among
														// eligible
