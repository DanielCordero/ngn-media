...
import net.sf.fmj.media.rtp.rtpmediaformat.RTPMediaFormatFactory;
...
public class MediaServiceImpl extends PropertyChangeNotifier implements MediaService {
	 ...
	 */
	private RTPMediaFormatFactory formatFactory;
	 ...
	 */
	private static Map<RTPMediaFormat, Byte> dynamicPayloadTypePreferences;
	 ...
	 */
	public MediaStream createMediaStream(RTPMediaType mediaType) {
	 ...
	 * {@inheritDoc}
	 */
	public MediaStream createMediaStream(StreamConnector connector, RTPMediaType mediaType) {
	 ...
	 * {@inheritDocs}
	 */
	public MediaStream createMediaStream(StreamConnector connector, RTPMediaType mediaType, SrtpControl srtpControl) {
	 ...
	 * @return a new <tt>MediaStream</tt> instance
	 */
	private MediaStream createMediaStream(RTPMediaType mediaType, StreamConnector connector, MediaDevice device, SrtpControl srtpControl) {
	 ...
	 */
	public MediaDevice getDefaultDevice(RTPMediaType mediaType, MediaUseCase useCase) {
	 ...
	 */
	public RTPMediaFormatFactory getFormatFactory() {
		if (formatFactory == null)
			formatFactory = new RTPMediaFormatFactory();
	...
	private MediaDevice getNonSendVideoDevice() {
		if (nonSendVideoDevice == null)
			nonSendVideoDevice = new MediaDeviceImpl(RTPMediaType.VIDEO);
	...
	public MediaDevice getMediaDeviceForPartialDesktopStreaming(int width, int height, int x, int y) {
		...
		Format formats[] = new Format[] { new AVFrameFormat(size, Format.NOT_SPECIFIED, FFmpeg.PIX_FMT_ARGB, Format.NOT_SPECIFIED), new RGBFormat(size, // size
		...
		CaptureDeviceInfo devInfo = new CaptureDeviceInfo(name + " " + display, new MediaLocator(DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING + ":" + display + "," + x + "," + y), formats);

		device = new MediaDeviceImpl(devInfo, RTPMediaType.VIDEO);
		return device;
	}
	...
	private void deviceConfigurationPropertyChange(PropertyChangeEvent event) {
		 ...
		 */
		if (DeviceConfiguration.AUDIO_CAPTURE_DEVICE.equals(propertyName) || DeviceConfiguration.AUDIO_NOTIFY_DEVICE.equals(propertyName) || DeviceConfiguration.AUDIO_PLAYBACK_DEVICE.equals(propertyName) || DeviceConfiguration.VIDEO_CAPTURE_DEVICE.equals(propertyName)) {
			/*
	 ...
	 * 
	 * @param mediaType
	 *            the <tt>MediaType</tt> to be determined whether the support
	 *            for it is enabled
	 * @return <tt>true</tt> if the support for the specified <tt>mediaType</tt>
	 *         is enabled; otherwise, <tt>false</tt>
	 */
	public static boolean isMediaTypeSupportEnabled(RTPMediaType mediaType) {
		String propertyName;
