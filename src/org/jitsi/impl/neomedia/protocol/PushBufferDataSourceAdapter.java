...
public class PushBufferDataSourceAdapter extends PushBufferDataSourceDelegate<PullBufferDataSource> {
	...
	private static class PushBufferStreamAdapter implements PushBufferStream {
		 ...
		 * 
		 * @param stream
		 *            the <tt>PullBufferStream</tt> the new instance is to
		 ...
		 *         <tt>PullBufferStream</tt> which describes the type of the
		 *         media data it gives access to
		 */
		@Override
		 ...
		 *            control of the wrapped <tt>PullBufferStream</tt> to be
		 *            retrieved
		 * @return an <tt>Object</tt> which represents the control of the
		 *         wrapped <tt>PushBufferStream</tt> of the requested type if
		 *         the wrapped <tt>PushBufferStream</tt> has such a control;
		 *         <tt>null</tt> if the wrapped <tt>PushBufferStream</tt> does
		 ...
		 * 
		 * @param buffer
		 *            a <tt>Buffer</tt> in which media data is to be written by
		 *            this <tt>PushBufferDataSource</tt>
		 * @throws IOException
		 *             if anything wrong happens while reading media data from
		 *             this <tt>PushBufferDataSource</tt> into the specified
		 *             <tt>buffer</tt>
		 */
		@Override
		 ...
		 *            the <tt>BufferTransferHandler</tt> through which
		 *            <tt>PushBufferStream</tt> is to notify its user that media
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>PullBufferDataSource</tt> the new instance is to
	 *            implement <tt>PushBufferDataSource</tt> capabilities for
	 */
	 ...
	 * 
	 * @param stream
	 *            the <tt>PullBufferStream</tt> adapted by a
	 *            <tt>PushBufferStreamAdapter</tt> that is to have the priority
	 *            of its <tt>streamReadThread</tt> set
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if anything wrong happens while starting the wrapped
	 *             <tt>PullBufferDataSource</tt> or the pushing from the
	 *             <tt>PushBufferStreamAdapter</tt>s which wrap the
	 *             <tt>PullBufferStream</tt>s of the
	 ...
	 * 
	 * @throws IOException
	 *             if anything wrong happens while stopping the wrapped
	 *             <tt>PullBufferDataSource</tt> or the pushing from the
	 *             <tt>PushBufferStreamAdapter</tt>s which wrap the
	 *             <tt>PullBufferStream</tt>s of the
	 *             <tt>PullBufferDataSource</tt> wrapped by this instance
