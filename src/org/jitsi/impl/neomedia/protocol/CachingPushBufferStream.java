...
/**
 * Enables reading from a <tt>PushBufferStream</tt> a certain maximum number of
 * data units (e.g. bytes, shorts, ints) even if the <tt>PushBufferStream</tt>
 * itself pushes a larger number of data units.
 * <p>
 * An example use of this functionality is pacing a <tt>PushBufferStream</tt>
 * which pushes more data units in a single step than a <tt>CaptureDevice</tt>.
 * When these two undergo audio mixing, the different numbers of per-push data
 * units will cause the <tt>PushBufferStream</tt> "play" itself faster than the
 * <tt>CaptureDevice</tt>.
 * </p>
 * 
...
public class CachingPushBufferStream implements PushBufferStream {
	 ...
	 * <tt>#read(Buffer)</tt> method of the wrapped stream and to be thrown by
	 * this stream on the earliest call of its <tt>#read(Buffer)</tt> method.
	 */
	private IOException readException;

	/**
	 * The <tt>PushBufferStream</tt> being paced by this instance with respect
	 * to the maximum number of data units it provides in a single push.
	 */
	...
	/**
	 * Initializes a new <tt>CachingPushBufferStream</tt> instance which is to
	 * pace the number of per-push data units a specific
	 * <tt>PushBufferStream</tt> provides.
	 * 
	 * @param stream
	 *            the <tt>PushBufferStream</tt> to be paced with respect to the
	 *            number of per-push data units it provides
	 */
	 ...
	 *         is acceptable; otherwise, <tt>false</tt> which means that the
	 *         reading from the wrapped <tt>PushBufferStream</tt> should be
	 *         blocked until <tt>true</tt> is returned
	 */
	 ...
	 *         which allows the adjustment of the size of the buffering it
	 *         performs
	 */
	 ...
	 *         <tt>PushBufferStream</tt> if such a value has been set;
	 *         otherwise, {@link BufferControl#DEFAULT_VALUE}
	 */
	private long getBufferLength() {
		synchronized (bufferControlSyncRoot) {
			return (bufferControl == null) ? BufferControl.DEFAULT_VALUE : bufferControl.getBufferLength();
		}
	}
	 ...
	 * 
	 * @param controlType
	 *            a <tt>String</tt> value which names the type of the control of
	 *            the wrapped <tt>PushBufferStream</tt> to be retrieved
	 * @return an <tt>Object</tt> which represents the control of the wrapped
	 *         <tt>PushBufferStream</tt> with the specified type if such a
	 *         control is available; otherwise, <tt>null</tt>
	 */
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> which contains media the length in
	 *            milliseconds of which is to be calculated
	 * @return the length in milliseconds of the media in <tt>buffer</tt> if
	 *         there actually is media in <tt>buffer</tt> and its length in
	 *         milliseconds can be calculated; otherwise, <tt>0</tt>
	 */
	 ...
	 * @throws IOException
	 *             if the wrapped stream has thrown such an exception when data
	 *             was last read from it
	 */
	 ...
	 * @param out
	 *            the <tt>Buffer</tt> into which to write the data read from the
	 *            specified <tt>in</tt>
	 * @param outOffset
	 *            the offset in <tt>out</tt> at which the data read from
	 *            <tt>in</tt> is to be written
	 * @return the offset in <tt>out</tt> at which a next round of writing is to
	 *         continue; <tt>-1</tt> if no more writing in <tt>out</tt> is to be
	 *         performed and <tt>out</tt> is to be returned to the caller
	 * @throws IOException
	 *             if reading from <tt>in</tt> into <tt>out</tt> fails including
	 *             if either of the formats of <tt>in</tt> and <tt>out</tt> are
	 *             not supported
	 */
	 ...
	 *            <tt>PushBufferStream</tt> when media data is available for
	 *            reading
	 */
	...
	private static class BufferControlImpl implements BufferControl {
		 ...
		 *         owner of this instance; {@link BufferControl#DEFAULT_VALUE}
		 *         if it is up to the owner of this instance to decide the
		 *         length in milliseconds of the buffering to perform if any
		 */
		 ...
		 *         exported by the owner of this instance if such a
		 ...
		 * 
		 * @param bufferLength
		 *            the length in milliseconds of the buffering to be
		 *            performed by the owner of this instance
		 * @return the length in milliseconds of the buffering performed by the
		 *         owner of this instance that is actually in effect after the
		 *         attempt to set it to the specified <tt>bufferLength</tt>
		 */
		 ...
		 *            <tt>true</tt> if threshold calculations are to be enabled;
		 *            otherwise, <tt>false</tt>
		 */
		 ...
		 *            the minimum threshold in milliseconds for the buffering to
		 *            be performed by the owner of this instance
		 * @return the minimum threshold in milliseconds for the buffering
		 *         performed by the owner of this instance that is actually in
		 *         effect after the attempt to set it to the specified
