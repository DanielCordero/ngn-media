 ...
 * <tt>PushBufferStream</tt> but rather a specific substitute in order to give
 * full control to the {@link PushBufferStream#read(javax.media.Buffer)} method
 * of the substitute.
 * <p>
 * The purpose is achieved in <code>#transferData(PushBufferStream)</code> where
 * the method argument <code>stream</code> is ignored and the substitute is used
 * instead.
 * </p>
 * 
...
public class StreamSubstituteBufferTransferHandler implements BufferTransferHandler {
	...
	/**
	 * The wrapped <tt>BufferTransferHandler</tt> which receives the actual
	 * events from the wrapped <tt>PushBufferStream</tt>.
	 */
	 ...
	 *            the <tt>BufferTransferHandler</tt> the new instance is to
	 *            overwrite the source <tt>PushBufferStream</tt> of
	 * @param stream
	 *            the <code>PushBufferStream</code> to be overridden for the
	 *            specified <code>transferHandler</code> with the specified
	 *            <code>substitute</code>
	 * @param substitute
	 *            the <code>PushBufferStream</code> to override the specified
	 *            <code>stream</code> for the specified
