...
/**
 * A <tt>Recorder</tt> implementation which attaches to an
 * <tt>RTPTranslator</tt>.
 * 
...
public class RecorderRtpImpl implements Recorder, ReceiveStreamListener, ActiveSpeakerChangedListener, ControllerListener {
	...
	private static final Format redFormat = new VideoFormat(VideoFormat.RED);
	private static final Format ulpfecFormat = new VideoFormat(VideoFormat.ULPFEC);
	private static final Format vp8RtpFormat = new VideoFormat(VideoFormat.VP8_RTP);
	private static final Format vp8Format = new VideoFormat(VideoFormat.VP8);
	private static final Format opusFormat = new AudioFormat(AudioFormat.OPUS_RTP, 48000, Format.NOT_SPECIFIED, Format.NOT_SPECIFIED);
	 ...
	 */
	private static final ContentDescriptor AUDIO_CONTENT_DESCRIPTOR = new ContentDescriptor(FileTypeDescriptor.MPEG_AUDIO);
	 ...
	 * 
	 * @param translator
	 *            the <tt>RTPTranslator</tt> to which this instance will attach
	 *            in order to record media.
	 */
	public RecorderRtpImpl(RTPTranslator translator) {
		...
		if (cfg != null) {
			performActiveSpeakerDetection = cfg.getBoolean(PERFORM_ASD_PNAME, false);
		}
	 ...
	 * @param dirname
	 *            the path to the directory into which this <tt>Recorder</tt>
	...
	public void start(String format, String dirname) throws IOException, MediaException {
		...
		/*
		 * Note that we use only one RTPConnector for both the RTPTranslator and
		 * the RTPManager instances. The this.translator will write to its
		...
		/*
		 * Register a fake call participant. TODO: can we use a more generic
		 * MediaStream here?
		 */
		mediaStream = mediaService.createMediaStream(new MediaDeviceImpl(new CaptureDeviceInfo(), RTPMediaType.VIDEO));
		...
		// redFormat, redPayloadType);
		// ((RTPTranslatorImpl)videoRTPTranslator).addFormat(streamRTPManager,
		// ulpfecFormat, ulpfecPayloadType);
		// ((RTPTranslatorImpl)videoRTPTranslator).addFormat(streamRTPManager,
	...
	public void stop() {
		if (started) {
			...
			// remove the recorder from the translator (e.g. stop new packets
			// from
	...
	public void update(ReceiveStreamEvent event) {
		...
		if (event instanceof NewReceiveStreamEvent) {
			...
			/*
			 * XXX TODO IRBABOON This is a terrible hack which works around a
			 * failure to realize() some of the Processor-s for audio streams,
			 * when multiple streams start nearly simultaneously. The cause of
			 * the problem is currently unknown (and synchronizing all FMJ calls
			 * in RecorderRtpImpl does not help). XXX TODO NOOBABRI
			 */
			if (receiveStreamDesc.format instanceof AudioFormat) {
			...
			if (receiveStreamDesc != null) {
				...
				if (logger.isInfoEnabled()) {
					logger.info("ReceiveStream timeout for an unknown stream" + " (already removed?) " + getReceiveStreamSSRC(receiveStream));
				}
			}
	...
	private void removeReceiveStream(ReceiveStreamDesc receiveStream, boolean emptyJB) {
		...
		if (receiveStream.format instanceof VideoFormat) {
			 ...
			 * SSRC starts again later, we will obtain new mappings based on the
			 * new Sender Reports. See
		...
		if (ev instanceof ConfigureCompleteEvent) {
			...
			for (TrackControl track : processor.getTrackControls()) {
				...
				if (audio) {
					...
					if (AudioFormat.OPUS_RTP.equals(desc.format.getEncoding())) {
	 ...
	 * 
	 * @param ssrc
	 *            the SSRC for which to restart recording. RTP packet of the new
	 *            recording).
	 */
	private void resetRecording(long ssrc, long timestamp) {
		...
		if (receiveStream != null && receiveStream.format instanceof AudioFormat) {
			...
			try {
				receiveStream.dataSink = Manager.createDataSink(receiveStream.dataSource, new MediaLocator("file:" + newFilename));
			} catch (NoDataSinkException ndse) {
	...
	private void audioRecordingStarted(long ssrc, long timestamp) {
		...
		event.setMediaType(RTPMediaType.AUDIO);
		event.setSsrc(ssrc);
	 ...
	 * 
	 * @param dataSink
	 *            the <tt>DataSink</tt> which requests that a keyframe be
	 ...
	 * @param ssrc
	 *            The SSRC to match.
	 * @return the <tt>ReceiveStreamDesc</tt> with a particular SSRC, or
	 *         <tt>null</tt>.
	 */
	...
	/**
	 * Implements
	 * {@link ActiveSpeakerChangedListener#activeSpeakerChanged(long)}. Notifies
	 * this <tt>RecorderRtpImpl</tt> that the audio <tt>ReceiveStream</tt>
	 * considered active has changed, and that the new active stream has SSRC
	 * <tt>ssrc</tt>.
	 * 
	 * @param ssrc
	...
	public void activeSpeakerChanged(long ssrc) {
		if (performActiveSpeakerDetection) {
			if (eventHandler != null) {
				...
				e.setMediaType(RTPMediaType.VIDEO);
	...
	private class RTPConnectorImpl implements RTPConnector {
		...
		private RTPConnectorImpl(byte redPT, byte ulpfecPT) {
			...
			// incoming packets.
			TransformEngine transformEngine = new TransformEngineChain(new TransformEngine[] { packetBuffer, new TransformEngineImpl(), new CompoundPacketEngine(), new FECTransformEngine(ulpfecPT, (byte) -1), new REDTransformEngine(redPT, (byte) -1) });
		...
		private class OutputDataStreamImpl implements OutputDataStream {
			...
			public synchronized int write(byte[] buffer, int offset, int length, boolean transform) {
				...
				if (transform) {
					PacketTransformer packetTransformer = isControlStream ? rtcpPacketTransformer : rtpPacketTransformer;
		...
		private class PushSourceStreamImpl implements PushSourceStream {
			...
			/**
			 * Not implemented because there are currently no uses of the
		...
		private class TransformEngineImpl implements TransformEngine {
			SinglePacketTransformer rtpTransformer = new SinglePacketTransformer() {
				@Override
			...
			SinglePacketTransformer rtcpTransformer = new SinglePacketTransformer() {
				@Override
				public RawPacket transform(RawPacket pkt) {
					return pkt;
				}
