...
/**
 * The call recording implementation. Provides the capability to start and stop
 * call recording.
 * 
...
public class RecorderImpl implements Recorder {
	...
	/*
	 * Disables formats currently not working SoundFileUtils.aif,
	 * SoundFileUtils.au, SoundFileUtils.gsm,
	 */
	SoundFileUtils.wav, SoundFileUtils.mp3 };
	...
	/**
	 * The <tt>RecorderEventHandler</tt> which this <tt>Recorder</tt> should
	 * notify when events related to recording (such as start/end of a
	 * recording) occur.
	 */
	...
	/**
	 * The indicator which determines whether this <tt>Recorder</tt> is set to
	 * skip media from mic.
	 */
	private boolean mute = false;

	/**
	 * The filename we will use to record data, supplied when Recorder is
	 * started.
	 */
	 ...
	 * 
	 * @param device
	 *            device that can create a session that provides the output data
	 *            source
	 */
	 ...
	 *            listeners interested in notifications from this
	 *            <tt>Recorder</tt>
	 ...
	 * 
	 * @param format
	 *            the format that corresponding to the content descriptor
	 * @return content descriptor
	 * @throws IllegalArgumentException
	 *             if the specified <tt>format</tt> is not a supported recording
	 *             format
	 */
	 ...
	 * 
	 * @param listener
	 *            the existing <tt>Recorder.Listener</tt> to be removed from the
	 *            list of listeners interested in notifications from this
	 *            <tt>Recorder</tt>
	 ...
	 * @param filename
	 *            the name of the file into which the media associated with this
	 *            <tt>Recorder</tt> is to be recorded
	 * @throws IOException
	 *             if anything goes wrong with the input and/or output performed
	 *             by this <tt>Recorder</tt>
	 * @throws MediaException
	 *             if anything else goes wrong while starting the recording of
	 *             media performed by this <tt>Recorder</tt>
	...
	/**
	 * Put the recorder in mute state. It won't record the local input. This is
	 * used when the local call is muted and we don't won't to record the local
	 * input.
	 * 
	 * @param mute
	...
	/**
	 * Returns the filename we are last started or stopped recording to, null if
	 * not started.
	 * 
	 * @return the filename we are last started or stopped recording to, null if
	 *         not started.
	 */
