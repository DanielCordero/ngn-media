...
/**
 * An effect that would pass data to the <tt>AudioLevelEventDispatcher</tt> so
 * that it would calculate levels and dispatch changes to interested parties.
 * 
...
public class AudioLevelEffect extends ControlsAdapter implements Effect {
	...
	public AudioLevelEffect() {
		supportedAudioFormats = new Format[] { new AudioFormat(AudioFormat.LINEAR, Format.NOT_SPECIFIED, 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16, Format.NOT_SPECIFIED, Format.byteArray) };
	}
	 ...
	 * 
	 * @param listener
	 *            the <tt>SimplAudioLevelListener</tt> that we'd like to receive
	 *            level changes or <tt>null</tt> if we'd like level measurements
	 *            to stop.
	 */
	 ...
	 * 
	 * @param input
	 *            The <tt>Format</tt> of the data to be used as input to the
	 *            plug-in.
	 * @return An array that contains the supported output <tt>Formats</tt>.
	 */
	public Format[] getSupportedOutputFormats(Format input) {
		return new Format[] { new AudioFormat(AudioFormat.LINEAR, ((AudioFormat) input).getSampleRate(), 16, 1, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, 16, Format.NOT_SPECIFIED, Format.byteArray) };
	}
	 ...
	 * 
	 * @param inputBuffer
	 *            The <tt>Buffer</tt> that contains the media data to be
	 *            processed.
	 * @param outputBuffer
	 *            The <tt>Buffer</tt> in which to store the processed media
	 *            data.
