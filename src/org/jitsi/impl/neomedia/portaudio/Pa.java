...
public final class Pa {
	...
	public static enum HostApiTypeId {
		paAL(9), paALSA(8), paASIO(3), paAudioScienceHPI(14), paBeOS(10), paCoreAudio(5), paDirectSound(1), paInDevelopment(0) /*
																																 * use
																																 * while
																																 * developing
																																 * support
																																 ...
																																 * host
																																 * API
																																 */, paJACK(12), paMME(2), paOSS(7), paSoundManager(4), paWASAPI(13), paWDMKS(11);
		 ...
		 *         <tt>value</tt> or <tt>null</tt> if there is no such
		 *         representation
		 */
	 ...
	 */
	public static final long STREAM_FLAGS_PRIME_OUTPUT_BUFFERS_USING_STREAM_CALLBACK = 0x00000008;
	 ...
	 */
	public static native void AbortStream(long stream) throws PortAudioException;
	 ...
	 * @throws PortAudioException
	 */
	public static native void CloseStream(long stream) throws PortAudioException;
	 ...
	 */
	public static native double DeviceInfo_getDefaultHighInputLatency(long deviceInfo);
	 ...
	 */
	public static native double DeviceInfo_getDefaultHighOutputLatency(long deviceInfo);
	 ...
	 */
	public static native double DeviceInfo_getDefaultLowInputLatency(long deviceInfo);
	 ...
	 */
	public static native double DeviceInfo_getDefaultLowOutputLatency(long deviceInfo);
	 ...
	 */
	public static native double DeviceInfo_getDefaultSampleRate(long deviceInfo);
	 ...
	 * 
	 * @param deviceInfo
	 *            the pointer to the <tt>PaDeviceInfo</tt> to get the
	 ...
	 * 
	 * @param deviceInfo
	 *            the pointer to the <tt>PaDeviceInfo</tt> instance to get the
	 *            name of
	 ...
	 * @return The transport type identifier.
	 */
	public static native byte[] DeviceInfo_getTransportTypeBytes(long deviceInfo);
	 ...
	 *         <tt>paNoDevice</tt> if no default input device is available or an
	 ...
	 * 
	 * @param deviceID
	 *            a <tt>String</tt> identifying the PortAudio device to retrieve
	 *            the index of. It is either a <tt>deviceUID</tt> or a
	 ...
	 *         <tt>deviceID</tt> or <tt>Pa.paNoDevice</tt> if no such device
	 *         exists
	 */
	public static int getDeviceIndex(String deviceID, int minInputChannels, int minOutputChannels) {
		if (deviceID != null) {
			...
			for (int deviceIndex = 0; deviceIndex < deviceCount; ++deviceIndex) {
				...
				if (deviceID.equals(((deviceUID == null) || (deviceUID.length() == 0)) ? Pa.DeviceInfo_getName(deviceInfo) : deviceUID)) {
					/*
					 * Resolve deviceID clashes by further identifying the
					 * device through the numbers of channels that it supports
					 * for input and output.
					 */
					if ((minInputChannels > 0) && (Pa.DeviceInfo_getMaxInputChannels(deviceInfo) < minInputChannels))
						continue;
					if ((minOutputChannels > 0) && (Pa.DeviceInfo_getMaxOutputChannels(deviceInfo) < minOutputChannels))
						continue;
	 ...
	 *            the size in bits of the native <tt>PaSampleFormat</tt> to get
	 ...
	 *         <tt>paSampleFormatNotSupported</tt> if the format is not
	 *         supported.
	 */
	 ...
	 *         frames that can be read from the stream without blocking or busy
	 *         waiting or, a <tt>PaErrorCode</tt> (which are always negative) if
	 ...
	 *            pointer to the stream.
	 * @return returns a non-negative value representing the maximum number of
	 *         frames that can be written to the stream without blocking or busy
	 *         waiting or, a PaErrorCode (which are always negative) if
	 *         PortAudio is not initialized or an error is encountered.
	 */
	 ...
	 * 
	 * @param hostApiInfo
	 *            pointer to host API info structure.
	 * @return The default input device for this host API.
	 */
	public static native int HostApiInfo_getDefaultInputDevice(long hostApiInfo);
	 ...
	 */
	public static native int HostApiInfo_getDefaultOutputDevice(long hostApiInfo);
	 ...
	 * @param hostApiInfo
	 *            pointer to host API info structure.
	 * @return The well known unique identifier of this host API. Enumerator:
	 ...
	 *            A structure that describes the input parameters used to open a
	 *            stream.
	 ...
	 *         indicating why the format is not supported otherwise. The
	 *         constant paFormatIsSupported is provided to compare with the
	 *         return value for success.
	 */
	public static native boolean IsFormatSupported(long inputParameters, long outputParameters, double sampleRate);

	/**
	 * Opens a stream for either input, output or both.
	 * 
	 * @param inputParameters
	 *            the input parameters or 0 if absent.
	 * @param outputParameters
	 ...
	 *            stream
	 * @param streamFlags
	 *            Flags which modify the behavior of the streaming process.
	 * @param streamCallback
	 *            A pointer to a client supplied function that is responsible
	 *            for processing and filling input and output buffers. If
	 *            <tt>null</tt>, the stream will be opened in 'blocking
	 *            read/write' mode.
	 * @return pointer to the opened stream.
	 * @throws PortAudioException
	 */
	public static native long OpenStream(long inputParameters, long outputParameters, double sampleRate, long framesPerBuffer, long streamFlags, PortAudioStreamCallback streamCallback) throws PortAudioException;

	/**
	 * Read samples from an input stream. The function doesn't return until the
	 * entire buffer has been filled - this may involve waiting for the
	 * operating system to supply the data.
	 * 
	 * @param stream
	 *            pointer to the stream.
	 * @param buffer
	 *            a buffer of sample frames.
	 * @param frames
	 *            The number of frames to be read into buffer.
	 * @throws PortAudioException
	 */
	public static native void ReadStream(long stream, byte[] buffer, long frames) throws PortAudioException;
	 ...
	 *            enabled or disabled
	 * @param denoise
	 *            <tt>true</tt> if denoise is to be performed on the audio data
	 *            provided by <tt>stream</tt>; otherwise, <tt>false</tt>
	 */
	 ...
	 * @param echoFilterLengthInMillis
	 *            the number of milliseconds of echo to be canceled in the audio
	 *            data provided by <tt>stream</tt>
	 */
	public static native void setEchoFilterLengthInMillis(long stream, long echoFilterLengthInMillis);
	 ...
	 * @throws PortAudioException
	 */
	public static native void StartStream(long stream) throws PortAudioException;
	 ...
	 * @throws PortAudioException
	 */
	public static native void StopStream(long stream) throws PortAudioException;

	/**
	 * Free StreamParameters resources specified by a pointer to it.
	 * 
	 * @param streamParameters
	 *            the pointer to the <tt>PaStreamParameters</tt> to free
	 */
	 ...
	 *            the suggested latency in milliseconds: LATENCY_UNSPECIFIED -
	 *            use default(default high input/output latency) LATENCY_HIGH -
	 *            use default high input/output latency LATENCY_LOW - use
	 *            default low input/output latency ... - any other value in
	 *            milliseconds (e.g. 0.1 is acceptable)
	 * @return pointer to the params used for Pa_OpenStream.
	 */
	public static native long StreamParameters_new(int deviceIndex, int channelCount, long sampleFormat, double suggestedLatency);
	 ...
	 * calls to {@link #WriteStream(long, byte[], long)} with one and the same
	 * buffer because the JNI access to the bytes of the buffer which is likely
	 * to copy the whole buffer is only performed once.
	 * </p>
	 * 
	 * @param stream
	 *            the pointer to the PortAudio stream to write the samples to
	 * @param buffer
	 *            the buffer containing the samples to be written
	 * @param offset
	 *            the byte offset in <tt>buffer</tt> at which the samples to be
	 *            written start
	 * @param frames
	 *            the number of frames from <tt>buffer</tt> starting at
	 *            <tt>offset</tt> are to be written with a single write
	 * @param numberOfWrites
	 *            the number of writes each writing <tt>frames</tt> number of
	 *            frames to be performed
	 * @throws PortAudioException
	 *             if anything goes wrong while writing
	 */
	public static native void WriteStream(long stream, byte[] buffer, int offset, long frames, int numberOfWrites) throws PortAudioException;
	 ...
	 *            A buffer of sample frames.
	 * @param frames
	 *            The number of frames to be written from buffer.
	 * @throws PortAudioException
	 */
