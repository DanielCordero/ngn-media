...
public class AudioNotifierServiceImpl implements AudioNotifierService, PropertyChangeListener {
		...
		synchronized (audiosSyncRoot) {
			...
			if (audio != null) {
				...
				audio = new SCAudioClip() {
					 ...
					 * 
					 * @param loopCondition
					 *            the <tt>Callable&lt;Boolean&gt;</tt> which
					 *            represents the <tt>loopCondition</tt> to be
					 *            evaluated
					 * @return {@link Boolean#FALSE} if <tt>loopCondition</tt>
					 *         is <tt>null</tt>; otherwise, the value returned
					 *         by invoking {@link Callable#call()} on the
					 *         specified <tt>loopCondition</tt>
					 * @throws Exception
					 *             if the specified <tt>loopCondition</tt>
					 *             throws an <tt>Exception</tt>
					 */
					private Boolean evaluateLoopCondition(Callable<Boolean> loopCondition) throws Exception {
						/*
						 * SCAudioClip.play(int,Callable<Boolean>) is documented
						 * to play the SCAudioClip once only if the
						 * loopCondition is null. The same will be accomplished
						 * by returning Boolean.FALSE.
						 */
					 ...
					 * {@inheritDoc}
					 * 
					 * Returns the wrapped <tt>SCAudioClip</tt> into the cache
					 * from it has earlier been retrieved in order to allow its
					 * reuse.
					 */
					@Override
					...
					public void play() {
						 ...
						 * null. We have to take care that this instance does
						 * not get garbage collected until the finalAudio
						 * finishes playing so we will delegate to this
						 * instance's implementation of
						 * SCAudioClip.play(int,Callable<Boolean>) instead of to
						 * the finalAudio's.
						 */
						play(-1, null);
					}
					...
					public void play(int loopInterval, final Callable<Boolean> finalLoopCondition) {
						/*
						 * We have to make sure that this instance does not get
						 * garbage collected before the finalAudio finishes
						 * playing. The argument loopCondition of the method
						 * SCAudioClip.play(int,Callable<Boolean>) will live/be
						 * referenced during that time so we will use it to hold
						 * on to this instance.
						 */
	 ...
	 * 
	 * @param ev
	 *            the event that notify device has changed.
	 */
	public void propertyChange(PropertyChangeEvent ev) {
		String propertyName = ev.getPropertyName();

		if (DeviceConfiguration.AUDIO_NOTIFY_DEVICE.equals(propertyName) || DeviceConfiguration.AUDIO_PLAYBACK_DEVICE.equals(propertyName)) {
			synchronized (audiosSyncRoot) {
				/*
	 ...
	 * 
	 * @param mute
	 *            when <tt>true</tt> disables the sound; otherwise, enables the
	 *            sound.
	 */
	...
	private static class AudioKey {
		...
		public boolean equals(Object o) {
			...
			return (playback == that.playback) && ((uri == null) ? (that.uri == null) : uri.equals(that.uri));
		}
