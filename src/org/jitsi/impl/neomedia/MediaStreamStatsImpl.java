...
public class MediaStreamStatsImpl implements MediaStreamStats {
	 ...
	 * computed.
	 * 
	 * @param nbStepSinceLastUpdate
	 *            The number of step which has not been computed since last
	 *            update. In our case the number of packets received since the
	 *            last computation.
	 * @param lastValue
	 *            The value computed during the last update.
	 * @param newValue
	 *            The value newly computed.
	 * 
	 ...
	 * 
	 * @param nbLostAndRecv
	 *            The number of lost and received packets.
	 * @param nbLost
	 ...
	 * @param intervalMs
	 *            The number of milliseconds during which <tt>nbBytes</tt> bytes
	...
	private final RTCPReportListener rtcpReportListener = new RTCPReportAdapter() {
		...
		public void rtcpReportReceived(RTCPReport report) {
			...
			if (!feedbackReports.isEmpty()) {
				updateNewReceivedFeedback((RTCPFeedback) feedbackReports.get(0));
			}
		}
	 ...
	 * subsection "delay since last SR (DLSR): 32 bits"). -1 if the RTT has not
	 * been computed yet. Otherwise the RTT in ms.
	 */
	 ...
	 * 
	 * @param mediaStreamImpl
	 *            The MediaStreamImpl used to compute the stats.
	 */
	...
	/**
	 * Computes the RTT with the data (LSR and DLSR) contained in the last RTCP
	 * Sender Report (RTCP feedback). This RTT computation is based on RFC3550,
	 * section 6.4.1, subsection "delay since last SR (DLSR): 32 bits".
	 * 
	 * @param feedback
	 *            The last RTCP feedback received by the MediaStream.
	 * 
	...
	private long computeRTTInMs(RTCPFeedback feedback) {
		...
		if ((rtt >= 65536) && logger.isInfoEnabled()) {
			logger.info("Stream: " + mediaStreamImpl.getName() + ", RTT computation may be wrong (" + rtt + ">= 65536 milliseconds): now " + now + ", lsr " + lsr + ", dlsr " + dlsr);
		}
	...
	/**
	 * Returns the download video size if this stream downloads a video, or null
	 * if not.
	 * 
	...
	public String getEncoding() {
		RTPMediaFormat format = mediaStreamImpl.getFormat();
	 ...
	 *         <tt>DataSource</tt>s of all the <tt>ReceiveStream</tt>s. The set
	...
	/**
	 * Returns the delay in milliseconds introduced by the jitter buffer. Since
	 * there might be multiple <tt>ReceiveStreams</tt>, returns the biggest
	 * delay found in any of them.
	 * 
	 ...
	 *            The stream direction (DOWNLOAD or UPLOAD) of the stream from
	 *            which this function retrieve the jitter.
	 * 
	 * @return the last jitter average computed (in ms).
	 */
	private double getJitterMs(StreamDirection streamDirection) {
		...
		if (format == null) {
			RTPMediaType mediaType = mediaStreamImpl.getMediaType();

			clockRate = RTPMediaType.VIDEO.equals(mediaType) ? 90000 : -1;
		} else
	 ...
	 * @param streamDirection
	 *            The stream direction (DOWNLOAD or UPLOAD) of the stream from
	 *            which this function retrieve the number of sent/received
	 *            bytes.
	 * 
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ
	 * packet queue since the beginning of the session because it was full. It's
	 * the sum over all <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 * 
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ
	 * packet queue since the beginning of the session because they were late.
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ
	 * packet queue since the beginning of the session during resets. It's the
	 * sum over all <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 * 
	...
	/**
	 * Returns the number of Protocol Data Units (PDU) discarded by the FMJ
	 * packet queue since the beginning of the session due to shrinking. It's
	 * the sum over all <tt>ReceiveStream</tt>s of the <tt>MediaStream</tt>
	 * 
	...
	public long getNbSentBytes() {
		...
		try {
			...
			// We should not enter here because we are not creating output
			// stream
		}
	 ...
	 * 
	 * @param rttMs
	 *            the value to set on <tt>MediaStreamStatsImpl.rttMs</tt>
	 */
		...
		if (this.rttMs != rttMs) {
			...
			if (rttMs >= 0) {
				...
				if (mediaStream instanceof VideoMediaStream) {
					RemoteBitrateEstimator remoteBitrateEstimator = ((VideoMediaStream) mediaStream).getRemoteBitrateEstimator();

					if (remoteBitrateEstimator instanceof CallStatsObserver) {
						((CallStatsObserver) remoteBitrateEstimator).onRttUpdate(rttMs);
					}
				}
			}
		}
	 ...
	 * 
	 * @param feedback
	 *            The last RTCP feedback sent by the MediaStream.
	 * @param streamDirection
	 *            The stream direction (DOWNLOAD or UPLOAD) of the stream from
	 *            which this function retrieve the jitter.
	 */
	 ...
	 * 
	 * @param newNbDiscarded
	 *            The last update of the number of lost.
	 * @param nbSteps
	 *            The number of elapsed steps since the last number of loss
	 *            update.
	 */
	...
	/**
	 * Updates the <tt>nbFec</tt> field with the sum of FEC-decoded packets over
	 * the different <tt>ReceiveStream</tt>s
	 */
	 ...
	 * @param streamDirection
	 *            The stream direction (DOWNLOAD or UPLOAD) of the stream from
	 *            which this function updates the stats.
	 * @param newNbLost
	 *            The last update of the number of lost.
	 * @param nbSteps
	 *            The number of elapsed steps since the last number of loss
	...
	private void updateNbLoss(StreamDirection streamDirection, long newNbLost, long nbSteps) {
		...
		percentLoss[streamDirectionIndex] = MediaStreamStatsImpl.computeEWMA(nbSteps, percentLoss[streamDirectionIndex], newPercentLoss);
		// Saves the last update number download lost value.
	...
	private void updateStreamDirectionStats(StreamDirection streamDirection, long currentTimeMs) {
		...
		double newRateKiloBitPerSec = computeRateKiloBitPerSec(newNbByte - nbByte[streamDirectionIndex], currentTimeMs - updateTimeMs);
		rateKiloBitPerSec[streamDirectionIndex] = computeEWMA(nbSteps, rateKiloBitPerSec[streamDirectionIndex], newRateKiloBitPerSec);
