...
public class PortAudioRenderer extends AbstractAudioRenderer<PortAudioSystem> {
	...
	/**
	 * The constant which represents an empty array with <tt>Format</tt> element
	 ...
	 * input.
	 */
	private static final double[] SUPPORTED_INPUT_SAMPLE_RATES = new double[] { 8000, 11025, 16000, 22050, 32000, 44100, 48000 };
	...
	static {
		...
		for (int i = 0; i < count; i++) {
			SUPPORTED_INPUT_FORMATS[i] = new AudioFormat(AudioFormat.LINEAR, SUPPORTED_INPUT_SAMPLE_RATES[i], 16 /* sampleSizeInBits */, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
		}
	}

	/**
	 * The audio samples left unwritten by a previous call to
	 * {@link #process(Buffer)}. As {@link #bytesPerBuffer} number of bytes are
	 * always written, the number of the unwritten audio samples is always less
	 * than that.
	 */
	...
	private final UpdateAvailableDeviceListListener paUpdateAvailableDeviceListListener = new UpdateAvailableDeviceListListener() {
		...
		public void didUpdateAvailableDeviceList() throws Exception {
			synchronized (PortAudioRenderer.this) {
				...
				/*
				 * PortAudioRenderer's field flags represents its open and
				 * started state from the public point of view. We will
				 * automatically open and start this Renderer i.e. we will be
				 * modifying the state from the private point of view only and,
				 * consequently, we have to make sure that we will not modify it
		...
		public void willUpdateAvailableDeviceList() throws Exception {
			synchronized (PortAudioRenderer.this) {
				 ...
				 * PortAudioRenderer's field flags represents its open and
				 * started state from the public point of view. We will
				 * automatically close this Renderer i.e. we will be modifying
				 * the state from the private point of view only and,
				 * consequently, we have to make sure that we will not modify it
				 * from the public point of view.
				 */
				byte flags = PortAudioRenderer.this.flags;
	...
	public PortAudioRenderer(boolean enableVolumeControl) {
		super(AudioSystem.LOCATOR_PROTOCOL_PORTAUDIO, enableVolumeControl ? AudioSystem.DataFlow.PLAYBACK : AudioSystem.DataFlow.NOTIFY);
	...
	public Format[] getSupportedInputFormats() {
		if (supportedInputFormats == null) {
			...
			if ((locator == null) || ((deviceID = DataSource.getDeviceID(locator)) == null) || (deviceID.length() == 0) || ((deviceIndex = Pa.getDeviceIndex(deviceID,
			/* minInputChannels */0,
			/* minOutputChannels */1)) == Pa.paNoDevice) || ((deviceInfo = Pa.GetDeviceInfo(deviceIndex)) == 0)) {
			...
			if (outputParameters != 0) {
				try {
					if (Pa.IsFormatSupported(0, outputParameters, sampleRate)) {
						supportedInputFormats.add(new AudioFormat(audioFormat.getEncoding(), sampleRate, sampleSizeInBits, channels, audioFormat.getEndian(), audioFormat.getSigned(), Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, audioFormat.getDataType()));
					}
				} finally {
					Pa.StreamParameters_free(outputParameters);
				}
	 ...
	 *             if the PortAudio device or output stream cannot be created or
	 *             opened
	 */
	@Override
	...
	private void doOpen() throws ResourceUnavailableException {
		if (stream == 0) {
			...
			framesPerBuffer = (int) ((sampleRate * Pa.DEFAULT_MILLIS_PER_BUFFER) / (channels * 1000));

			try {
				outputParameters = Pa.StreamParameters_new(deviceIndex, channels, sampleFormat, Pa.getSuggestedLatency());

				stream = Pa.OpenStream(0 /* inputParameters */, outputParameters, sampleRate, framesPerBuffer, Pa.STREAM_FLAGS_CLIP_OFF | Pa.STREAM_FLAGS_DITHER_OFF, null /* streamCallback */);
			} catch (PortAudioException paex) {
	 ...
	 * 
	 * @param ev
	 *            a <tt>PropertyChangeEvent</tt> which specifies details about
	 *            the change such as the name of the property and its old and
	 *            new values
	 */
	@Override
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> which contains the audio data to be
	 *            rendered
	 ...
	 * 
	 * @param locator
	 *            a <tt>MediaLocator</tt> which specifies the device index of
	 *            the PortAudio device to be used by this instance for rendering
	 */
	@Override
	 ...
	 *            <tt>true</tt> if <tt>Pa_WriteStream</tt> is malfunctioning;
