...
public class JAWTRenderer extends AbstractRenderer<VideoFormat> implements VideoRenderer {
	 ...
	 *            <tt>JAWTRenderer</tt> as returned by {@link #open(Component)}
	 *            which is to be closed
	 * @param component
	 *            the AWT <tt>Component</tt> into which the
	 *            <tt>JAWTRenderer</tt> and its native counterpart are drawing.
	 *            The platform-specific info of <tt>component</tt> is not
	 ...
	 * 
	 * @param component
	 *            the AWT <tt>Component</tt> into which a <tt>JAWTRenderer</tt>
	 *            and the native counterpart to be opened are to draw. The
	 *            platform-specific info of <tt>component</tt> is not guaranteed
	 *            to be valid.
	 ...
	 *             if there is a problem during opening
	 */
	 ...
	 *            <tt>component</tt> is guaranteed to be valid only during the
	 *            execution of <tt>paint</tt>.
	 * @param g
	 *            the <tt>Graphics</tt> context into which the drawing is to be
	 *            performed
	 * @param zOrder
	 * @return <tt>true</tt> if the native counterpart of a
	 *         <tt>JAWTRenderer</tt> wants to continue receiving the
	 *         <tt>paint</tt> calls on the AWT <tt>Component</tt>; otherwise,
	 *         false. For example, after the native counterpart has been able to
	 *         acquire the native handle of the AWT <tt>Component</tt>, it may
	 *         be able to determine when the native handle needs painting
	 *         without waiting for AWT to call <tt>paint</tt> on the
	 *         <tt>Component</tt>. In such a scenario, the native counterpart
	 ...
	 * @param handle
	 *            the handle to the native counterpart of a
	 *            <tt>JAWTRenderer</tt> to process the specified data and render
	 *            it
	 * @param component
	 *            the <tt>AWT</tt> component into which the specified
	 *            <tt>JAWTRenderer</tt> and its native counterpart draw
	 * @param data
	 *            an <tt>int</tt> array which contains the data to be processed
	 *            and rendered
	 * @param offset
	 *            the index in <tt>data</tt> at which the data to be processed
	 *            and rendered starts
	 * @param length
	 *            the number of elements in <tt>data</tt> starting at
	 *            <tt>offset</tt> which represent the data to be processed and
	 *            rendered
	 * @param width
	 *            the width of the video frame in <tt>data</tt>
	 * @param height
	 *            the height of the video frame in <tt>data</tt>
	 * @return <tt>true</tt> if data has been successfully processed
	 */
	static native boolean process(long handle, Component component, int[] data, int offset, int length, int width, int height);
	 ...
	 *         the video is rendered; <tt>null</tt> if the entire component is
	 *         used
	 */
	...
	public synchronized Component getComponent() {
		if (component == null) {
			 ...
			 */
			component.setSize(DEFAULT_COMPONENT_HEIGHT_OR_WIDTH, DEFAULT_COMPONENT_HEIGHT_OR_WIDTH);
			/*
	 ...
	 * 
	 * @param buffer
	 *            a <tt>Buffer</tt> containing the data to be processed and
	 *            rendered
	 * @return <tt>BUFFER_PROCESSED_OK</tt> if the processing is successful;
	 *         otherwise, the other possible return codes defined in the
	 *         <tt>PlugIn</tt> interface
	 */
	 ...
	 * 
	 * @param bounds
	 *            the region in the component of this <tt>VideoRenderer</tt>
	 *            where the video is to be rendered; <tt>null</tt> if the entire
	 ...
	 * 
	 * @param component
	 *            the AWT <tt>Component</tt> into which this
	 *            <tt>VideoRenderer</tt> is to draw
	 * @return <tt>true</tt> if this <tt>VideoRenderer</tt> accepted the
	 *         specified <tt>component</tt> as the AWT <tt>Component</tt> into
	 *         which it is to draw; <tt>false</tt>, otherwise
	 */
	 ...
	 * 
	 * @param format
	 *            the <tt>Format</tt> to be set as the <tt>Format</tt> of the
	 *            input to be processed by this <tt>Renderer</tt>
	 * @return the <tt>Format</tt> of the input to be processed by this
	 *         <tt>Renderer</tt> if the specified <tt>format</tt> is supported
	 *         or <tt>null</tt> if the specified <tt>format</tt> is not
	 *         supported by this <tt>Renderer</tt>. Typically, it is the
	 *         supported input <tt>Format</tt> which most closely matches the
