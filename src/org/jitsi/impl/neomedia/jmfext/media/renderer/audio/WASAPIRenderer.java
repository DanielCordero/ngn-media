...
import static org.jitsi.impl.neomedia.jmfext.media.protocol.wasapi.WASAPI.WAIT_ABANDONED;
...
public class WASAPIRenderer extends AbstractAudioRenderer<WASAPISystem> {
	 ...
	 * 
	 * @param formats
	 *            the array of <tt>AudioFormat</tt>s in which the first non-
	 *            <tt>null</tt> element is to be found
	 ...
	 * 
	 * @param inFormat
	 *            the <tt>AudioFormat</tt> in which the new instance is to input
	 *            media data
	 * @param outFormat
	 *            the <tt>AudioFormat</tt> in which the new instance is to
	 *            output media data
	 * @return a new <tt>Codec</tt> which is able to resample media data from
	 *         the specified <tt>inFormat</tt> into the specified
	 *         <tt>outFormat</tt> if such a resampler could be found,
	 *         initialized and opened; otherwise, <tt>null</tt>
	 */
	 ...
	 * 
	 * @param array
	 *            the array of <tt>byte</tt> from which the specified number of
	 *            bytes are to be popped
	 * @param arrayLength
	 *            the number of elements in <tt>array</tt> which contain valid
	 *            data
	 * @param length
	 ...
	 * the audio stream is busy during the execution of {@link #process(Buffer)}
	 * .
	 */
	 ...
	 * <tt>Renderer</tt> via {@link #process(Buffer)} into {@link #dstFormat} if
	 * necessary.
	 */
	 ...
	 * 
	 * @param dataFlow
	 *            {@link AudioSystem.DataFlow#PLAYBACK} if the new instance is
	 *            to perform playback or {@link AudioSystem.DataFlow#NOTIFY} if
	 *            the new instance is to sound a notification
	 */
	 ...
	 * 
	 * @param playback
	 *            <tt>true</tt> if the new instance is to perform playback or
	 *            <tt>false</tt> if the new instance is to sound a notification
	 */
	 ...
	 *         <tt>inputFormat</tt> with which an attempt is to be made to
	 *         initialize a new <tt>IAudioClient</tt> instance
	 */
	private AudioFormat[] getFormatsToInitializeIAudioClient() {
		...
		if (inputFormat == null)
			...
			List<AudioFormat> formats = new ArrayList<AudioFormat>(preferredFormats.length + supportedFormats.length);
			...
			for (Format format : supportedFormats) {
				if (!formats.contains(format) && (format instanceof AudioFormat)) {
				...
				Collections.sort(formats, new Comparator<AudioFormat>() {
					...
					private int computeSampleRateDistance(AudioFormat af) {
						...
						if (min == 0)
							...
							/*
							 * Prefer AudioFormats which will cause upsampling
							 * to AudioFormats which will cause downsampling.
							 */
							if (downsample) {
								l = Short.MAX_VALUE - l;
								if (h != 0)
	...
	public Format[] getSupportedInputFormats() {
		if (getLocator() == null) {
			 ...
			 * this writing.
			 */
			double sampleRate = RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE;
			int sampleSizeInBits = 16;
			int channels = 2;

			if ((sampleRate == Format.NOT_SPECIFIED) && (Constants.AUDIO_SAMPLE_RATES.length != 0))
				sampleRate = Constants.AUDIO_SAMPLE_RATES[0];
			return WASAPISystem.getFormatsToInitializeIAudioClient(new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, channels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED,
	 ...
	 * 
	 * @param data
	 *            the bytes of the audio samples to be written into the render
	 *            endpoint buffer
	 * @param offset
	 *            the offset in <tt>data</tt> at which the bytes of the audio
	 *            samples to be written into the render endpoint buffer begin
	 * @param length
	 *            the number of the bytes in <tt>data</tt> beginning at
	 *            <tt>offset</tt> of the audio samples to be written into the
	 *            render endpoint buffer
	 * @param srcSampleSize
	 ...
	 *         <tt>offset</tt>) which have been written into the render endpoint
	 *         buffer or <tt>0</tt> upon <tt>HResultException</tt>
	 */
	private int maybeIAudioRenderClientWrite(byte[] data, int offset, int length, int srcSampleSize, int srcChannels) {
		int written;

		try {
			written = IAudioRenderClient_Write(iAudioRenderClient, data, offset, length, srcSampleSize, srcChannels, dstSampleSize, dstChannels);
		} catch (HResultException hre) {
			written = 0;
	...
	private void maybeOpenResampler() {
		...
		if (outFormat.getChannels() != channels) {
			outFormat = new AudioFormat(outFormat.getEncoding(), outFormat.getSampleRate(), outFormat.getSampleSizeInBits(), channels, outFormat.getEndian(), outFormat.getSigned(),
			/* frameSizeInBits */Format.NOT_SPECIFIED,
			/* frameRate */Format.NOT_SPECIFIED, outFormat.getDataType());
		}
	 ...
	 * {@link #resampler} i.e. resamples them in order to produce media data in
	 * {@link #resamplerData} to be written into the render endpoint buffer.
	 * 
	 * @param numFramesRequested
	 *            the number of audio frames in the units of {@link #dstFormat}
	 *            requested by the rendering endpoint
	 */
	private void maybeResample(int numFramesRequested) {
		...
		if (outLength < numFramesRequested * resamplerFrameSize) {
			...
			int outFrames = (resamplerData.length - outLength) / resamplerFrameSize;
			...
			if (outFrames > 0) {
				...
				int inLength = (outFrames * srcSampleRate / dstSampleRate) * srcFrameSize;
				...
				if (inLength > 0) {
					...
					if (resampled > 0) {
						 ...
						 */
						resampled = ((resampled / resamplerFrameSize) * srcSampleRate / dstSampleRate) * srcFrameSize;
						if (resampled > 0)
	...
	public synchronized void open() throws ResourceUnavailableException {
		...
		try {
			...
			if (locatorIsNull = (locator == null)) {
				...
				/*
				 * The method getFormatsToInitializeIAudioClient will assert
				 * that inputFormat is set.
				 */
				AudioFormat[] formats = getFormatsToInitializeIAudioClient();
				long eventHandle = CreateEvent(0, false, false, null);

				try {
					long iAudioClient = audioSystem.initializeIAudioClient(locator, dataFlow,
					/* streamFlags */0, eventHandle, WASAPISystem.DEFAULT_BUFFER_DURATION, formats);
					...
					try {
						...
						try {
							 ...
							 * shared-mode stream.
							 */
							devicePeriod = IAudioClient_GetDefaultDevicePeriod(iAudioClient) / 10000L;
							numBufferFrames = IAudioClient_GetBufferSize(iAudioClient);

							int dstSampleRate = (int) dstFormat.getSampleRate();

							bufferDuration = numBufferFrames * 1000L / dstSampleRate;
							/*
							 * We will very likely be inefficient if we fail to
							 * synchronize with the scheduling period of the
							 * audio engine but we have to make do with what we
							 * have.
							 */
							if (devicePeriod <= 1) {
								devicePeriod = bufferDuration / 2;
								if ((devicePeriod > WASAPISystem.DEFAULT_DEVICE_PERIOD) || (devicePeriod <= 1))
									devicePeriod = WASAPISystem.DEFAULT_DEVICE_PERIOD;
							}
							devicePeriodInFrames = (int) (devicePeriod * dstSampleRate / 1000L);

							dstChannels = dstFormat.getChannels();
							dstSampleSize = WASAPISystem.getSampleSizeInBytes(dstFormat);

							maybeOpenResampler();

							srcChannels = srcFormat.getChannels();
							srcSampleSize = WASAPISystem.getSampleSizeInBytes(srcFormat);
							...
							if (resampler == null) {
								...
								/*
								 * The units of srcBuffer are based on
								 * srcFormat, the units of numBufferFrames are
								 * based on dstFormat.
								 */
								int srcSampleRate = (int) srcFormat.getSampleRate();

								srcBufferCapacityInFrames = numBufferFrames * srcSampleRate / dstSampleRate;
							}
							srcBuffer = new byte[srcBufferCapacityInFrames * srcFrameSize];
							if (resamplerInBuffer != null)
								resamplerInBuffer.setData(srcBuffer);

							/*
							 * Introduce latency in order to decrease the
							 * likelihood of underflow.
							 */
							srcBufferLength = srcBuffer.length;

							writeIsMalfunctioningSince = DiagnosticsControl.NEVER;
							writeIsMalfunctioningTimeout = 2 * Math.max(bufferDuration, devicePeriod);
	...
	protected synchronized void playbackDevicePropertyChange(PropertyChangeEvent ev) {
		...
		boolean open = ((iAudioClient != 0) && (iAudioRenderClient != 0)) || locatorIsNull;
	 ...
	 * 
	 * @param length
	 *            the number of bytes to pop from <tt>srcBuffer</tt>
	 */
	...
	public int process(Buffer buffer) {
		...
		synchronized (this) {
			if ((iAudioClient == 0) || (iAudioRenderClient == 0)) {
				 ...
				 * the playback.
				 */
				return locatorIsNull ? BUFFER_PROCESSED_OK : BUFFER_PROCESSED_FAILED;
			} else if (!started)
		...
		try {
			...
			if (ret != BUFFER_PROCESSED_FAILED) {
				...
				if (numFramesRequested == 0) {
					if (eventHandle == 0) {
						...
						if (toCopy > 0) {
							if (toCopy > length)
								toCopy = length;
							System.arraycopy(data, offset, srcBuffer, srcBufferLength, toCopy);
					...
					/*
					 * There is available space in the rendering endpoint buffer
					 * into which this Renderer can write data.
					 */
					int effectiveLength = srcBufferLength + length;
					int toWrite = Math.min(effectiveLength, numFramesRequested * srcFrameSize);
					byte[] effectiveData;
					int effectiveOffset;
					...
					if ((toWrite / srcFrameSize) == 0)
						...
						try {
							written = IAudioRenderClient_Write(iAudioRenderClient, effectiveData, effectiveOffset, toWrite, srcSampleSize, srcChannels, dstSampleSize, dstChannels);
						} catch (HResultException hre) {
							written = 0;
							ret = BUFFER_PROCESSED_FAILED;
							logger.error("IAudioRenderClient_Write", hre);
					...
					if (ret != BUFFER_PROCESSED_FAILED) {
						if (effectiveData == data) {
							...
							if (written == 0) {
								 ...
								 * input Buffer.
								 */
								System.arraycopy(data, offset, srcBuffer, srcBufferLength, toWrite);
				 ...
				 */
				if (((ret & INPUT_BUFFER_NOT_CONSUMED) == INPUT_BUFFER_NOT_CONSUMED) && (writeIsMalfunctioningSince != DiagnosticsControl.NEVER)) {
		 ...
		 */
		if (((ret & INPUT_BUFFER_NOT_CONSUMED) == INPUT_BUFFER_NOT_CONSUMED) && (sleep > 0)) {
	 ...
	 * 
	 * @param eventHandleCmd
	 *            the <tt>Runnable</tt> which has been initialized to wait for
	 *            <tt>eventHandle</tt> to be signaled and in whose associated
			...
			do {
				...
				try {
					...
					if (numFramesRequested > 0) {
						...
						if ((numFramesRequested > bufFrames) && (bufFrames >= devicePeriodInFrames))
						...
						int written = maybeIAudioRenderClientWrite(buf, 0, toWrite, bufSampleSize, bufChannels);
	 ...
	 * started.
	 * 
	 * @param writeIsMalfunctioning
	 *            <tt>true</tt> if the writing to the render endpoint buffer is
	 *            (believed to be) malfunctioning; otherwise, <tt>false</tt>
	 */
		...
		if (iAudioClient == 0) {
			...
			if (srcBuffer != null) {
				if (srcBufferLength > 0) {
					 ...
					 */
					for (int i = srcBuffer.length - 1, j = srcBufferLength - 1; j >= 0; i--, j--) {
				...
				if ((eventHandle != 0) && (this.eventHandleCmd == null)) {
					...
					try {
						if (eventHandleExecutor == null) {
							eventHandleExecutor = Executors.newSingleThreadExecutor();
						}
	 ...
	 * 
	 * @param locator
	 *            the <tt>MediaLocator</tt> that is to be represented in a
	...
	private String toString(MediaLocator locator) {
		...
		if (locator == null)
			...
			try {
				...
				if (id != null) {
					CaptureDeviceInfo2 cdi2 = audioSystem.getDevice(dataFlow, locator);
	...
	/**
	 * Waits on this instance while the value of {@link #eventHandleCmd} is non-
	 * <tt>null</tt>.
	 */
