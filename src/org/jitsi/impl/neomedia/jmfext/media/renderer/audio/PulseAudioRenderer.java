...
public class PulseAudioRenderer extends AbstractAudioRenderer<PulseAudioSystem> {
	...
	private static final Format[] SUPPORTED_INPUT_FORMATS = new Format[] { new AudioFormat(AudioFormat.LINEAR, Format.NOT_SPECIFIED /* sampleRate */, 16, Format.NOT_SPECIFIED /* channels */, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray) };
	 ...
	 * 
	 * @param mediaRole
	 *            the PulseAudio media role to initialize the new instance with
	 */
	public PulseAudioRenderer(String mediaRole) {
		super(PulseAudioSystem.getPulseAudioSystem(), ((mediaRole == null) || PulseAudioSystem.MEDIA_ROLE_PHONE.equals(mediaRole)) ? AudioSystem.DataFlow.PLAYBACK : AudioSystem.DataFlow.NOTIFY);
	...
	private void openWithMainloopLock() throws ResourceUnavailableException {
		...
		if ((sampleRate == Format.NOT_SPECIFIED) && (RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE != Format.NOT_SPECIFIED))
			sampleRate = (int) RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE;
		...
		try {
			stream = audioSystem.createStream(sampleRate, channels, getClass().getName(), mediaRole);
		...
		try {
			...
			try {
				...
				PA.stream_connect_playback(stream, dev, attr, PA.STREAM_ADJUST_LATENCY | PA.STREAM_START_CORKED, 0, 0);
	...
	private void setStreamVolume(long stream, float level) {
		...
		long o = PA.context_set_sink_input_volume(audioSystem.getContext(), PA.stream_get_index(stream), cvolume, null);
