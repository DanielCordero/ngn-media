...
public abstract class AbstractVideoPullBufferStream<T extends PullBufferDataSource> extends AbstractPullBufferStream<T> {
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> this <tt>PullBufferStream</tt> is to read
	 *            into
	 * @throws IOException
	 *             if an I/O error occurs while this <tt>PullBufferStream</tt>
	 *             reads into the specified <tt>Buffer</tt>
	 */
			...
			if (frameRate > 0) {
				...
				if (minimumVideoFrameInterval > 0) {
					...
					if (!buffer.isDiscard()) {
						...
						while (true) {
							// Sleep to respect the frame rate as much as
							// possible.
							long sleep = minimumVideoFrameInterval - (System.currentTimeMillis() - startTime);
							...
							if (sleep > 0) {
								...
								// Yield a little bit to not use all the whole
								// CPU.
								Thread.yield();
								break;
							}
	...
	public void start() throws IOException {
		...
		frameRateControl = (FrameRateControl) dataSource.getControl(FrameRateControl.class.getName());
	}
