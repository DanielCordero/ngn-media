...
public class MMNotificationClient {
	 ...
	 * 
	 * @param pNotify
	 *            the <tt>IMMNotificationClient</tt> to register
	 */
	public static void RegisterEndpointNotificationCallback(IMMNotificationClient pNotify) {
		...
		synchronized (MMNotificationClient.class) {
			...
			if (pNotifySet == null)
				...
				newPNotifySet = new ArrayList<IMMNotificationClient>(pNotifySet.size() + 1);
	 ...
	 * 
	 * @param pNotify
	 *            the <tt>IMMNotificationClient</tt> to delete the registration
	 *            of
	 */
		...
		synchronized (MMNotificationClient.class) {
			...
			if ((pNotifySet != null) && pNotifySet.contains(pNotify)) {
				if (pNotifySet.size() == 1)
					...
					Collection<IMMNotificationClient> newPNotifySet = new ArrayList<IMMNotificationClient>(pNotifySet);
