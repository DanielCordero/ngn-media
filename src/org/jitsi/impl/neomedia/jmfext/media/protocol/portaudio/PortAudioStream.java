...
public class PortAudioStream extends AbstractPullBufferStream<DataSource> {
	 ...
	 * name) of the PortAudio device read through this <tt>PullBufferStream</tt>
	 * .
	 */
	...
	private final DiagnosticsControl diagnosticsControl = new DiagnosticsControl() {
		 ...
		 * {@inheritDoc}
		 * 
		 * <tt>PortAudioStream</tt>'s <tt>DiagnosticsControl</tt> implementation
		 * does not provide its own user interface and always returns
		 * <tt>null</tt>.
		 */
		...
		public void didUpdateAvailableDeviceList() throws Exception {
			synchronized (PortAudioStream.this) {
				try {
					...
					/*
					 * The stream should be closed. If it is not, then something
					 * else happened in the meantime and we cannot be sure that
					 * restoring the old state of this PortAudioStream is the
					 * right thing to do in its new state.
					 */
					if (stream == 0) {
					...
					/*
					 * If we had to attempt to restore the state of this
					 * PortAudioStream, we just did attempt to.
					 */
					deviceID = null;
					start = false;
				}
			...
			synchronized (PortAudioStream.this) {
				...
				if (stream == 0) {
					...
					try {
						...
						/*
						 * If we failed to disconnect this PortAudioStream, we
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> which is creating the new instance so
	 *            that it becomes one of its <tt>streams</tt>
	 * @param formatControl
	 *            the <tt>FormatControl</tt> which is to abstract the
	 *            <tt>Format</tt>-related information of the new instance
	 * @param audioQualityImprovement
	 *            <tt>true</tt> to enable audio quality improvement for the new
	 *            instance in accord with the preferences of the user or
	...
	public PortAudioStream(DataSource dataSource, FormatControl formatControl, boolean audioQualityImprovement) {
		 ...
		 * WeakReference.
		 */
		AudioSystem2 audioSystem = (AudioSystem2) AudioSystem.getAudioSystem(AudioSystem.LOCATOR_PROTOCOL_PORTAUDIO);
	...
	private void connect() throws IOException {
		...
		if (deviceIndex == Pa.paNoDevice) {
			throw new IOException("The audio device " + deviceID + " appears to be disconnected.");
		}
		...
		int framesPerBuffer = (int) ((sampleRate * Pa.DEFAULT_MILLIS_PER_BUFFER) / (channels * 1000));

		try {
			inputParameters = Pa.StreamParameters_new(deviceIndex, channels, sampleFormat, Pa.getSuggestedLatency());

			stream = Pa.OpenStream(inputParameters, 0 /* outputParameters */, sampleRate, framesPerBuffer, Pa.STREAM_FLAGS_CLIP_OFF | Pa.STREAM_FLAGS_DITHER_OFF, null /* streamCallback */);
		...
		/*
		 * Know the Format in which this PortAudioStream will output audio data
		 * so that it can report it without going through its DataSource.
		 */
		this.format = new AudioFormat(AudioFormat.LINEAR, sampleRate, sampleSizeInBits, channels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);

		boolean denoise = false;
		boolean echoCancel = false;
		long echoCancelFilterLengthInMillis = DeviceConfiguration.DEFAULT_AUDIO_ECHOCANCEL_FILTER_LENGTH_IN_MILLIS;
		...
		Pa.setEchoFilterLengthInMillis(stream, echoCancel ? echoCancelFilterLengthInMillis : 0);
	 ...
	 *         known by it or <tt>null</tt> if this <tt>PullBufferStream</tt>
	 *         does not directly know its <tt>Format</tt> and it relies on the
	 ...
	 * 
	 * @param buffer
	 *            the <tt>Buffer</tt> in which media data is to be read from
	...
	public void read(Buffer buffer) throws IOException {
		...
		try {
			 ...
			 * allocations.
			 */
			byte[] data = AbstractCodec2.validateByteArraySize(buffer, bytesPerBuffer, false);
			...
			synchronized (this) {
				...
				if (errorCode == Pa.paNoError) {
					...
					if (readIsMalfunctioningSince != NEVER)
						setReadIsMalfunctioning(false);
				} else if ((Pa.paTimedOut == errorCode) || (Pa.HostApiTypeId.paMME.equals(hostApiType) && (Pa.MMSYSERR_NODRIVER == errorCode))) {
					if (readIsMalfunctioningSince == NEVER)
	 ...
	 *            available, the device name. If set to null, then there was no
		...
		if (this.deviceID != null) {
			...
			if (stream != 0) {
				...
				try {
					...
					if ((errorCode == Pa.paTimedOut) || (Pa.HostApiTypeId.paMME.equals(pae.getHostApiType()) && (errorCode == Pa.MMSYSERR_NODRIVER))) {
						closed = true;
					}
	 ...
	 * 
	 * @param malfunctioning
	 *            <tt>true</tt> if <tt>Pa_ReadStream</tt> is malfunctioning;
	 ...
	 * 
	 * @throws IOException
	 *             if anything goes wrong while starting the transfer of media
	 *             data from this <tt>PullBufferStream</tt>
	 */
	@Override
	 ...
	 * 
	 * @throws IOException
	 *             if anything goes wrong while stopping the transfer of media
	 *             data from this <tt>PullBufferStream</tt>
	 */
	@Override
