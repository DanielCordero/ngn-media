...
public class DirectShowStream extends AbstractPushBufferStream<DataSource> {
	 ...
	 *            suitable for attempts to be set on <tt>DirectShowStream</tt>
	 *            instances
	...
	static boolean isSupportedFormat(Format format) {
		if (format instanceof AVFrameFormat) {
			...
			if (pixFmt != -1) {
				 ...
				 * Consequently, in the absence of further calls to setFormat()
				 * by JMF, a crash may occur later (typically, during scaling)
	...
	/**
	 * The indicator which determines whether {@link #delegate} automatically
	 * drops late frames. If <tt>false</tt>, we have to drop them ourselves
	 * because DirectShow will buffer them all and the video will be late.
	 */
	 ...
	 * media data captured by {@link #delegate} out of this instance through the
	...
	/**
	 * The <tt>Object</tt> which synchronizes the access to the {@link #data}
	 * -related fields of this instance.
	 */
	 ...
	 * <tt>DirectShowStream</tt> so that the call is not made in DirectShow and
	 * we can drop late frames when {@link #automaticallyDropsLateVideoFrames}
	 * is <tt>false</tt>.
	 */
	 ...
	 *             connects to the DirectShow video capture device identified by
	 *             <tt>device</tt>
	 */
	 ...
	 * @throws IOException
	 *             if anything goes wrong while this <tt>SourceStream</tt>
	 *             disconnects from the DirectShow video capture device it has
	 *             previously connected to during the execution of
	 *             <tt>connect()</tt>
	 */
	 ...
	 *         known by it or <tt>null</tt> if this <tt>PushBufferStream</tt>
		...
		synchronized (dataSyncRoot) {
			...
			if (bufferFormat instanceof AVFrameFormat) {
				if (nativePixelFormat == DSFormat.MJPG) {
					...
					if (FFmpeg.avcodec_decode_video(avctx, avframe, data.getPtr(), data.getLength()) != -1) {
					...
					if (AVFrame.read(buffer, bufferFormat, data) < 0)
						data.free();
					/*
					 * XXX For the sake of safety, make sure that this instance
					 * does not reference the data instance as soon as it is set
					 * on the AVFrame.
					 */
					data = null;
				}
	 ...
	 * 
	 * @param source
	 *            pointer to the native <tt>DSCaptureDevice</tt> which is the
	...
	private void SampleCB(long source, long ptr, int length) {
		...
		synchronized (dataSyncRoot) {
			if (!automaticallyDropsLateVideoFrames && (data != null)) {
				...
				if (nextData != null) {
					nextData.setLength(DSCaptureDevice.samplecopy(source, ptr, nextData.getPtr(), length));
			...
			if (data != null) {
				data.setLength(DSCaptureDevice.samplecopy(source, ptr, data.getPtr(), length));
	 ...
	 * 
	 * @param device
	 *            a <tt>DSCaptureDevice</tt> which identifies the DirectShow
	 *            video capture device this <tt>SourceStream</tt> is to capture
	 *            data from
	 * @throws IOException
	 ...
	 * 
	 * @param format
	 *            the <tt>Format</tt> to set on the <tt>DSCaptureDevice</tt> of
	 *            this instance
	 * @throws IOException
	 *             if setting the specified <tt>format</tt> on the
	...
	private void setDeviceFormat(Format format) throws IOException {
		if (format == null)
			...
			if (size == null)
				...
				int hresult = device.setFormat(new DSFormat(size.width, size.height, avFrameFormat.getDeviceSystemPixFmt()));
	 ...
	 *             data from this <tt>PushBufferStream</tt>
	 */
	@Override
	...
	/**
	 * Throws a new <tt>IOException</tt> the detail message of which describes a
	 * specific <tt>HRESULT</tt> value indicating a failure.
	 * 
	 * @param hresult
	 *            the <tt>HRESUlT</tt> to be described by the detail message of
	 *            the new <tt>IOException</tt> to be thrown
	 * @throws IOException
	 */
	private void throwNewHResultException(int hresult) throws IOException {
		throw new IOException("HRESULT 0x" + Long.toHexString(hresult & 0xffffffffL));
	}
}
