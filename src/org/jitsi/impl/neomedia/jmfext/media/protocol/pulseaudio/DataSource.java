...
public class DataSource extends AbstractPullBufferCaptureDevice {
	...
	private class PulseAudioStream extends AbstractPullBufferStream<DataSource> {
		...
		private final PA.stream_request_cb_t readCallback = new PA.stream_request_cb_t() {
			@Override
		...
		private void connectWithMainloopLock() throws IOException {
			...
			try {
				stream = audioSystem.createStream(sampleRate, channels, getClass().getName(), PulseAudioSystem.MEDIA_ROLE_PHONE);
		...
		private void setStreamVolume(long stream, float level) {
			...
			long o = PA.context_set_source_output_volume(audioSystem.getContext(), PA.stream_get_index(stream), cvolume, null);
