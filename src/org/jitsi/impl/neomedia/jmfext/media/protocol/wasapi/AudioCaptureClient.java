...
import static org.jitsi.impl.neomedia.jmfext.media.protocol.wasapi.WASAPI.IAudioCaptureClient_GetNextPacketSize;
import static org.jitsi.impl.neomedia.jmfext.media.protocol.wasapi.WASAPI.IAudioCaptureClient_Read;
import static org.jitsi.impl.neomedia.jmfext.media.protocol.wasapi.WASAPI.IAudioCaptureClient_Release;
...
import static org.jitsi.impl.neomedia.jmfext.media.protocol.wasapi.WASAPI.IID_IAudioCaptureClient;
...
public class AudioCaptureClient {
	 ...
	 * 
	 * @param iAudioCaptureClient
	 *            the <tt>IAudioCaptureClient</tt> of which to retrieve the
	 *            number of frames in the next data packet
	...
	private static int maybeIAudioCaptureClientGetNextPacketSize(long iAudioCaptureClient) {
		...
		try {
			numFramesInNextPacket = IAudioCaptureClient_GetNextPacketSize(iAudioCaptureClient);
	 ...
	 * 
	 * @param audioSystem
	 *            the <tt>WASAPISystem</tt> instance which has contributed
	 *            <tt>locator</tt>
	 * @param locator
	 *            a <tt>MediaLocator</tt> which identifies the audio endpoint
	 *            device to be opened and read by the new instance
	 * @param dataFlow
	 *            the <tt>AudioSystem.DataFlow</tt> of the audio endpoint device
	 *            identified by <tt>locator</tt>. If
	 *            <tt>AudioSystem.DataFlow.PLAYBACK</tt> and
	 *            <tt>streamFlags</tt> includes
	 *            {@link WASAPI#AUDCLNT_STREAMFLAGS_LOOPBACK}, allows opening a
	 *            render endpoint device in loopback mode and inputing the data
	 *            that is being written on that render endpoint device
	 * @param streamFlags
	 *            zero or more of the <tt>AUDCLNT_STREAMFLAGS_XXX</tt> flags
	 *            defined by the <tt>WASAPI</tt> class
	 ...
	 * @param outFormat
	 *            the <tt>AudioFormat</tt> of the data to be made available by
	 *            the new instance. Eventually, the <tt>IAudioCaptureClient</tt>
	 *            to be represented by the new instance may be initialized with
	 *            a different <tt>AudioFormat</tt> in which case the new
	 *            instance will automatically transcode the data input from the
	 *            <tt>IAudioCaptureClient</tt> into the specified
	 *            <tt>outFormat</tt>.
	 * @param transferHandler
	 *            the <tt>BufferTransferHandler</tt> to be invoked when the new
	 *            instance has made data available to be read via
	 *            {@link #read(byte[], int, int)}
	 * @throws Exception
	 *             if the initialization of the new instance fails
	 */
	public AudioCaptureClient(WASAPISystem audioSystem, MediaLocator locator, AudioSystem.DataFlow dataFlow, int streamFlags, long hnsBufferDuration, AudioFormat outFormat, BufferTransferHandler transferHandler) throws Exception {
		...
		try {
			...
			try {
				...
				long iAudioCaptureClient = IAudioClient_GetService(iAudioClient, IID_IAudioCaptureClient);
				...
				try {
					...
					srcSampleSize = WASAPISystem.getSampleSizeInBytes(inFormat);

					dstChannels = outFormat.getChannels();
					dstSampleSize = WASAPISystem.getSampleSizeInBytes(outFormat);

					dstFrameSize = dstSampleSize * dstChannels;
					bufferFrames = (int) (hnsBufferDuration * sampleRate / 1000);
	 ...
	 * @param offset
	 *            the offset into <tt>buffer</tt> at which the writing of the
	 *            audio data is to begin
	 * @param length
	 *            the maximum number of bytes in <tt>buffer</tt> starting at
	 ...
	 * @throws IOException
	 *             if the reading from the internal buffer of this instance or
	 *             writing into the specified <tt>buffer</tt> fails
	 */
	 ...
	 *         the associated <tt>IAudioCaptureClient</tt> by this instance and
	 *         are available to be read out of this instance via
	 ...
	 * 
	 * @param length
	 *            the number of bytes to pop from <tt>available</tt>
	 */
	private void popFromAvailable(int length) {
		availableLength = WASAPIRenderer.pop(available, availableLength, length);
	}
	 ...
	 * @param offset
	 *            the offset in <tt>buffer</tt> at which the writing of the
	 ...
	 * @throws IOException
	 *             if the reading from this instance or the writing into the
	 *             specified <tt>buffer</tt> fails
	 */
	 ...
	 * 
	 * @param iMediaBuffer
	 *            the <tt>IMediaBuffer</tt> into which the audio data read from
	 *            this instance is to be written. If <tt>null</tt>, the writing
	 *            occurs on <tt>buffer</tt> starting at <tt>offset</tt>. If non-
	 *            <tt>null</tt>, <tt>buffer</tt> and <tt>offset</tt> are
	 *            ignored.
	 * @param buffer
	 *            the <tt>byte</tt> array into which the audio data read from
	 *            this instance is to be written if <tt>iMediaBuffer</tt> is
	 *            <tt>null</tt>; otherwise, <tt>buffer</tt> and <tt>offset</tt>
	 *            are ignored
	 * @param offset
	 *            the offset in <tt>buffer</tt> at which the writing of the
	 *            audio data is to start if <tt>iMediaBuffer</tt> is
	 *            <tt>null</tt>; otherwise, <tt>buffer</tt> and <tt>offset</tt>
	 *            are ignored
	 * @param length
	 *            the maximum number of bytes to be read from this instance into
	 *            the specified <tt>iMediaBuffer</tt> or the specified
	 *            <tt>buffer</tt> starting at <tt>offset</tt>
	 * @return the number of bytes read from this instance into the specified
	 *         <tt>iMediaBuffer</tt> or the specified <tt>buffer</tt> starting
	 *         at <tt>offset</tt>
	 * @throws IOException
	 *             if the reading of audio data from this instance fails
	 */
	 ...
	 *            this instance is to be written
	 * @param length
	 *            the maximum number of bytes to read from this instance and
	 *            write into the specified <tt>iMediaBuffer</tt>
	 * @return the number of bytes read from this instance and written into the
	 *         specified <tt>iMediaBuffer</tt>
	 * @throws IOException
	 *             if the reading of audio data from this instance or the
	 *             writing into the specified <tt>iMediaBuffer</tt> fails
	 */
	...
	private BufferTransferHandler readInEventHandleCmd() {
		 ...
		 * endpoint buffer.
		 */
		int numFramesInNextPacket = maybeIAudioCaptureClientGetNextPacketSize(iAudioCaptureClient);
		...
		if (numFramesInNextPacket != 0) {
			...
			try {
				int read = IAudioCaptureClient_Read(iAudioCaptureClient, available, availableLength, toRead, srcSampleSize, srcChannels, dstSampleSize, dstChannels);
		...
		try {
			...
			do {
				...
				synchronized (this) {
					...
					if ((iAudioClient == 0) || (iAudioCaptureClient == 0) || !started)
						break;
				...
				try {
					 ...
					 * available samples.
					 */
					numFramesInNextPacket = maybeIAudioCaptureClientGetNextPacketSize(iAudioCaptureClient);
				} finally {
					synchronized (this) {
						busy = false;
						notifyAll();
	 ...
	 * identified by the <tt>MediaLocator</tt> with which this instance has been
	 * initialized.
	 * 
	 * @throws IOException
	 *             if the starting of the transfer of media fails
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if the stopping of the transfer of media fails
	 */
