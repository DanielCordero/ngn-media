...
public class DataSource extends AbstractPushBufferCaptureDevice {
	 ...
	 * 
	 * @param locator
	 *            the <tt>MediaLocator</tt> to initialize the new instance with
	 */
	 ...
	 * 
	 * @param streamIndex
	 *            the index of the <tt>SourceStream</tt> within the list of
	 *            <tt>SourceStream</tt>s of this <tt>DataSource</tt> on behalf
	 *            of which the query is being made
	 * @return the <tt>Format</tt>s of media data supported by the audio
	 *         endpoint device associated with this instance
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * The <tt>Format</tt>s supported by this <tt>CaptureDevice</tt>/
	 * <tt>DataSource</tt> are either the ones supported by the capture endpoint
	 * device or the ones supported by the voice capture DMO that implements the
	 * acoustic echo cancellation (AEC) feature depending on whether the feature
	 * in question is disabled or enabled.
	 */
	@Override
