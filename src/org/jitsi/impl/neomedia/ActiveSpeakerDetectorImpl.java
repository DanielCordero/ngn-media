...
public class ActiveSpeakerDetectorImpl implements ActiveSpeakerDetector {
	 ...
	 * conference.
	 * 
	 * @param implClassNames
	 *            the class names of the algorithm implementations to search
	 *            through and in which the first available is to be found and
	 *            used for the detection/identification of the active/dominant
	 *            speaker in a multipoint conference
	 * @throws RuntimeException
	 *             if none of the algorithm implementations specified by
	 *             <tt>implClassNames</tt> is available
	 */
	public ActiveSpeakerDetectorImpl(String... implClassNames) {
		...
		for (String implClassName : implClassNames) {
			try {
				Class<?> implClass = Class.forName(normalizeClassName(implClassName));
	 ...
	 * conference.
	 * 
	 * @return the actual algorithm implementation used by this instance for the
	 *         detection/identification of the active/dominant speaker in a
	 ...
	 *         <tt>ActiveSpeakerDetectorImpl</tt> to implement actual algorithms
	 *         for the detection/identification of the active/dominant speaker
	...
	private static String[] getImplClassNames() {
		...
		if (implClassName == null) {
			...
			List<String> implClassNameList = new ArrayList<String>(1 + IMPL_CLASS_NAMES.length);
	 ...
	 * 
	 * @param className
	 *            the class name to prefix with a package name if necessary
