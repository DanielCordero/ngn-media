...
/**
 * TransformConnector implements the RTPConnector interface. RTPConnector is
 * originally designed for programmers to abstract the underlying transport
 * mechanism for RTP control and data from the RTPManager. However, it provides
 * the possibility to modify / transform the RTP and RTCP packets before they
 * are sent to network, or after the have been received from the network.
 * 
...
public class RTPTransformUDPConnector extends RTPConnectorUDPImpl {
	...
	protected RTPConnectorUDPInputStream createControlInputStream() throws IOException {
		RTPConnectorUDPInputStream controlInputStream = new ControlTransformInputStream(getControlSocket());
	...
	protected RTPConnectorUDPInputStream createDataInputStream() throws IOException {
		RTPConnectorUDPInputStream dataInputStream = new RTPConnectorUDPInputStream(getDataSocket());
	 ...
	 *         <tt>TransformEngine</tt> which is used to transform and
	 *         reverse-transform RTCP packets if there is currently a
	 *         <tt>TransformEngine</tt> and it specifies a
	 *         <tt>TransformEngine</tt> for RTCP data; otherwise, <tt>null</tt>
	 */
	 ...
	 * @return the <tt>PacketTransformer</tt> specified by the current
	 *         <tt>TransformEngine</tt> which is used to transform and
	 *         reverse-transform RTP packets if there is currently a
	 *         <tt>TransformEngine</tt> and it specifies a
	 *         <tt>TransformEngine</tt> for RTP data; otherwise, <tt>null</tt>
	 */
	 ...
	 * 
	 * @param engine
	 *            the <tt>TransformEngine</tt> which contains the concrete
	 *            transform logic
	 */
