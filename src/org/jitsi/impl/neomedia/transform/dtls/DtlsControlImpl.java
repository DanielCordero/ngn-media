...
public class DtlsControlImpl extends AbstractSrtpControl<DtlsTransformEngine> implements DtlsControl {
	 ...
	 * .
	 */
	static final int[] SRTP_PROTECTION_PROFILES = { SRTPProtectionProfile.SRTP_AES128_CM_HMAC_SHA1_80, SRTPProtectionProfile.SRTP_AES128_CM_HMAC_SHA1_32 };
	 ...
	 * 
	 * @param theirs
	 *            the list of <tt>SRTPProtectionProfile</tt>s to choose from
	...
	/**
	 * Computes the fingerprint of a specific certificate using a specific hash
	 * function.
	 * 
	 * @param certificate
	 *            the certificate the fingerprint of which is to be computed
	 * @param hashFunction
	 *            the hash function to be used in order to compute the
	 ...
	 */
	private static final String computeFingerprint(org.bouncycastle.asn1.x509.Certificate certificate, String hashFunction) {
		try {
	...
	static SecureRandom createSecureRandom() {
		return new SecureRandom() {
			 ...
			 * importantly though, works around a possible hang on Linux when
			 * reading from <tt>/dev/random</tt>.
			 */
			@Override
	 ...
	 * 
	 * @param certificate
	 *            the certificate the hash function of which is to be determined
	 ...
	 *         self-signed certificate to be generated by
	 *         <tt>DtlsControlImpl</tt>
	 */
	 ...
	 *            generated
	 * @param keyPair
	 *            the pair of private and public keys of the certificate to be
	 *            generated
	 * @return a new self-signed certificate with the specified <tt>subject</tt>
	 *         and <tt>keyPair</tt>
	 */
	private static org.bouncycastle.asn1.x509.Certificate generateX509Certificate(X500Name subject, AsymmetricCipherKeyPair keyPair) {
		try {
	 ...
	 * 
	 * @param fingerprint
	 *            an array of <tt>bytes</tt> which represents a fingerprint the
	 *            <tt>String</tt> representation in accord with RFC 4572 of
	 ...
	 * 
	 * @param disableSRTP
	 *            <tt>true</tt> if pure DTLS mode without SRTP extensions should
	 *            be used.
	 */
	public DtlsControlImpl(boolean disableSRTP) {
		...
		certificate = new org.bouncycastle.crypto.tls.Certificate(new org.bouncycastle.asn1.x509.Certificate[] { x509Certificate });
	 ...
	 * 
	 * @param certificate
	 *            the certificate to be verified and validated against the
	 *            fingerprints presented by the remote endpoint via the
	 *            signaling path
	 * @throws Exception
	 *             if the specified <tt>certificate</tt> failed to verify and
	 *             validate against the fingerprints presented by the remote
	 *             endpoint via the signaling path
	 */
	 ...
	 *         successfully verified and validated against the fingerprints
	 *         presented by the remote endpoint over the signaling path
	 * @throws Exception
	 *             if the specified <tt>certificate</tt> failed to verify and
	 *             validate against the fingerprints presented by the remote
	 *             endpoint over the signaling path
	 */
