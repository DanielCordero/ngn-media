...
public class TlsClientImpl extends DefaultTlsClient {
	...
	public int[] getCipherSuites() {
		...
		CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256, CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256, CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, CipherSuite.TLS_RSA_WITH_AES_128_GCM_SHA256, CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA256, CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA };
	}
	...
	public Hashtable getClientExtensions() throws IOException {
		...
		if (!getDtlsControl().isSrtpDisabled() && TlsSRTPUtils.getUseSRTPExtension(clientExtensions) == null) {
			if (clientExtensions == null)
				clientExtensions = new Hashtable();
			TlsSRTPUtils.addUseSRTPExtension(clientExtensions, new UseSRTPData(DtlsControlImpl.SRTP_PROTECTION_PROFILES, mki));
		}
	...
	public void processServerExtensions(Hashtable serverExtensions) throws IOException {
		...
		if (useSRTPData == null) {
			...
			int chosenProtectionProfile = (protectionProfiles.length == 1) ? DtlsControlImpl.chooseSRTPProtectionProfile(protectionProfiles[0]) : 0;
	...
	private class TlsAuthenticationImpl implements TlsAuthentication {
		...
		public TlsCredentials getClientCredentials(CertificateRequest certificateRequest) throws IOException {
			if (clientCredentials == null) {
				 ...
				 */
				clientCredentials = new DefaultTlsSignerCredentials(context, dtlsControl.getCertificate(), dtlsControl.getKeyPair().getPrivate(), new SignatureAndHashAlgorithm(HashAlgorithm.sha1, SignatureAlgorithm.rsa));
			}
		...
		public void notifyServerCertificate(Certificate serverCertificate) throws IOException {
			try {
				getDtlsControl().verifyAndValidateCertificate(serverCertificate);
			} catch (Exception e) {
