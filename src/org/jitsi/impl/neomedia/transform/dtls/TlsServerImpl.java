...
public class TlsServerImpl extends DefaultTlsServer {
	 ...
	 */
	private final CertificateRequest certificateRequest = new CertificateRequest(new short[] { ClientCertificateType.rsa_sign },
	 ...
	 * 
	 * @param packetTransformer
	 *            the <tt>PacketTransformer</tt> which is initializing the new
	 *            instance
	 */
	...
	protected TlsEncryptionCredentials getRSAEncryptionCredentials() throws IOException {
		if (rsaEncryptionCredentials == null) {
			...
			rsaEncryptionCredentials = new DefaultTlsEncryptionCredentials(context, dtlsControl.getCertificate(), dtlsControl.getKeyPair().getPrivate());
		}
	...
	protected TlsSignerCredentials getRSASignerCredentials() throws IOException {
		if (rsaSignerCredentials == null) {
			 ...
			 * the certificate.
			 */
			rsaSignerCredentials = new DefaultTlsSignerCredentials(context, dtlsControl.getCertificate(), dtlsControl.getKeyPair().getPrivate(), new SignatureAndHashAlgorithm(HashAlgorithm.sha1, SignatureAlgorithm.rsa));
		}
	...
	/**
	 * FIXME: If Client Hello does not include points format extensions then we
	 * will end up with alert 47 failure caused by NPE on serverECPointFormats.
	 * It was causing JitsiMeet to fail with Android version of Chrome.
	 * 
