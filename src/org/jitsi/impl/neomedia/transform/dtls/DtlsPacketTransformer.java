...
public class DtlsPacketTransformer extends SinglePacketTransformer {
	 ...
	 * @param off
	 *            the offset within <tt>buf</tt> at which the analysis is to
	 *            start
	 * @param len
	 *            the number of bytes within <tt>buf</tt> starting at
	 ...
	 * associated with.
	 */
	private RTPMediaType mediaType;
	 ...
	 * 
	 * If enabled, and this is the transformer for RTCP, it will not establish a
	 * DTLS session on its own, but rather wait for the RTP transformer to do
	 * so, and reuse it to initialize the SRTP transformer.
	 */
	 ...
	 * 
	 * @param transformEngine
	 *            the <tt>TransformEngine</tt> which is initializing the new
	 *            instance
	 * @param componentID
	 ...
	 * 
	 * @param i
	 *            the number of tries remaining after the current one
	 ...
	 * 
	 * @param ioe
	 *            the <tt>IOException</tt> to handle
	 * @param msg
	...
	private boolean handleRunInConnectThreadException(IOException ioe, String msg, int i) {
		...
		if (ioe instanceof TlsFatalAlert) {
			...
			if (alertDescription == AlertDescription.unexpected_message) {
				...
				if ((i == 0) || !Thread.currentThread().equals(connectThread) || (connector == null) || (mediaType == null)) {
	 ...
	 * 
	 * @param srtpProtectionProfile
	 *            the (negotiated) <tt>SRTPProtectionProfile</tt> to initialize
	 *            the new instance with
	 * @param tlsContext
	 *            the <tt>TlsContext</tt> which represents the keying material
	...
	private SinglePacketTransformer initializeSRTPTransformer(int srtpProtectionProfile, TlsContext tlsContext) {
		...
		/* sender */tlsContext instanceof TlsClientContext, client_write_SRTP_master_key, client_write_SRTP_master_salt, srtpPolicy, srtcpPolicy);
		SRTPContextFactory serverSRTPContextFactory = new SRTPContextFactory(
		/* sender */tlsContext instanceof TlsServerContext, server_write_SRTP_master_key, server_write_SRTP_master_salt, srtpPolicy, srtcpPolicy);
	 ...
	 * 
	 * @param tlsPeer
	 *            the <tt>TlsPeer</tt> whose associated DTLS record layer has
	 *            raised an alert
	 * @param alertLevel
	 *            {@link AlertLevel}
	 ...
	 * @param message
	 *            a human-readable message explaining what caused the alert. May
	 *            be <tt>null</tt>.
	 * @param cause
	 *            the exception that caused the alert to be raised. May be
	 *            <tt>null</tt>.
	 */
	void notifyAlertRaised(TlsPeer tlsPeer, short alertLevel, short alertDescription, String message, Exception cause) {
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		if (isDtlsRecord(buf, off, len)) {
			if (rtcpmux && Component.RTCP == componentID) {
				...
				logger.warn("Dropping a DTLS record, because it was received" + " on the RTCP channel while rtcpmux is in use.");
				return null;
			}
			...
			if (srtpTransformer == null && rtcpmux && Component.RTCP == componentID) {
	...
	private void runInConnectThread(DTLSProtocol dtlsProtocol, TlsPeer tlsPeer, DatagramTransport datagramTransport) {
		...
		if (dtlsProtocol instanceof DTLSClientProtocol) {
			...
			for (int i = CONNECT_TRIES - 1; i >= 0; i--) {
				...
				try {
					...
					if (handleRunInConnectThreadException(ioe, "Failed to accept a connection from a DTLS client!", i)) {
						continue;
					} else {
						break;
					}
		...
		SinglePacketTransformer srtpTransformer = (dtlsTransport == null || transformEngine.isSrtpDisabled()) ? null : initializeSRTPTransformer(srtpProtectionProfile, tlsContext);
	 ...
	 * 
	 * @param mediaType
	 *            the <tt>MediaType</tt> of the stream which this instance is to
	 *            work for/be associated with
	 */
	synchronized void setMediaType(RTPMediaType mediaType) {
		if (this.mediaType != mediaType) {
			RTPMediaType oldValue = this.mediaType;
	 ...
	 * 
	 * @param setup
	 *            the value of the <tt>setup</tt> SDP attribute to set on this
	 *            instance in order to determine whether this instance is to act
	 *            as a DTLS client or a DTLS server
	 */
	...
	private synchronized void start() {
		...
		if (rtcpmux && Component.RTCP == componentID) {
			/*
			 * In the case of rtcp-mux, the RTCP transformer does not create a
			 * DTLS session. The SRTP context (srtpTransformer) will be
	 ...
	 * @param off
	 *            the offset in <tt>buf</tt> where the data begins.
	 * @param len
	...
	public void sendApplicationData(byte[] buf, int off, int len) {
		...
		if (exception != null) {
			 ...
			 */
			if ((mediaType != null) && !tlsPeerHasRaisedCloseNotifyWarning) {
				logger.error("Failed to send application data over DTLS transport: ", exception);
			}
		}
	}
	...
	private SinglePacketTransformer initializeSRTCPTransformerFromRtp() {
		...
		DtlsPacketTransformer rtpDtlsPacketTransformer = (DtlsPacketTransformer) getTransformEngine().getRTPTransformer();

		PacketTransformer rtpSrtpTransformer = rtpDtlsPacketTransformer.srtpTransformer;

		if (rtpSrtpTransformer != null && rtpSrtpTransformer instanceof SRTPTransformer) {
			synchronized (this) {
				if (srtpTransformer == null) // previous check was not
												// synchronized
				{
					DtlsPacketTransformer.this.srtpTransformer = new SRTCPTransformer((SRTPTransformer) rtpSrtpTransformer);
				}
				return srtpTransformer;
			}
		}
