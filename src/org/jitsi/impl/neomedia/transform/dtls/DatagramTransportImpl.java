...
public class DatagramTransportImpl implements DatagramTransport {
	...
	/**
	 * The pool of <tt>RawPacket</tt>s instances to reduce their allocations and
	 * garbage collection.
	 */
	 ...
	 * 
	 * @param componentID
	 *            {@link Component#RTP} if the new instance is to work on
	 *            data/RTP packets or {@link Component#RTCP} if the new instance
	 *            is to work on control/RTCP packets
	 */
	public DatagramTransportImpl(int componentID) {
		...
		receiveQCapacity = RTPConnectorOutputStream.MAX_PACKETS_PER_MILLIS_POLICY_PACKET_QUEUE_CAPACITY;
	 ...
	 * 
	 * @param cause
	 *            the <tt>Throwable</tt> which would have been thrown if the bug
	 *            did not exist
	 */
	 ...
	 * 
	 * @param buf
	 *            the array of <tt>byte</tt>s which contains the packet to be
	 *            queued
	 * @param off
	 *            the offset within <tt>buf</tt> at which the packet to be
	 *            queued starts
	 * @param len
	 *            the length within <tt>buf</tt> starting at <tt>off</tt> of the
	 *            packet to be queued
	 */
	...
	public int receive(byte[] buf, int off, int len, int waitMillis) throws IOException {
		...
		/*
		 * If this DatagramTransportImpl is to be received from, then what is to
		 * be received may be a response to a request that was earlier scheduled
		 * for send.
		 */
		/*
				...
				synchronized (sendBufSyncRoot) {
					...
					if (newSendBufLength <= sendLimit) {
						if (sendBuf == null) {
							...
							sendBuf = new byte[sendLimit];
							System.arraycopy(oldSendBuf, 0, sendBuf, 0, Math.min(sendBufLength, sendBuf.length));
						}
	 ...
	 * 
	 * @param connector
	 *            the <tt>RTPConnector</tt> which represents and implements the
	 *            actual <tt>DatagramSocket</tt> to be adapted by this instance
	 */
