...
/**
 * A <tt>PacketTransformer</tt> which handles incoming ulpfec packets for a
 * single SSRC.
 * 
 * @author Boris Grozev
 */
class FECReceiver implements PacketTransformer {
	/**
	 * The <tt>Logger</tt> used by the <tt>FECReceiver</tt> class and its
	 ...
	 * <tt>a</tt> and <tt>b</tt>, taking into account the wrap at 2^16.
	 * 
	 ...
	 * 
	 * We keep them ordered by their RTP sequence numbers, so that we can easily
	 * select the oldest one to discard when the buffer is full (when the map
	 * has more than <tt>MEDIA_BUFF_SIZE</tt> entries.
	 * 
	 * We keep them in a <tt>Map</tt> so that we can easily search for a packet
	 ...
	 * We keep them ordered by their RTP sequence numbers, so that we can easily
	 * select the oldest one to discard when the buffer is full (when the map
	 * has more than <tt>FEC_BUFF_SIZE</tt> entries.
	 * 
	 * We keep them in a <tt>Map</tt> so that we can easily search for a packet
	 * with a specific sequence number.
	 */
	...
	/**
	 * Used to check whether a media packet can be recovered using available
	 * media and ulpfec packets, and to do the recovery.
	 */
	private final Reconstructor reconstructor;

	/**
	 * A <tt>Set</tt> of packets which will be reused every time a packet is
	 * recovered. Defined here to avoid recreating it on every call to
	 * <tt>reverseTransform</tt>.
	 */
	...
	/**
	 * Initialize a new <tt>FECReceiver</tt> which is to handle packets with
	 * SSRC equal to <tt>ssrc</tt>
	 * 
	 * @param ssrc
	 *            the SSRC for the <tt>FECReceiver</tt>
	 */
	 ...
	 * {@inheritDoc}
	 * 
	 * Handles incoming ulpfec packets. Modifies the sequence numbers of media
	 * packets, by subtracting the number of received ulpfec packets. This is
	 * necessary because ulpfec packets are not passed to the rest of the
	 * application, and if sequence numbers aren't rewritten, the missing fec
	 * packets will be incorrectly assumed lost.
	 * 
	 * We transform a stream that looks like this: Packets: M1 M2 M3 F1 M4 M5 F2
	 * M6 M7 Seq: 1 2 3 4 5 6 7 8 9
	 * 
	 * Into Packets: M1 M2 M3 M4 M5 M6 M7 Seq: 1 2 3 4 5 6 7
	 */
	@Override
	...
	public void close() {
		if (logger.isInfoEnabled()) {
			logger.info("Closing FECReceiver for SSRC=" + ssrc + ". Received " + nbFec + " ulpfec packets, recovered " + nbRecovered + " media packets.");
		}
	}
	 ...
	 * <tt>mediaPackets</tt> has reached <tt>MEDIA_BUFF_SIZE</tt> discards the
	 * oldest packet from it and reuses it.
	 * 
	 * @param p
	 *            the packet to copy.
	 */
	...
	/**
	 * A class that allows the recovery of a <tt>RawPacket</tt> given a set of
	 * media packets and an ulpfec packet.
	 * 
	 * Usage: 0. Create an instance specifying the map off all available media
	 * packets
	 * 
	 * 1. Call setFecPacket() with an ulpfec packet 2. Check if a recovery is
	 * possible using canRecover() 3. Recover a packet with recover()
	 * 
	 */
	private static class Reconstructor {
		 ...
		 * specific value of <tt>fecPacket</tt>) which are not available. If the
		 * value is <tt>0</tt>, this indicates that all media packets referenced
		 * in <tt>fecPacket</tt> *are* available, and so no recovery is needed.
		 */
		...
		/**
		 * Sets the ulpfec packet to be used for recovery and also updates the
		 * values of <tt>numMissing</tt>, <tt>sequenceNumber</tt> and
		 * <tt>neededPackets</tt>.
		 * 
		 * @param p
		...
		private void setFecPacket(RawPacket p) {
			...
			int base = fecPacket.readUnsignedShortAsInt(fecPacket.getHeaderLength() + 2);

			idx += 12; // skip FEC Header and Protection Length, point to mask

			outer: for (int i = 0; i < maskLen; i++) {
				for (int j = 0; j < 8; j++) {
		...
		private RawPacket recover() {
			...
			byte[] recoveredBuf = new byte[lengthRecovery + 12]; // include RTP
																	// header
			...
			int protectionLength = ((fecBuf[idx] & 0xff) << 8) | (fecBuf[idx + 1] & 0xff);
