...
/**
 * SRTPCipherF8 implements SRTP F8 Mode AES Encryption (AES-f8). F8 Mode AES
 * Encryption algorithm is defined in RFC3711, section 4.1.2.
 * 
 ...
 * size of packet data is not required to be a multiple of the AES block size
...
public class SRTPCipherF8 {
	...
	public static void process(BlockCipher cipher, byte[] data, int off, int len, byte[] iv, BlockCipher f8Cipher) {
		...
		/*
		 * Use the derived IV encryption setup to encrypt the original IV to
		 * produce IV'.
		 */
	...
	private static void processBlock(BlockCipher cipher, F8Context f8ctx, byte[] in, int inOff, byte[] out, int outOff, int len) {
		...
		/*
		 * Now XOR (S(n-1) xor IV') with the current counter, then increment the
		 * counter
		 */
