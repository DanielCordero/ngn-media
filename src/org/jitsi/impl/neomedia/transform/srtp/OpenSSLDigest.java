...
public class OpenSSLDigest implements ExtendedDigest {
	...
	private static native int EVP_DigestFinal_ex(long ctx, byte[] md, int off);

	private static native boolean EVP_DigestInit_ex(long ctx, long type, long impl);

	private static native boolean EVP_DigestUpdate(long ctx, byte[] d, int off, int cnt);
	...
	public void reset() {
		...
		if (ctx == 0) {
			...
			throw new RuntimeException("EVP_DigestInit_ex(" + getAlgorithmName() + ")");
		}
	}
