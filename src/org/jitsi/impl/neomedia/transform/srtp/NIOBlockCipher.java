 ...
 */
public interface NIOBlockCipher extends BlockCipher {
	/**
	 * Processes one block of input from the <tt>ByteBuffer</tt> <tt>in</tt> and
	 * writes it to the <tt>ByteBuffer</tt> out.
	 * 
	 * @param in
	 *            the <tt>ByteBuffer</tt> containing the input data
	 * @param inOff
	 *            offset into the <tt>ByteBuffer</tt> <tt>in</tt> the data
	 *            starts at
	 * @param out
	 *            the <tt>ByteBuffer</tt> the output data will be copied into
	 * @param outOff
	 *            the offset into the <tt>ByteBuffer</tt> out the output will
	 *            start at
	 * @exception DataLengthException
	 *                if there isn't enough data in <tt>in</tt>, or space in
	 *                <tt>out</tt>
