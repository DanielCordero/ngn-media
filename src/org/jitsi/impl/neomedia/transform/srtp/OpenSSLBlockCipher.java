 ...
 */
public class OpenSSLBlockCipher implements NIOBlockCipher {
	...
	private static native boolean EVP_CIPHER_CTX_set_padding(long x, boolean padding);

	private static native int EVP_CipherFinal_ex(long ctx, byte[] out, int outOff, int outl);

	private static native boolean EVP_CipherInit_ex(long ctx, long type, long impl, byte[] key, byte[] iv, int enc);

	private static native int EVP_CipherUpdate(long ctx, byte[] out, int outOff, int outl, byte[] in, int inOff, int inl);

	private static native int EVP_CipherUpdate(long ctx, ByteBuffer out, int outOff, int outl, ByteBuffer in, int inOff, int inl);
	 ...
	 * 
	 * @param algorithm
	 *            the algorithm with which to initialize the new instance
	 * @see #AES_128_CTR
	 * @see #AES_128_ECB
	 */
	...
	public int processBlock(byte[] in, int inOff, byte[] out, int outOff) throws DataLengthException, IllegalStateException {
		...
		if (ctx == 0) {
			...
			int i = EVP_CipherUpdate(ctx, out, outOff, blockSize, in, inOff, blockSize);
	...
	public void reset() {
		...
		if (ctx == 0) {
			...
			if (EVP_CipherInit_ex(ctx, type,
			/* impl */0L, key,
			/* iv */null, (forEncryption == null) ? -1 : (forEncryption.booleanValue() ? 1 : 0))) {
				// The manual page of EVP_CIPHER_CTX_set_padding documents it to
				// always return 1 i.e. true.
				EVP_CIPHER_CTX_set_padding(ctx, false);
			} else {
				throw new RuntimeException("EVP_CipherInit_ex(" + getAlgorithmName() + ")");
			}
		}
	}
}
