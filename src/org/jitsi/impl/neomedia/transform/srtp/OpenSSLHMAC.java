...
public class OpenSSLHMAC implements Mac {
	 ...
	 *            the algorithm of the digest to initialize the new instance
	 *            with
	...
	public void reset() {
		...
		if (ctx == 0) {
			...
			if (!HMAC_Init_ex(ctx, key, (key == null) ? 0 : key.length, md,
			/* impl */0)) {
				throw new RuntimeException("HMAC_Init_ex(" + getAlgorithmName() + ")");
			}
		}
	}
