...
public class SRTPCryptoContext extends BaseSRTPCryptoContext {
	 ...
	 * 
	 * @param sender
	 *            <tt>true</tt> if the new instance is to be used by an SRTP
	 *            sender; <tt>false</tt> if the new instance is to be used by an
	 *            SRTP receiver
	 * @param ssrc
	 ...
	 * 
	 * @param pkt
	 *            the <tt>RawPacket</tt> to authenticate
	 * @return <tt>true</tt> if the <tt>policy</tt> of this
	 *         <tt>SRTPCryptoContext</tt> specifies that authentication is to
	 *         not be performed or <tt>pkt</tt> was successfully authenticated;
	...
	boolean checkReplay(int seqNo, long guessedIndex) {
		...
		if (delta > 0) {
			...
			if (sender) {
				logger.error("Discarding RTP packet with sequence number " + seqNo + ", SSRC " + Long.toString(0xFFFFFFFFL & ssrc) + " because it is outside the replay window! (roc " + roc + ", s_l " + s_l + ", guessedROC " + guessedROC);
			}
			return false; // Packet too old.
		} else if (((replayWindow >> (-delta)) & 0x1) != 0) {
			if (sender) {
				logger.error("Discarding RTP packet with sequence number " + seqNo + ", SSRC " + Long.toString(0xFFFFFFFFL & ssrc) + " because it has been received already! (roc " + roc + ", s_l " + s_l + ", guessedROC " + guessedROC);
			}
	...
	public void deriveSrtpKeys(long index) {
		...
		if (authKey != null) {
			...
			case SRTPPolicy.SKEIN_AUTHENTICATION:
				...
				mac.init(new ParametersForSkein(new KeyParameter(authKey), ParametersForSkein.Skein512, tagStore.length * 8));
				break;
			}
	 ...
	 * 
	 * @param seqNo
	 *            the sequence number of the received SRTP packet
	...
	public void processPacketAESF8(RawPacket pkt) {
		...
		SRTPCipherF8.process(cipher, pkt.getBuffer(), pkt.getOffset() + payloadOffset, payloadLength, ivStore, cipherF8);
	}
	 ...
	 * 
	 * @param seqNo
	 *            the sequence number of the accepted SRTP packet
	 * @param guessedIndex
	 *            the SRTP index of the accepted SRTP packet calculated by
	 *            <tt>guessIndex(int)</tt>
	 */
