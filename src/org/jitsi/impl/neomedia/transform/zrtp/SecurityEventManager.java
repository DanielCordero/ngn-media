...
public class SecurityEventManager extends ZrtpUserCallback {
	 ...
	 */
	private RTPMediaType sessionType;
	 ...
	 * 
	 * @param zrtpControl
	 *            that this manager is to be associated with.
	 */
	 ...
	 * 
	 * @param sessionType
	 *            the <tt>MediaType</tt> of this session
	 */
	public void setSessionType(RTPMediaType sessionType) {
	...
	/**
	 * ZRTP computes the SAS string after nearly all the negotiation and
	 * computations are done internally.
	 * 
	 * @param sas
	 ...
	 * 
	 * @param isVerified
	 *            flag indicating whether SAS has been verified.
	 */
	...
	/**
	 * Show some information to user. ZRTP calls this method to display some
	 * information to the user. Along with the message ZRTP provides a severity
	 * indicator that defines: Info, Warning, Error, and Alert.
	 * 
	 * @param sev
	...
	public void showMessage(ZrtpCodes.MessageSeverity sev, EnumSet<?> subCode) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug(sessionTypeToString(sessionType) + ": ZRTP message: severity: " + sev + ", sub code: " + msgCode + ", multi: " + multiStreams);
		}
	}
	...
	public void zrtpNotSuppOther() {
		if (logger.isDebugEnabled()) {
			logger.debug(sessionTypeToString(sessionType) + ": Other party does not support ZRTP key negotiation" + " protocol, no secure calls possible.");
		}
	 ...
	 * 
	 * @param sessionType
	 *            the <tt>MediaType</tt> to be converted into a <tt>String</tt>
	 *            for the purposes of this <tt>SecurityEventManager</tt>
	 * @return a <tt>String</tt> representation of <tt>sessionType</tt>.
	 */
	private String sessionTypeToString(RTPMediaType sessionType) {
		switch (sessionType) {
		case AUDIO:
	...
	/**
	 * Gets the localized message and replace the param. If the param is null we
	 * ignore it.
	 * 
	 * @param key
	 *            the key for the localized message.
	 * @param param
	 *            the param to replace.
	 ...
	 * 
	 * @param securityListener
	 *            An object that receives the security events.
	 */
