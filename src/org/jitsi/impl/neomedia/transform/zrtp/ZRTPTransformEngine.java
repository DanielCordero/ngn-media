 ...
 * hashes and HMAC.</li>
 * <li>The second part of an implementation is specific <em>glue</em> code the
 * binds the GNU ZRTP core to the actual RTP/SRTP implementation and other
...
public class ZRTPTransformEngine extends SinglePacketTransformer implements SrtpControl.TransformEngine, ZrtpCallback {
	 ...
	 * 
	 * This very simple timeout provider can handle one timeout request at one
	 * time only. A second request would overwrite the first one and would lead
	 ...
	 * this time we can receive encrypted packets but we hadn't established a
	 * secure communication. This happens when a secure stream is recreated.
	 */
	 ...
	 * This gives the user interface (UI) the indication to handle the SAS as
	 * <b>not verified</b>. See implementation note below.</li>
	 ...
	 * <code>SASRelay</code> packet but do not process the relayed data. This
	 * protects the user from a malicious "trusted PBX".</li>
	 * </ul>
	 ...
	 * continuity data.
	 * 
	 * <b>Implementation note:</b></br> An application shall always display the
	 * SAS code if the SAS verify flag is <code>false</code>. The application
	 * shall also use mechanisms to remind the user to compare the SAS code, for
	 * example useing larger fonts, different colours and other display
	 * features.
	 */
	...
	/**
	 * Engine initialization method. Calling this for engine initialization and
	 * start it with auto-sensing and a given configuration setting.
	 * 
	 * @param zidFilename
	 *            The ZID file name
	 * @param config
	...
	/**
	 * Engine initialization method. Calling this for engine initialization and
	 * start it with defined auto-sensing and a default configuration setting.
	 * 
	 * @param zidFilename
	 *            The ZID file name
	 * @param autoEnable
	 ...
	 * 
	 * Calling this for engine initialization and start it with auto-sensing and
	...
	/**
	 * Custom engine initialization method. This allows to explicit specify if
	 * the engine starts with auto-sensing or not.
	 * 
	 * @param zidFilename
	 *            The ZID file name
	 * @param autoEnable
	 *            Set this true to start with auto-sensing and false to disable
	 *            it.
	 * @param config
	 *            the zrtp config to use
	 * @return true if initialization fails, false if succeeds
	 */
	 ...
	 * 
	 * @param startMuted
	 *            whether to be started as muted if no secure communication is
	 *            established
	 */
	 ...
	 * stream then check if we need to set RS2 to vaild. This is the case if we
	 * received less than 10 good SRTP packets. In this case we enable RS2 to
	 * make sure the ZRTP self-synchronization is active.
	 * 
	...
	/**
	 * The callback method required by the ZRTP implementation. First allocate
	 * space to hold the complete ZRTP packet, copy the message part in its
	 * place, the initialize the header, counter, SSRC and crc.
	 * 
	 * @param data
	 *            The ZRTP packet data
	...
	public boolean srtpSecretsReady(ZrtpSrtpSecrets secrets, EnableSecurity part) {
		...
		if (part == EnableSecurity.ForSender) {
			...
			if (secrets.getRole() == Role.Initiator) {
				srtpPolicy = new SRTPPolicy(cipher, secrets.getInitKeyLen() / 8, // key
																					// length
						authn, authKeyLen, // auth key length
						secrets.getSrtpAuthTagLen() / 8,// auth tag length
						secrets.getInitSaltLen() / 8 /* salt length */);

				SRTPContextFactory engine = new SRTPContextFactory(true /* sender */, secrets.getKeyInitiator(), secrets.getSaltInitiator(), srtpPolicy, srtpPolicy);

				srtpOutTransformer = new SRTPTransformer(engine);
				getRTCPTransformer().setSrtcpOut(new SRTCPTransformer(engine));
			} else {
				srtpPolicy = new SRTPPolicy(cipher, secrets.getRespKeyLen() / 8, // key
																					// length
						authn, authKeyLen, // auth key length
						secrets.getSrtpAuthTagLen() / 8,// auth taglength
						secrets.getRespSaltLen() / 8 /* salt length */);

				SRTPContextFactory engine = new SRTPContextFactory(true /* sender */, secrets.getKeyResponder(), secrets.getSaltResponder(), srtpPolicy, srtpPolicy);
			...
			if (secrets.getRole() == Role.Initiator) {
				...
				SRTPContextFactory engine = new SRTPContextFactory(false /* receiver */, secrets.getKeyResponder(), secrets.getSaltResponder(), srtpPolicy, srtpPolicy);
				...
				SRTPContextFactory engine = new SRTPContextFactory(false /* receiver */, secrets.getKeyInitiator(), secrets.getSaltInitiator(), srtpPolicy, srtpPolicy);
	 ...
	 * 
	 * ZRTP calls this method after it has computed the SAS and check if it is
	 * verified or not.
	 * 
	 * @param c
	...
	public boolean checkSASSignature(byte[] sasHash) {
		return (securityEventManager != null) ? securityEventManager.checkSASSignature(sasHash) : false;
	}
	 ...
	 * unsecured call back to secure mode. Just forwards the request to the Zrtp
	 * class.
	 */
	...
	/**
	 * Sets the multistream params (The multistream part needs further
	 * development)
	 * 
	 * @param parameters
	...
	/**
	 * Gets the multistream flag (The multistream part needs further
	 * development)
	 * 
	...
	/**
	 * Used to accept a PBX enrollment request (The PBX part needs further
	 * development)
	 * 
	 * @param accepted
	 *            The boolean value indicating if the request is accepted
	 */
	 ...
	 * 
	 * @return a refernce to the byte array that contains the full SAS hash.
	 */
	...
	public boolean sendSASRelayPacket(byte[] sh, ZrtpConstants.SupportedSASTypes render) {
		return (zrtpEngine != null) ? zrtpEngine.sendSASRelayPacket(sh, render) : false;
	}
	 ...
	 * 
	 * @param mitmMode
	 *            defines the new state of the mitmMode flag
	 */
	 ...
	 * 
	 * For further explanation of paranoid mode refer to the documentation of
	 * ZRtp class.
	 * 
	 * @param yesNo
	 ...
	 * 
	 * If true then we will set the enrollment flag (E) in the confirm packets
	 * and performs the enrollment actions. A MitM (PBX) enrollment service sets
	 * this flag.
	 * 
	 ...
	 * 
	 * If true then we will set the enrollment flag (E) in the confirm packets
	 * and perform the enrollment actions. A MitM (PBX) enrollment service must
	 * set this mode to true.
	 * 
	 * Can be set to true only if mitmMode is also true.
	 * 
	 * @param enrollmentMode
	 *            defines the new state of the enrollmentMode flag
	 */
	...
	public byte[] getSignatureData() {
		return (zrtpEngine != null) ? zrtpEngine.getSignatureData() : new byte[0];
	}
