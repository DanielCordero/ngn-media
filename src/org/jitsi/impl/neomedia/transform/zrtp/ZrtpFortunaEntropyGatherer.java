 ...
 * local sources only and that entropy data is never send out (via networks for
 * example).
 * 
...
public class ZrtpFortunaEntropyGatherer {
	/**
	 * The <tt>Logger</tt> used by <tt>GatherEntropy</tt> class for logging
	 * output.
	 */
	 ...
	 *            <tt>DeviceConfiguration</tt> needed to initialize
	 *            <tt>GatherEntropy</tt>
	 */
	 ...
	 * 
	 * The methods reads entropy data and seeds the ZrtpFortuna singleton. The
	 * methods seeds the first pool (0) of Fortuna to make sure that this
	 * entropy is always used.
	 * 
	...
	private class GatherAudio extends Thread implements BufferTransferHandler {
		/**
		 * The PortAudio <tt>DataSource</tt> which provides {@link #audioStream}
		 * .
		 */
		...
		/**
		 * The next three elements control the push buffer that Javasound uses.
		 */
		...
		private boolean prepareAudioEntropy() {
			CaptureDeviceInfo audioCaptureDevice = deviceConfiguration.getAudioCaptureDevice();
