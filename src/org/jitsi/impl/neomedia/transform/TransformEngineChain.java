...
public class TransformEngineChain implements TransformEngine {
	 ...
	 * specified by the <tt>engineChain</tt> array for outgoing packets and in
	 * the reverse order for incoming packets.
	 * 
	 * @param engineChain
	 *            an array containing <tt>TransformEngine</tt>s in the order
	 *            that they are to be applied on outgoing packets.
	 */
	...
	/**
	 * Returns the meta <tt>PacketTransformer</tt> that will be applying RTCP
	 * transformations from all engines registered in this
	...
	private class PacketTransformerChain implements PacketTransformer {
		/**
		 * Indicates whether this transformer will be dealing with RTP or, in
		 * other words, whether it will transform packets via the RTP
		 ...
		 * 
		 * @param isRtp
		 *            <tt>true</tt> if this transformer will be dealing with RTP
		 *            (i.e. will transform packets via the RTP transformers in
		 *            this chain rather than the RTCP ones) and <tt>false</tt>
		 *            otherwise.
		 */
		...
		public void close() {
			for (TransformEngine engine : engineChain) {
				PacketTransformer pTransformer = isRtp ? engine.getRTPTransformer() : engine.getRTCPTransformer();
