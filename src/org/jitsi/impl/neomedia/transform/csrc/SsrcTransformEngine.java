...
public class SsrcTransformEngine extends SinglePacketTransformer implements TransformEngine {
	 ...
	 * 
	 * @param mediaStream
	 *            the <tt>MediaStreamImpl</tt> to utilize the new instance
	 */
	...
	private static synchronized void readConfigurationServicePropertiesOnce() {
		...
		if (cfg != null) {
			dropMutedAudioSourceInReverseTransform = cfg.getBoolean(SsrcTransformEngine.DROP_MUTED_AUDIO_SOURCE_IN_REVERSE_TRANSFORM, dropMutedAudioSourceInReverseTransform);
		}
	}
	 ...
	 * 
	 * @param pkt
	 *            the RTP <tt>RawPacket</tt> that we are to extract a SSRC list
	 *            from.
	 * 
	 * @return the same <tt>RawPacket</tt> that was received as a parameter
	 *         since we don't need to worry about hiding the SSRC list from the
	...
	public RawPacket reverseTransform(RawPacket pkt) {
		...
		if ((ssrcAudioLevelExtID > 0) && ssrcAudioLevelDirection.allowsReceiving() && !pkt.isInvalid() && (RTPHeader.VERSION == ((pkt.readByte(0) & 0xC0) >>> 6))) {
			...
			if (level == 127 /* a muted audio source */) {
				if (dropMutedAudioSourceInReverseTransform) {
					dropPkt = droppedMutedAudioSourceInReverseTransform < MAX_DROPPED_MUTED_AUDIO_SOURCE_IN_REVERSE_TRANSFORM;
				} else {
	 ...
	 * <tt>SsrcTransformEngine</tt> supports only reading SSRC audio levels, not
	 * writing them.
	 */
	@Override
