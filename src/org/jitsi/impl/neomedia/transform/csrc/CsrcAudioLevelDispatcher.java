...
/**
 * A simple thread that waits for new levels to be reported from incoming RTP
 * packets and then delivers them to the <tt>AudioMediaStream</tt> associated
 * with this engine. The reason we need to do this in a separate thread is, of
 * course, the time sensitive nature of incoming RTP packets.
 * 
 * @author Emil Ivov
...
public class CsrcAudioLevelDispatcher implements Runnable {
	/**
	 * The pool of <tt>Thread</tt>s which run <tt>CsrcAudioLevelDispatcher</tt>
	 * s.
	 */
	...
	/**
	 * Initializes a new <tt>CsrcAudioLevelDispatcher</tt> to dispatch events to
	 * a specific <tt>AudioMediaStreamImpl</tt>.
	 * 
	 * @param mediaStream
	 *            the <tt>AudioMediaStreamImpl</tt> to which the new instance is
	 *            to dispatch events
	 */
