...
/**
 * We use this engine to add the list of CSRC identifiers in RTP packets that we
 * send to conference participants during calls where we are the mixer.
 * 
 * @author Emil Ivov
...
public class CsrcTransformEngine extends SinglePacketTransformer implements TransformEngine {
	 ...
	 * 
	 * @param stream
	 *            that <tt>MediaStream</tt> whose RTP packets we are going to be
	 *            adding CSRC lists. to
	 */
	public CsrcTransformEngine(MediaStreamImpl mediaStream) {
		...
		if ((activeRTPExtensions != null) && !activeRTPExtensions.isEmpty()) {
			for (Map.Entry<Byte, RTPExtension> e : activeRTPExtensions.entrySet()) {
				...
				if (RTPExtension.CSRC_AUDIO_LEVEL_URN.equals(uri)) {
					...
					setCsrcAudioLevelExtensionID((extID == null) ? -1 : extID.byteValue(), rtpExtension.getDirection());
				}
			}
		}
		...
		if (this.mediaStream instanceof AudioMediaStreamImpl) {
			csrcAudioLevelDispatcher = new CsrcAudioLevelDispatcher((AudioMediaStreamImpl) this.mediaStream);
		} else {
	 ...
	 * 
	 * @param csrcList
	 *            the list of CSRC IDs whose level we'd like the extension to
	 *            contain.
	 ...
	 *            the minimum length that we need the returned buffer to have.
	 ...
	 * method does not do any transformations since CSRC lists are part of RFC
	 * 3550 and they shouldn't be disrupting the rest of the application.
	 * 
	 * @param pkt
	 *            the RTP <tt>RawPacket</tt> that we are to extract a CSRC list
	 *            from.
	 * 
	 * @return the same <tt>RawPacket</tt> that was received as a parameter
	 *         since we don't need to worry about hiding the CSRC list from the
	 *         rest of the RTP stack.
	 */
	@Override
	 ...
	 * 
	 * @param extID
	 *            ID that this transformer should be using for audio level
	 *            extensions or <tt>-1</tt> if audio level extensions should be
	 *            disabled
	 * @param dir
	 *            the direction that we are expected to hand this extension in.
	 * 
	 */
	 ...
	 * 
	 * @param pkt
	 *            the RTP <tt>RawPacket</tt> that we need to add a CSRC list to.
	 * 
	...
	public synchronized RawPacket transform(RawPacket pkt) {
		...
		if ((csrcAudioLevelExtID > 0) && csrcAudioLevelDirection.allowsSending() && (mediaStream instanceof AudioMediaStreamImpl)) {
