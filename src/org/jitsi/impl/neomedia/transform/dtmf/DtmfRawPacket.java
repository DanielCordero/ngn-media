...
/**
 * <tt>DtmfRawPacket</tt> represent an RTP Packet. You create your
 * <tt>DtmfRawPacket</tt> by calling the constructor. You specify the DTMF
 * attributes : code=9, end=false, marker=true ... Then you fill the packet
 * using init( ... dtmf attributes ... );
 * 
...
public class DtmfRawPacket extends RawPacket {
	 ...
	 *            content
	 * @param offset
	 *            the index where we should start using the <tt>buffer</tt>.
	 * @param length
	 *            Length of the packet's data.
	 * @param payload
	 *            the payload that has been negotiated for telephone events by
	 *            our signaling modules.
	 */
	...
	/**
	 * Used for incoming DTMF packets, creating <tt>DtmfRawPacket</tt> from RTP
	 * one.
	 * 
	 * @param pkt
	 *            the RTP packet.
	 ...
	 * 
	 * @param code
	 *            the DTMF code representing the digit.
	 * @param end
	 *            the DTMF End flag
	 * @param marker
	 *            the RTP Marker flag
	...
	/**
	 * Initializes the a DTMF raw data using event, E and duration field. Event
	 * : the digits to transmit (0-15). E : End field, used to mark the two last
	 * packets. R always = 0. Volume always = 0. Duration : duration increments
	 * for each dtmf sending updates, stay unchanged at the end for the 3 last
	 * packets.
	 * 
	 * <pre>
	 *  0                   1                   2                   3
	 ...
	 * 
	 * @param code
	 *            the digit to transmit 0-15
	 * @param end
	 *            boolean used to mark the two last packets
	 * @param duration
	 *            int increments for each dtmf sending updates, stay unchanged
	 *            at the end for the 2 last packets.
	 * @param volume
