...
public class DtmfTransformEngine extends SinglePacketTransformer implements TransformEngine {
	 ...
	 * control concurrent modification access to "currentTone", "nbToneToStop"
	 * and "toneTransmissionState".
	 */
	...
	/**
	 * The maximal DTMF tone dration. The default value is -1 telling to stop
	 * only when the user asks to. This can be changed by using the
	...
	/**
	 * Creates an engine instance that will be replacing audio packets with DTMF
	 * ones upon request.
	 * 
	 * @param stream
	 *            the <tt>AudioMediaStream</tt> whose RTP packets we are going
	 *            to be replacing with DTMF.
	 */
	...
	private int getCurrentSpacingDuration() {
		if (currentSpacingDuration == Format.NOT_SPECIFIED) {
			RTPMediaFormat format = mediaStream.getFormat();
			double clockRate;

			if (format == null) {
				RTPMediaType mediaType = mediaStream.getMediaType();
	...
	/**
	 * Always returns <tt>null</tt> since this engine does not require any RTCP
	 * transformations.
	 * 
	 * @return <tt>null</tt> since this engine does not require any RTCP
	 *         transformations.
	 */
	 ...
	 * 
	 * @param pkt
	 *            an incoming packet that we need to parse and handle in case we
	 *            determine it to be DTMF.
	 * 
	 * @return the <tt>pkt</tt> if it is not a DTMF tone and <tt>null</tt>
	 *         otherwise since we will be handling the packet ourselves and
	 *         their's no point in feeding it to the application.
	 */
	public RawPacket reverseTransform(RawPacket pkt) {
		byte currentDtmfPayload = mediaStream.getDynamicRTPPayloadType(AudioFormat.TELEPHONE_EVENT);
	...
	public RawPacket transform(RawPacket pkt) {
		...
		if (toneTransmissionState == ToneTransmissionState.IDLE) {
		...
		} else if (toneTransmissionState == ToneTransmissionState.SENDING || (toneTransmissionState == ToneTransmissionState.END_REQUESTED && !lastMinimalDuration)) {
			currentDuration += getCurrentSpacingDuration();
			pktDuration = currentDuration;
			...
			if (currentDuration > 0xFFFF) {
				...
				// 0xFFFF. For the next packet, the duration start from
				// beginning
		...
		dtmfPkt.init(toneCode, pktEnd, pktMarker, pktDuration, currentTimestamp, volume);
		pkt = dtmfPkt;
	 ...
	 * 
	 * @param tone
	 *            the tone that we'd like to start sending.
	...
	public void stopSendingDTMF() {
		synchronized (startStopToneMutex) {
			...
			int stoppingTone = (toneTransmissionState == ToneTransmissionState.END_REQUESTED || toneTransmissionState == ToneTransmissionState.END_SEQUENCE_INITIATED) ? 1 : 0;
	...
	/**
	 * A simple thread that waits for new tones to be reported from incoming RTP
	 * packets and then delivers them to the <tt>AudioMediaStream</tt>
	...
	private class DTMFDispatcher implements Runnable {
		...
		/**
		 * A packet that we should convert to tone and deliver to our media
		 * stream and its listeners in a separate thread.
		 * 
		 * @param p
		 *            the packet we will convert and deliver.
		 */
