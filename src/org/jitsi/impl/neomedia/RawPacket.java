...
public class RawPacket {
	...
	/**
	 * Start offset of the packet data inside buffer. Usually this value would
	 * be 0. But in order to be compatible with RTPManager we store this info.
	 * (Not assuming the offset is always zero)
	 */
	private int offset;
	 ...
	 * 
	 * @param buffer
	 *            the <tt>byte</tt> array to be the buffer of the new instance
	 * @param offset
	 *            the offset in <tt>buffer</tt> at which the actual data to be
	 *            represented by the new instance starts
	 * @param length
	 *            the number of <tt>byte</tt>s in <tt>buffer</tt> which
	 ...
	 * according the rules specified in RFC 5285. Note that this method does not
	 * replace extensions so if you add the same buffer twice it would be added
	 * as to separate extensions.
	 * 
	 * @param extBuff
	 *            the buffer that we'd like to add as an extension in this
	 *            packet.
	 * @param newExtensionLen
	 *            the length of the data in extBuff.
	 */
	 ...
	 * 
	 * @param csrcExtID
	 *            the ID of the extension that's transporting csrc audio levels
	 *            in the session that this <tt>RawPacket</tt> belongs to.
	 * 
	 * @return an array representing a map binding CSRC IDs to audio levels as
	 *         reported by the remote party that sent this packet. The entries
	 *         of the map are contained in consecutive elements of the returned
	 *         array where elements at even indices stand for CSRC IDs and
	...
	public long[] extractCsrcAudioLevels(byte csrcExtID) {
		...
		for (int i = 0, csrcStartIndex = offset + FIXED_HEADER_SIZE; i < csrcCount; i++, csrcStartIndex += 4) {
	 ...
	 * 
	 * @param ssrcExtID
	 *            the ID of the extension that's transporting ssrc audio levels
	 *            in the session that this <tt>RawPacket</tt> belongs to
	 * @return the source audio level reported by the remote party which sent
	 *         this packet and carried in this packet or a negative value if
	 *         this packet contains no extension such as the specified by
	 *         <tt>ssrcExtID</tt>
	 */
	 ...
	 * 
	 * @param extensionID
	 *            the ID of the extension whose content we are looking for.
	 * 
	 * @return the index of the first byte of the content of the extension with
	 *         the specified <tt>extensionID</tt> or -1 if no such extension was
	 *         found.
	 */
	private int findExtension(int extensionID) {
		...
		int extOffset = offset + FIXED_HEADER_SIZE + getCsrcCount() * 4 + EXT_HEADER_SIZE;
	 ...
	 *         <tt>-1</tt> in case there were no extension headers here or we
	 *         didn't understand the kind of extension being used.
	 */
	private int getExtensionHeaderLength() {
		...
		if (buffer[extLenIndex] == (byte) 0xBE && buffer[extLenIndex + 1] == (byte) 0xDE)
			return 1;

		// 0x100 means a two-byte extension header.
		if (buffer[extLenIndex] == (byte) 0x10 && (buffer[extLenIndex + 1] >> 4) == 0)
			return 2;
	...
	public int getExtensionLength() {
		...
		return ((buffer[extLenIndex] << 8) | (buffer[extLenIndex + 1] & 0xFF)) * 4;
	}
	...
	public int getHeaderExtensionType() {
		...
		return readUnsignedShortAsInt(offset + FIXED_HEADER_SIZE + getCsrcCount() * 4);
	}
	 ...
	 * 
	 * @param contentStart
	 *            the index of the first element of the content of the extension
	 *            whose size we are trying to obtain.
	 * 
	 ...
	 * <tt>true</tt> if it is found to be invalid. A return value of
	 * <tt>false</tt> does not necessarily mean that the packet is valid.
	 * 
	 * @return <tt>true</tt> if the RTP/RTCP packet represented by this instance
	 *         is found to be invalid, <tt>false</tt> otherwise.
	 */
	public boolean isInvalid() {
		return (buffer == null) || (buffer.length < offset + length) || (length < FIXED_HEADER_SIZE);
	}
	...
	public int readInt(int off) {
		off += offset;
		return ((buffer[off++] & 0xFF) << 24) | ((buffer[off++] & 0xFF) << 16) | ((buffer[off++] & 0xFF) << 8) | (buffer[off] & 0xFF);
	}
	 ...
	 * 
	 * @param off
	 *            start offset of the region to be read
	 * @param len
	...
	public void removeExtension() {
		...
		System.arraycopy(buffer, payloadOffset, buffer, payloadOffset - extHeaderLen, getPayloadLength());
	 ...
	 * 
	 * @param newCsrcList
	 *            the list of CSRC identifiers that we'd like to set for this
	 *            <tt>RawPacket</tt>.
	 */
	public void setCsrcList(long[] newCsrcList) {
		...
		byte[] newBuffer = new byte[length + offset + csrcBuff.length - oldCsrcCount * 4];
		...
		System.arraycopy(oldBuffer, payloadOffsetForOldBuff, newBuffer, payloadOffsetForNewBuff, length - payloadOffsetForOldBuff);

		// set the new CSRC count
		newBuffer[offset] = (byte) ((newBuffer[offset] & 0xF0) | newCsrcCount);

		this.buffer = newBuffer;
		this.length = payloadOffsetForNewBuff + length - payloadOffsetForOldBuff - offset;
	}
	 ...
	 * 
	 * @param extBit
	 *            the flag that indicates whether we are to set or clear the
	 ...
	 * 
	 * @param flags
	 *            the bitmap/flag mask that specifies the set of boolean
	 ...
	 * 
	 * @param marker
	 *            <tt>true</tt> if we are to raise the marker bit and
	 ...
	 * 
	 * @param payload
	 *            the RTP payload type describing the content of this packet.
	 */
	 ...
	 * 
	 * @param seq
	 *            the sequence number to set (only the least-significant 16bits
	 *            are used)
	 */
	 ...
	 * @param data
	 *            The integer to store in the packet
	 */
