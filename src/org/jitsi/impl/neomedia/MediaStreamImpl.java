...
import org.jitsi.impl.neomedia.transform.rtcp.RTCPTerminationTransformEngine;
...
public class MediaStreamImpl extends AbstractMediaStream implements ReceiveStreamListener, SendStreamListener, SessionListener, RemoteListener {
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> to return a human-readable
	 *            representation of
	 ...
	 * <tt>MediaFormat</tt>s.
	 */
	private final Map<Byte, RTPMediaFormat> dynamicRTPPayloadTypes = new HashMap<Byte, RTPMediaFormat>();
	 ...
	 */
	private long localSourceID = Math.abs(new Random().nextLong()) % Integer.MAX_VALUE;
	...
	/**
	 * The MediaStreamStatsImpl object used to compute the statistics about this
	 * MediaStreamImpl.
	 */
	 ...
	 * 
	 * @param device
	 *            the <tt>MediaDevice</tt> the new instance is to use for both
	 *            capture and playback of media
	 * @param srtpControl
	 *            an existing control instance to control the SRTP operations
	 */
	 ...
	 * 
	 * @param connector
	 *            the <tt>StreamConnector</tt> the new instance is to use for
	 *            sending and receiving media or <tt>null</tt> if the
	 *            <tt>StreamConnector</tt> of the new instance is to not be set
	 *            at initialization time but specified later on
	 * @param device
	 *            the <tt>MediaDevice</tt> the new instance is to use for both
	 *            capture and playback of media exchanged via the specified
	 *            <tt>StreamConnector</tt>
	 * @param srtpControl
	 *            an existing control instance to control the ZRTP operations or
	 *            <tt>null</tt> if a new control instance is to be created by
	...
	public MediaStreamImpl(StreamConnector connector, MediaDevice device, SrtpControl srtpControl) {
		...
		this.srtpControl = (srtpControl == null) ? NeomediaServiceUtils.getMediaServiceImpl().createSrtpControl(SrtpControlType.ZRTP) : srtpControl;
	 ...
	 * @param format
	 *            the <tt>MediaFormat</tt> to be associated in this
	 *            <tt>MediaStream</tt> with <tt>rtpPayloadType</tt>
	 * @see MediaStream#addDynamicRTPPayloadType(byte, MediaFormat)
	 */
	@Override
	public void addDynamicRTPPayloadType(byte rtpPayloadType, RTPMediaFormat format) {
		@SuppressWarnings("unchecked")
		RTPMediaFormat mediaFormatImpl = (RTPMediaFormat) format;
		...
		synchronized (dynamicRTPPayloadTypes) {
			...
			if (VideoFormat.RED.equals(encoding)) {
			...
			} else if (VideoFormat.ULPFEC.equals(encoding)) {
	 ...
	 * we've set with {@link #addDynamicRTPPayloadType(byte, MediaFormat)}. This
	 * is necessary so that we can support the RFC3264 case where the answerer
	 * has the right to declare what payload type mappings it wants to receive
	 * RTP packets with even if they are different from those in the offer.
	 * RFC3264 claims this is for support of legacy protocols such as H.323 but
	 * we've been bumping with a number of cases where multi-component pure SIP
	 * systems also need to behave this way.
	 * <p>
	 * 
	 * @param originalPt
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> to add
	 ...
	 *            the SSRC identifier that this stream will be using in outgoing
	 *            RTP packets from now on.
	 */
	 ...
	 * 
	 * @param extensionID
	 *            the ID that is being mapped to <tt>rtpExtension</tt>
	 * @param rtpExtension
	...
	public void close() {
		/*
		 * Some statistics cannot be taken from the RTP manager and have to be
		 * gathered from the ReceiveStream. We need to do this before calling
		 * stop().
		 */
		...
		if (rtpManager != null) {
			...
			try {
				 ...
				 * documented in #stopSendStreams(Iterable<SendStream>,
				 * boolean). It is unknown at the time of this writing whether
				 * we can do anything to prevent the exception here but it is
				 * clear that, if we let it go through, we will not release at
				 * least one capture device (i.e. we will at least skip the
	 ...
	 *            the <tt>RTPConnectorInputStream</tt> to be used by an
	 *            <tt>RTPManager</tt> of this <tt>MediaStreamImpl</tt> and to be
	 *            configured
	 */
	 ...
	 *            the <tt>RTPConnectorOutputStream</tt> to be used by an
	 *            <tt>RTPManager</tt> of this <tt>MediaStreamImpl</tt> and to be
	 *            configured
	 */
	 ...
	 * 
	 * @param rtpManager
	 *            the <tt>RTPManager</tt> which is to be used by this
	 *            <tt>MediaStreamImpl</tt>
	 * @param bufferControl
	 *            the <tt>BufferControl</tt> of <tt>rtpManager</tt> on which any
	 *            optional configuration is to be performed
	 */
	...
	/**
	 * Creates a chain of transform engines for use with this stream. Note that
	 * this is the only place where the <tt>TransformEngineChain</tt> is and
	 * should be manipulated to avoid problems with the order of the
	 * transformers.
	 * 
	...
	private TransformEngineChain createTransformEngineChain() {
		...
		if (RTPMediaType.VIDEO.equals(getMediaType())) {
	 ...
	 * 
	 * @param oldValue
	 *            the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt>
	 *            this instance used work with
	 * @param newValue
	 *            the <tt>MediaDeviceSession</tt> with the <tt>MediaDevice</tt>
	 *            this instance is to work with
	 */
	 ...
	 * 
	 * @param ev
	 *            the <tt>PropetyChangeEvent</tt> containing the list of SSRC
	 *            identifiers handled by our device session before and after it
	 *            changed.
	 */
	 ...
	 * 
	 * @param target
	 *            the <tt>MediaStreamTarget</tt> describing the data (e.g. RTP)
	 *            and the control data (e.g. RTCP) locations to which this
	 *            <tt>MediaStream</tt> is to send and from which it is to
	 *            receive
	...
	private void doSetTarget(MediaStreamTarget target) {
		...
		if (target == null)
			...
			try {
				...
				rtpConnector.addTarget(new SessionAddress(newDataAddr.getAddress(), newDataAddr.getPort(), controlInetAddr, controlPort));
				targetIsSet = true;
			} catch (IOException ioe) {
				targetIsSet = false;
	 ...
	 * 
	 * @param rtpExtension
	 *            the RTP extension to get the currently assigned ID of
	 ...
	 * 
	 * @param encoding
	 *            the encoding whose payload type we are trying to obtain.
	 * 
	 * @return the payload type number that has been negotiated for the
	 *         specified <tt>encoding</tt> or <tt>-1</tt> if no payload type has
	 *         been negotiated for it.
	 */
	public byte getDynamicRTPPayloadType(String encoding) {
		synchronized (dynamicRTPPayloadTypes) {
			for (Map.Entry<Byte, RTPMediaFormat> dynamicRTPPayloadType : dynamicRTPPayloadTypes.entrySet()) {
	 ...
	 * payload types to <tt>MediaFormat</tt>s. The returned <tt>Map</tt> only
	 * contains associations previously added in this instance with
	 ...
	 *         <tt>MediaFormat</tt> describing the existing (dynamic)
	 *         associations in this instance of RTP payload types to
	 *         <tt>MediaFormat</tt>s. The <tt>Map</tt> represents a snapshot of
	 *         the existing associations at the time of the
	 *         <tt>getDynamicRTPPayloadTypes()</tt> method call and
	 *         modifications to it are not reflected on the internal storage
	 * @see MediaStream#getDynamicRTPPayloadTypes()
	 */
	@Override
	public Map<Byte, RTPMediaFormat> getDynamicRTPPayloadTypes() {
		synchronized (dynamicRTPPayloadTypes) {
			return new HashMap<Byte, RTPMediaFormat>(dynamicRTPPayloadTypes);
		}
	}

	/**
	 * Creates the <tt>FECTransformEngine</tt> for this <tt>MediaStream</tt>. By
	 * default none is created, allows extenders to implement it.
	 * 
	...
	/**
	 * Returns the list of CSRC identifiers for all parties currently known to
	 * contribute to the media that this stream is sending toward its remote
	 ...
	 *         currently known to contribute to the media that this stream is
	 *         sending or an <tt>null</tt> in case this <tt>MediaStream</tt> is
	 *         not part of a conference call.
	 */
	 ...
	 * 
	 * @return the synchronization source (SSRC) identifier of the local peer or
	 *         <tt>-1</tt> if it is not yet known
	...
	public RTPMediaType getMediaType() {
		RTPMediaFormat format = getFormat();
		RTPMediaType mediaType = null;
		...
		if (mediaType == null) {
			...
			if (mediaType == null) {
				if (this instanceof AudioMediaStream)
					mediaType = RTPMediaType.AUDIO;
				else if (this instanceof VideoMediaStream)
					mediaType = RTPMediaType.VIDEO;
			}
		}
	 ...
	 * implementations can override this and return different than current
	 * default value.
	 * 
	 ...
	 *         associated <tt>MediaDevice</tt> and which has the specified
	 *         <tt>ssrc</tt>
	 */
	...
	/**
	 * Creates the <tt>REDTransformEngine</tt> for this <tt>MediaStream</tt>. By
	 * default none is created, allows extenders to implement it.
	 * 
	 ...
	 * RTCP).
	 * 
	 * @return the <tt>MediaStreamTarget</tt> describing the data (e.g. RTP) and
	 *         the control data (e.g. RTCP) locations to which this
	 ...
	 * 
	 * @param rtpManager
	 *            the <tt>StreamRTPManager</tt> to update the registered FMJ
	 *            <tt>Format</tt>s of. If <tt>null</tt>, the method uses
	 *            {@link #rtpManager}.
	 */
	private void maybeUpdateDynamicRTPPayloadTypes(StreamRTPManager rtpManager) {
		...
		RTPMediaFormat mediaFormat = getFormat();

		if (!(mediaFormat instanceof RTPMediaFormat))
			return;
		...
		synchronized (dynamicRTPPayloadTypes) {
			for (Map.Entry<Byte, RTPMediaFormat> dynamicRTPPayloadType : dynamicRTPPayloadTypes.entrySet()) {
				RTPMediaFormat dynamicMediaFormat = dynamicRTPPayloadType.getValue();

				if (!(dynamicMediaFormat instanceof RTPMediaFormat))
					continue;

				@SuppressWarnings("unchecked")
				RTPMediaFormat dynamicMediaFormatImpl = (RTPMediaFormat) dynamicMediaFormat;
	 ...
	 * 
	 * @param rtpManager
	 *            the <tt>RTPManager</tt> to print statistics for
	 */
	private void printFlowStatistics(StreamRTPManager rtpManager) {
		try {
			...
			buff.append("call stats for incoming ").append((format == null) ? "" : format).append(" stream SSRC: ").append(getRemoteSourceID()).append(eol).append("packets received: ").append(rs.getPacketsRecd()).append(eol).append("bytes received: ").append(rs.getBytesRecd()).append(eol).append("packets lost: ").append(statisticsEngine.getLost()).append(eol).append("min interarrival jitter: ").append(statisticsEngine.getMinInterArrivalJitter()).append(eol).append("max interarrival jitter: ").append(statisticsEngine.getMaxInterArrivalJitter()).append(eol).append("RTCPs received: ").append(rs.getRTCPRecd()).append(eol).append("bad RTCP packets: ").append(rs.getBadRTCPPkts()).append(eol).append("bad RTP packets: ").append(rs.getBadRTPkts()).append(eol).append("local collisions: ")
					.append(rs.getLocalColls()).append(eol).append("malformed BYEs: ").append(rs.getMalformedBye()).append(eol).append("malformed RRs: ").append(rs.getMalformedRR()).append(eol).append("malformed SDESs: ").append(rs.getMalformedSDES()).append(eol).append("malformed SRs: ").append(rs.getMalformedSR()).append(eol).append("packets looped: ").append(rs.getPacketsLooped()).append(eol).append("remote collisions: ").append(rs.getRemoteColls()).append(eol).append("SRs received: ").append(rs.getSRRecd()).append(eol).append("transmit failed: ").append(rs.getTransmitFailed()).append(eol).append("unknown types: ").append(rs.getUnknownTypes());
	...
	protected void recreateSendStreams() {
		if (sendStreamsAreCreated) {
			...
			if ((getDeviceSession() != null) && (rtpManager != null)) {
				if (MediaDirection.SENDONLY.equals(startedDirection) || MediaDirection.SENDRECV.equals(startedDirection))
					startSendStreams();
			}
		}
	}
	 ...
	 * 
	 * @param rtpManager
	 *            the <tt>RTPManager</tt> to register any custom JMF
	 *            <tt>Format</tt>s with
	 */
	protected void registerCustomCodecFormats(StreamRTPManager rtpManager) {
		synchronized (dynamicRTPPayloadTypes) {
			for (Map.Entry<Byte, RTPMediaFormat> dynamicRTPPayloadType : dynamicRTPPayloadTypes.entrySet()) {
				@SuppressWarnings("unchecked")
				RTPMediaFormat mediaFormatImpl = (RTPMediaFormat) dynamicRTPPayloadType.getValue();
				Format format = mediaFormatImpl.getFormat();
	 ...
	 * 
	 * @param oldValue
	 *            the <tt>RTPConnector</tt> of this <tt>MediaStream</tt>
	 *            implementation before it got changed to <tt>newValue</tt>
	 * @param newValue
	 *            the current <tt>RTPConnector</tt> of this <tt>MediaStream</tt>
	 *            which replaced <tt>oldValue</tt>
	 */
	 ...
	 * 
	 * @param inputStream
	 *            the new <tt>RTPConnectorInputStream</tt> instance created by
	 *            the <tt>rtpConnector</tt> of this instance
	 * @param data
	 *            <tt>true</tt> if <tt>inputStream</tt> will be used for RTP or
	 *            <tt>false</tt> for RTCP
	 */
	private void rtpConnectorInputStreamCreated(RTPConnectorInputStream<?> inputStream, boolean data) {
		 ...
		 * RTPConnectorInputStreams created by the rtpConnector of this instance
		 * so they may be configured from outside the class hierarchy (e.g. to
		 * invoke addDatagramPacketFilter). That's why the property in use
		 * bellow is not defined as a well-known constant and is to be
		 * considered internal and likely to be removed in a future revision.
		 */
		try {
			firePropertyChange(MediaStreamImpl.class.getName() + ".rtpConnector." + (data ? "data" : "control") + "InputStream", null, inputStream);
		} catch (Throwable t) {
	 ...
	 * 
	 * @param connector
	 *            the <tt>StreamConnector</tt> to be used by this instance for
	 ...
	 * 
	 * @param device
	 *            the <tt>MediaDevice</tt> that this stream should use to play
	 *            back and capture media
	 ...
	 * 
	 * @param direction
	 *            the <tt>MediaDirection</tt> in which this <tt>MediaStream</tt>
	 ...
	 * 
	 * @param format
	 *            the <tt>MediaFormat</tt> that this <tt>MediaStream</tt> should
	 *            transmit in
	 * @see MediaStream#setFormat(MediaFormat)
	 */
	@Override
	public void setFormat(RTPMediaFormat format) {
		MediaDeviceSession devSess = getDeviceSession();
		RTPMediaFormat thisFormat = null;

		if (devSess != null) {
			thisFormat = devSess.getFormat();
			if ((thisFormat != null) && thisFormat.equals(format) && thisFormat.advancedAttributesAreEqual(thisFormat.getAdvancedSettings(), format.getAdvancedSettings())) {
				return;
			}
		}
		...
		handleAttributes(format, format.getAdvancedSettings());
		handleAttributes(format, format.getFormatSettings());
	 ...
	 *            "silence" instead of the actual media data that it captures
	 *            from its <tt>MediaDevice</tt>; <tt>false</tt> to transmit
	 *            actual media data captured from the <tt>MediaDevice</tt> of
	 ...
	 * 
	 * @param rtpTranslator
	 *            the <tt>RTPTranslator</tt> which is to forward RTP and RTCP
	 ...
	 * 
	 * @param direction
	 *            a <tt>MediaDirection</tt> value which represents the direction
	 *            of the processing of media to be started. For example,
	 *            {@link MediaDirection#SENDRECV} to start both capture and
	 *            playback of media in this instance or
	 *            {@link MediaDirection#SENDONLY} to only start the capture of
	...
	private void start(MediaDirection direction) {
		...
		if (direction.allowsSending() && ((startedDirection == null) || !startedDirection.allowsSending())) {
			...
			if (logger.isInfoEnabled()) {
				...
				logger.info(mediaType + " remote IP/port: " + stats.getRemoteIPAddress() + "/" + stats.getRemotePort());
			}
		}
	...
	private void startSendStreams() {
		 ...
		 * creation isn't only illogical but also causes the CaptureDevice to be
		 * used.
		 */
		if (!sendStreamsAreCreated)
			createSendStreams();
	 ...
	 * @param direction
	 *            a <tt>MediaDirection</tt> value which represents the direction
	 *            of the processing of media to be stopped. For example,
	 *            {@link MediaDirection#SENDRECV} to stop both capture and
	 *            playback of media in this instance or
	 *            {@link MediaDirection#SENDONLY} to only stop the capture of
	 *            media in this instance
	 */
	 ...
	 * 
	 * @param close
	 *            <tt>true</tt> to close the <tt>SendStream</tt>s that this
	 ...
	 * 
	 * @param sendStreams
	 *            the <tt>SendStream</tt>s to be stopped and optionally closed
	 * @param close
	 *            <tt>true</tt> to close the specified <tt>SendStream</tt>s
	 ...
	 * 
	 * @param ev
	 *            the <tt>ReceiveStreamEvent</tt> which specifies the
	 *            <tt>ReceiveStream</tt> that is the cause of the event and the
	...
	public void update(ReceiveStreamEvent ev) {
		if (ev instanceof NewReceiveStreamEvent) {
			...
			List<ReceiveStream> receiveStreamsToRemove = new ArrayList<ReceiveStream>();
			...
			if (evReceiveStream != null) {
				...
				for (ReceiveStream receiveStream : receiveStreams) {
					if (participant.equals(receiveStream.getParticipant()) && !participant.getStreams().contains(receiveStream) && !rtpManagerReceiveStreams.contains(receiveStream)) {
	...
	/**
	 * Method called back in the RemoteListener to notify listener of all RTP
	 * Remote Events.RemoteEvents are one of ReceiverReportEvent,
	 * SenderReportEvent or RemoteCollisionEvent
	 * 
	 * @param ev
	 *            the event
	 */
	...
	public void update(RemoteEvent ev) {
		if (ev instanceof SenderReportEvent || ev instanceof ReceiverReportEvent) {
			...
			if (report.getFeedbackReports().size() > 0) {
				...
				if ((remoteJitter < minRemoteInterArrivalJitter) || (minRemoteInterArrivalJitter == -1))
			...
			if (logger.isTraceEnabled()) {
				...
				buff.append("Received a ").append(senderReport ? "sender" : "receiver").append(" report for ").append(mediaTypeStr).append(" stream SSRC:").append(getLocalSourceID()).append(" [");
				if (senderReport) {
	 ...
	 * 
	 * @param ev
	 *            the <tt>SendStreamEvent</tt> which specifies the
	 *            <tt>SendStream</tt> that is the cause of the event and the
	 *            very type of the event
	 ...
	 * 
	 * @param ev
	 *            the <tt>SessionEvent</tt> which specifies the source and the
	 *            very type of the event
