...
public class Video4Linux2System extends DeviceSystem {
	...
	/**
	 * Initializes a new <tt>Video4Linux2System</tt> instance which discovers
	 * and registers <tt>CaptureDevice</tt>s which implement the Video for Linux
	 * Two API Specification with JMF.
	 * 
	 * @throws Exception
	 *             if anything goes wrong while discovering and registering
	 *             <tt>CaptureDevice</tt>s which implement the Video for Linux
	 *             Two API Specification with JMF
	 */
	public Video4Linux2System() throws Exception {
		super(RTPMediaType.VIDEO, LOCATOR_PROTOCOL);
	}
	 ...
	 * 
	 * @param deviceName
	 *            the device name of a candidate for a <tt>CaptureDevice</tt>
	 *            implementing the Video for Linux Two API Specification to be
	 *            discovered and registered with JMF
	 * @return <tt>true</tt> if a <tt>CaptureDeviceInfo</tt> for the specified
	 *         <tt>CaptureDevice</tt> has been added to
	 *         <tt>CaptureDeviceManager</tt>; otherwise, <tt>false</tt>
	 * @throws Exception
	 *             if anything goes wrong while discovering and registering the
	 *             specified <tt>CaptureDevice</tt> with JMF
	 */
	...
	protected void doInitialize() throws Exception {
		...
		for (int deviceMinorNumber = 0; deviceMinorNumber <= 63; deviceMinorNumber++) {
			captureDeviceInfoIsAdded = discoverAndRegister(baseDeviceName + deviceMinorNumber) || captureDeviceInfoIsAdded;
		}
	 ...
	 * 
	 * @param deviceName
	 *            name of the device (i.e. /dev/videoX)
	 * @param fd
	 *            file descriptor of the device
	 * @param v4l2_capability
	 *            device V4L2 capability
	 * @return <tt>true</tt> if a <tt>CaptureDeviceInfo</tt> for the specified
	 *         <tt>CaptureDevice</tt> has been added to
	 *         <tt>CaptureDeviceManager</tt>; otherwise, <tt>false</tt>
	 * @throws Exception
	 *             if anything goes wrong while registering the specified
