...
public class PortAudioSystem extends AudioSystem2 {
	 ...
	 * 
	 * @param input
	 *            <tt>true</tt> if the supported sample rate is to be retrieved
	 *            for the PortAudio device with the specified device index as an
	 *            input device or <tt>false</tt> for an output device
	 * @param deviceIndex
	 *            the device index of the PortAudio device for which a supported
	 *            sample rate is to be retrieved
	 * @param channelCount
	 *            number of channel
	 * @param sampleFormat
	 *            sample format
	...
	private static double getSupportedSampleRate(boolean input, int deviceIndex, int channelCount, long sampleFormat) {
		...
		if (deviceInfo != 0) {
			...
			if (defaultSampleRate >= RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE)
				...
				if (streamParameters == 0)
					...
					try {
						...
						boolean formatIsSupported = Pa.IsFormatSupported(inputParameters, outputParameters, Pa.DEFAULT_SAMPLE_RATE);

						supportedSampleRate = formatIsSupported ? Pa.DEFAULT_SAMPLE_RATE : defaultSampleRate;
					} finally {
	 ...
	 * 
	 * @param diagnosticsControl
	 *            the <tt>DiagnosticsControl</tt> to be placed under monitoring
	 *            of its functional health because of a malfunction in its
	 *            procedure/process
	 */
	...
	PortAudioSystem() throws Exception {
		super(LOCATOR_PROTOCOL, FEATURE_DENOISE | FEATURE_ECHO_CANCELLATION | FEATURE_NOTIFY_AND_PLAYBACK_DEVICES | FEATURE_REINITIALIZE);
	}
