...
public class DeviceConfiguration extends PropertyChangeNotifier implements PropertyChangeListener {
	...
	/**
	 * The currently supported resolutions we will show as option and user can
	 * select.
	 */
	...
	public DeviceConfiguration() {
		...
		setAudioSystemIsDisabled = (cfg != null) && cfg.getBoolean(MediaServiceImpl.DISABLE_SET_AUDIO_SYSTEM_PNAME, false);
	...
	private void extractConfiguredAudioCaptureDevices() {
		if (!MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.AUDIO))
			return;
	...
	/**
	 * Returns the configured video capture device with the specified output
	 * format.
	 * 
	 * @param format
	 *            the output format of the video format.
	...
	private void extractConfiguredVideoCaptureDevices() {
		if (!MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.VIDEO))
			return;
		...
		if (NoneAudioSystem.LOCATOR_PROTOCOL.equalsIgnoreCase(videoCaptureDeviceString)) {
			...
			Format[] formats = new Format[] { new AVFrameFormat(), new VideoFormat(Constants.ANDROID_SURFACE), new VideoFormat(VideoFormat.RGB), new VideoFormat(VideoFormat.YUV), new VideoFormat(VideoFormat.H264) };
	 ...
	 * {@link #getAudioCaptureDevice()} and represent acceptable values for
	 * {@link #setAudioCaptureDevice(CaptureDeviceInfo, boolean)}
	 * 
				...
				if (!NoneAudioSystem.LOCATOR_PROTOCOL.equalsIgnoreCase(audioSystem.getLocatorProtocol())) {
					...
					if ((captureDevices == null) || (captureDevices.size() <= 0)) {
						if ((AudioSystem.FEATURE_NOTIFY_AND_PLAYBACK_DEVICES & audioSystem.getFeatures()) == 0) {
							...
							List<CaptureDeviceInfo2> notifyDevices = audioSystem.getDevices(AudioSystem.DataFlow.NOTIFY);

							if ((notifyDevices == null) || (notifyDevices.size() <= 0)) {
								List<CaptureDeviceInfo2> playbackDevices = audioSystem.getDevices(AudioSystem.DataFlow.PLAYBACK);
	 ...
	 * 
	 * @param useCase
	 *            extract video capture devices that correspond to this
	 *            <tt>MediaUseCase</tt>
		...
		for (Format format : formats) {
			...
			if (useCase != MediaUseCase.ANY) {
				for (CaptureDeviceInfo cdi : cdis) {
					MediaUseCase cdiUseCase = DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equalsIgnoreCase(cdi.getLocator().getProtocol()) ? MediaUseCase.DESKTOP : MediaUseCase.CALL;
	 ...
	 * 
	 * @param useCase
	 *            <tt>MediaUseCase</tt> that will determined device we will use
	 ...
	 * 
	 * @param ev
	 *            the <tt>PropertyChangeEvent</tt> to notify this
	 *            <tt>PropertyChangeListener</tt> about and which describes the
	 *            source and other specifics of the notification
	 */
	...
	private void registerCustomRenderers() {
		@SuppressWarnings("unchecked")
		Vector<String> renderers = PlugInManager.getPlugInList(null, null, PlugInManager.RENDERER);
		boolean audioSupportIsDisabled = !MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.AUDIO);
		boolean videoSupportIsDisabled = !MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.VIDEO);
	...
	public void setAudioSystem(AudioSystem audioSystem, boolean save) {
		if (this.audioSystem != audioSystem) {
			...
			if ((this.audioSystem != null) && (this.audioSystem.getFeatures() & DeviceSystem.FEATURE_REINITIALIZE) == 0) {
	 ...
	 * 
	 * @param frameRate
	 *            the frame rate to be set on this <tt>DeviceConfiguration</tt>
	 */
	...
	/**
	 * Sets the device which is to be used by this <tt>DeviceConfiguration</tt>
	 * for video capture.
	 * 
	 * @param device
	 *            a <tt>CaptureDeviceInfo</tt> describing device to be used by
	 *            this <tt>DeviceConfiguration</tt> for video capture.
	 * @param save
	 *            whether we will save this option or not.
	 */
	public void setVideoCaptureDevice(CaptureDeviceInfo device, boolean save) {
		if (videoCaptureDevice != device) {
			...
			if (save) {
				...
				if (cfg != null) {
					cfg.setProperty(PROP_VIDEO_DEVICE, (videoCaptureDevice == null) ? NoneAudioSystem.LOCATOR_PROTOCOL : videoCaptureDevice.getName());
				}
			}
	 ...
	 *            the maximum allowed video bandwidth
	 */
	 ...
	 * 
	 * @param videoSize
	 *            the video size to be set on this <tt>DeviceConfiguration</tt>
	 */
	public void setVideoSize(Dimension videoSize) {
		ConfigurationService cfg = LibJitsi.getConfigurationService();

		if (cfg != null) {
			if ((videoSize.getHeight() != DEFAULT_VIDEO_HEIGHT) || (videoSize.getWidth() != DEFAULT_VIDEO_WIDTH)) {
