...
public class PulseAudioSystem extends AudioSystem {
	 ...
	 * 
	 * @throws Exception
	 *             if anything goes wrong while initializing the new instance
	 */
	...
	private void createContext() {
		...
		try {
			...
			try {
				...
				long context = PA.context_new_with_proplist(PA.threaded_mainloop_get_api(mainloop), null /* PA_PROP_APPLICATION_NAME */, proplist);
	...
	protected synchronized void doInitialize() throws Exception {
		...
		if (!captureDeviceFormats.isEmpty()) {
			captureDevices.add(0, new CaptureDeviceInfo2(NULL_DEV_CAPTURE_DEVICE_INFO_NAME, new MediaLocator(LOCATOR_PROTOCOL + ":"), captureDeviceFormats.toArray(new Format[captureDeviceFormats.size()]), null, null, null));
		}
		if (!playbackDevices.isEmpty()) {
			playbackDevices.add(0, new CaptureDeviceInfo2(NULL_DEV_CAPTURE_DEVICE_INFO_NAME, new MediaLocator(LOCATOR_PROTOCOL + ":"), null, null, null, null));
		}
	 ...
	 * 
	 * @param proplist
	 *            the <tt>pa_proplist</tt> which is to be populated with
	 *            <tt>pa_context</tt>-related properties such as the application
	 *            name and version
	 */
	...
	private void sinkInfoListCallback(long context, long sinkInfo, List<CaptureDeviceInfo2> deviceList, List<Format> formatList) {
		...
		if (description == null)
			description = name;
		deviceList.add(new CaptureDeviceInfo2(description, new MediaLocator(LOCATOR_PROTOCOL + ":" + name), null, null, null, null));
	}
	...
	private void sourceInfoListCallback(long context, long sourceInfo, List<CaptureDeviceInfo2> deviceList, List<Format> formatList) {
		...
		if ((RTPMediaFormatUtils.MAX_AUDIO_CHANNELS != Format.NOT_SPECIFIED) && (RTPMediaFormatUtils.MAX_AUDIO_CHANNELS < channels))
			channels = RTPMediaFormatUtils.MAX_AUDIO_CHANNELS;
		if ((RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE != Format.NOT_SPECIFIED) && (RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE < rate))
			rate = (int) RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE;

		AudioFormat audioFormat = new AudioFormat(AudioFormat.LINEAR, rate, 16, channels, AudioFormat.LITTLE_ENDIAN, AudioFormat.SIGNED, Format.NOT_SPECIFIED /* frameSizeInBits */, Format.NOT_SPECIFIED /* frameRate */, Format.byteArray);
		...
		if (!formatList.isEmpty()) {
			...
			if (description == null)
				description = name;
			deviceList.add(new CaptureDeviceInfo2(description, new MediaLocator(LOCATOR_PROTOCOL + ":" + name), sourceInfoFormatList.toArray(new Format[sourceInfoFormatList.size()]), null, null, null));
		}
	}
	...
	private int waitForContextState(long context, int stateToWaitFor) {
		...
		do {
			...
			if ((PA.CONTEXT_FAILED == state) || (stateToWaitFor == state) || (PA.CONTEXT_TERMINATED == state))
				break;
	...
	public int waitForStreamState(long stream, int stateToWaitFor) {
		...
		do {
			...
			if ((stateToWaitFor == state) || (PA.STREAM_FAILED == state) || (PA.STREAM_TERMINATED == state))
				break;
