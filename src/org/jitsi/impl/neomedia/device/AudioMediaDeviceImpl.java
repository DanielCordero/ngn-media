...
public class AudioMediaDeviceImpl extends MediaDeviceImpl {
	 ...
	 *            the <tt>CaptureDeviceInfo</tt> of the <tt>CaptureDevice</tt>
	 *            to which the new instance is to provide an implementation of
	 *            <tt>MediaDevice</tt>
	 */
	public AudioMediaDeviceImpl(CaptureDeviceInfo captureDeviceInfo) {
		super(captureDeviceInfo, RTPMediaType.AUDIO);
	}
	 ...
	 * 
	 * @param captureDevice
	 *            an exclusive <tt>CaptureDevice</tt> for which sharing is to be
	 *            enabled
	...
	private boolean isLessThanOrEqualToMaxAudioFormat(Format format) {
		if (format instanceof AudioFormat) {
			...
			if ((channels == Format.NOT_SPECIFIED) || (RTPMediaFormatUtils.MAX_AUDIO_CHANNELS == Format.NOT_SPECIFIED) || (channels <= RTPMediaFormatUtils.MAX_AUDIO_CHANNELS)) {
				double sampleRate = audioFormat.getSampleRate();

				if ((sampleRate == Format.NOT_SPECIFIED) || (RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE == Format.NOT_SPECIFIED) || (sampleRate <= RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE)) {
					int sampleSizeInBits = audioFormat.getSampleSizeInBits();

					if ((sampleSizeInBits == Format.NOT_SPECIFIED) || (RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_SIZE_IN_BITS == Format.NOT_SPECIFIED) || (sampleSizeInBits <= RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_SIZE_IN_BITS)) {
						return true;
					}
				}
			}
					...
					for (FormatControl formatControl : formatControls) {
						...
						if (!supportedFormatToSet.matches(format)) {
							...
							if (sampleRate == Format.NOT_SPECIFIED)
								sampleRate = RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_RATE;
							if (sampleSizeInBits == Format.NOT_SPECIFIED) {
								sampleSizeInBits = RTPMediaFormatUtils.MAX_AUDIO_SAMPLE_SIZE_IN_BITS;
								/*
