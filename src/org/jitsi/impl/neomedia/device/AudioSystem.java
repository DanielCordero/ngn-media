...
public abstract class AudioSystem extends DeviceSystem {
	...
	public static AudioSystem[] getAudioSystems() {
		DeviceSystem[] deviceSystems = DeviceSystem.getDeviceSystems(RTPMediaType.AUDIO);
	...
	protected AudioSystem(String locatorProtocol, int features) throws Exception {
		super(RTPMediaType.AUDIO, locatorProtocol, features);
	}
	 ...
	 *         playback on or sound a notification through a device contributed
	 *         by this system
	 */
			...
			if (clazz == null) {
				...
				if (((getFeatures() & FEATURE_NOTIFY_AND_PLAYBACK_DEVICES) != 0) && AbstractAudioRenderer.class.isAssignableFrom(clazz)) {
					...
					try {
						 ...
						 * it will be swallowed and the super's createRenderer()
						 * will be invoked.
						 */
	 ...
	 * 
	 * @param uri
	 *            a valid uri to a sound resource.
	 ...
	 * @param locator
	 *            the <tt>MediaLocator</tt> of the <tt>CaptureDeviceInfo2</tt>
	 *            to be returned
	 * @return a <tt>CaptureDeviceInfo2</tt> which has been contributed by this
	 *         instance, supports the specified <tt>dataFlow</tt> and is
	 *         identified by the specified <tt>locator</tt>
	 */
	 ...
	 * 
	 * @param dataFlow
	 *            the data flow of the devices to retrieve: capture, notify or
	 *            playback
	 ...
	 *            the <tt>InputStream</tt> providing audio media to determine
	 *            the FMJ format of
	 ...
	 *            the (base) <tt>AudioSystem</tt>-specific property name of
	 *            which the associated (full) <tt>ConfigurationService</tt>
	 ...
	 * 
	 * @param dataFlow
	 *            the data flow of the selected device to retrieve: capture,
	 *            notify or playback.
	 ...
	 * 
	 * @param dataFlow
	 *            the data flow of the devices to perform post-initialization on
	 */
	 ...
	 * is to be performed for captured audio.
	 * 
	 * @param automaticGainControl
	 *            <tt>true</tt> if automatic gain control (AGC) is to be
	 *            performed for captured audio; otherwise, <tt>false</tt>
	 */
	 ...
	 *            The list of a kind of devices: capture, notify or playback.
	 */
	 ...
	 * 
	 * @param echoCancel
	 *            <tt>true</tt> if echo cancellation is to be performed for
	 *            captured audio; otherwise, <tt>false</tt>
	 */
