...
public class VideoTranslatorMediaDevice extends AbstractMediaDevice implements MediaDeviceWrapper, VideoListener {
	 ...
	 * 
	 * @param device
	 *            the <tt>MediaDevice</tt> which the new instance is to enable
	 *            to be used in video conferencing implemented with an RTP
	 *            translator
	 */
	 ...
	 *            the <tt>MediaStreamMediaDeviceSession</tt> which has been
	 *            closed
	 */
	...
	protected synchronized DataSource createOutputDataSource() {
		if (deviceSession == null) {
			...
			for (MediaStreamMediaDeviceSession streamDeviceSession : streamDeviceSessions) {
				RTPMediaFormat streamFormat = streamDeviceSession.getFormat();
			...
			if (newDeviceSession instanceof VideoMediaDeviceSession) {
				...
				for (MediaStreamMediaDeviceSession streamDeviceSession : streamDeviceSessions) {
					deviceSession.addRTCPFeedbackMessageCreateListner(streamDeviceSession);
				}
			}
			if (format != null)
	 ...
	 *         <tt>MediaDirection.RECVONLY</tt> if this is a write-only device
	 *         and <tt>MediaDirection.SENDRECV</tt> if this <tt>MediaDevice</tt>
	 *         can both capture and render media
	 ...
	 * 
	 * @param localPreset
	 *            the preset used to set the send format parameters, used for
	 *            video and settings
	 * @param remotePreset
	 ...
	 */
	public List<RTPMediaFormat> getSupportedFormats(QualityPreset localPreset, QualityPreset remotePreset) {
	 ...
	 */
	public List<RTPMediaFormat> getSupportedFormats(QualityPreset localPreset, QualityPreset remotePreset, EncodingConfiguration encodingConfiguration) {
	 ...
	 * {@link #deviceSession} to be in accord with the values of the property of
	 * {@link #streamDeviceSessions}.
	 */
	...
	private class MediaStreamMediaDeviceSession extends VideoMediaDeviceSession {
		 ...
		 * 
		 * @param captureDevice
		 *            the <tt>DataSource</tt> which is to have its media
		 *            rendered by the new instance as the local visual/video
		 *            <tt>Component</tt>
		 ...
		 *         provides the captured (RTP) data to be sent by
		 *         <tt>MediaStream</tt> to <tt>MediaStreamTarget</tt>
		 ...
		 * 
		 * @param oldValue
		 *            the <tt>MediaDirection</tt> which used to be the value of
