...
public abstract class AudioSystem2 extends AudioSystem {
	 ...
	 * 
	 * @param listener
	 *            the <tt>UpdateAvailableDeviceListListener</tt> to be notified
	 *            before and after this <tt>AudioSystem</tt>'s method
	 *            <tt>updateAvailableDeviceList()</tt> is invoked
	 */
	...
	/**
	 * Sorts a specific list of <tt>CaptureDeviceInfo2</tt>s so that the ones
	 * representing USB devices appear at the beginning/top of the specified
	 * list.
	 * 
	 * @param devices
	 *            the list of <tt>CaptureDeviceInfo2</tt>s to be sorted so that
	 *            the ones representing USB devices appear at the beginning/top
	 *            of the list
	 */
	protected static void bubbleUpUsbDevices(List<CaptureDeviceInfo2> devices) {
		if (!devices.isEmpty()) {
			List<CaptureDeviceInfo2> nonUsbDevices = new ArrayList<CaptureDeviceInfo2>(devices.size());
	 ...
	 * 
	 * @param will
	 *            <tt>true</tt> if this <tt>AudioSystem</tt>'s method
	 *            <tt>updateAvailableDeviceList()</tt> will be invoked or
	 *            <tt>false</tt> if it was invoked
	 */
	 ...
	 * <tt>CaptureDeviceInfo2</tt>s so that devices from the same hardware
	 * appear at the same indices in the respective lists. The judgment with
	 * respect to the belonging to the same hardware is based on the names of
	 * the specified <tt>CaptureDeviceInfo2</tt>s. The implementation is
	 * provided as a fallback to stand in for scenarios in which more accurate
	...
	protected static void matchDevicesByName(List<CaptureDeviceInfo2> captureDevices, List<CaptureDeviceInfo2> playbackDevices) {
		...
		Pattern pattern = Pattern.compile("array|headphones|microphone|speakers|\\p{Space}|\\(|\\)", Pattern.CASE_INSENSITIVE);
	 ...
	 * <tt>updateAvailableDeviceList()</tt> to update the devices on the native
	 * side and then {@link #initialize()} to reflect any changes on the Java
	 * side. Invoked by the native side of this <tt>AudioSystem</tt> when it
	 * detects that the list of available devices has changed.
	 * 
	 * @throws Exception
	...
	/**
	 * Waits for all API clients to finish executing a <tt>Pa_OpenStream</tt>
	 * -like function.
	 */
