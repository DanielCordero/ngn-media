	...
	protected void doInitialize() throws Exception {
		...
		try {
			...
			for (int i = 0, count = (devices == null) ? 0 : devices.length; i < count; i++) {
				...
				Format[] formatsArray = formats.toArray(new Format[formats.size()]);

				if (logger.isInfoEnabled()) {
					logger.info("Support for the formats of camera '" + name + "': " + Arrays.toString(formatsArray));
				}

				CaptureDeviceInfo cdi = new CaptureDeviceInfo(name, new MediaLocator(LOCATOR_PROTOCOL + ':' + name), formatsArray);
