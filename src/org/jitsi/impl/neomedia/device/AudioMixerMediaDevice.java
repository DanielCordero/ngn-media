...
public class AudioMixerMediaDevice extends AbstractMediaDevice implements MediaDeviceWrapper {
	...
	/**
	 * The dispatcher that delivers to listeners calculations of the local audio
	 * level.
	 */
	...
	/**
	 * The <tt>Map</tt> where we store audio level dispatchers and the streams
	 ...
	 * 
	 * @param device
	 *            the <tt>AudioMediaDeviceImpl</tt> which the new instance is to
	 ...
	 * 
	 * @param captureDevice
	 *            the <tt>CaptureDevice</tt> to be connected to
	 * @throws IOException
	 *             if anything wrong happens while connecting to the specified
	 *             <tt>captureDevice</tt>
	...
	public void connect(DataSource captureDevice) throws IOException {
		...
		/*
		 * AudioMixingPushBufferDataSource is definitely not a CaptureDevice and
		 * does not need the special connecting defined by AbstractMediaDevice
		 * and MediaDeviceImpl.
		 */
	 ...
	 * 
	 * @param level
	 *            the new/current audio level of the local media stream.
	 */
	 ...
	 * @return the list of <tt>MediaFormat</tt>s supported by this
	 *         <tt>MediaDevice</tt> and enabled in
	 ...
	 * the <tt>AudioMixer</tt> of this <tt>AudioMixerMediaDevice</tt> from which
	 * it reads audio to be mixed.
	 * 
	...
	private class AudioMixerMediaDeviceSession extends MediaDeviceSession {
		 ...
		 */
		private final List<MediaStreamMediaDeviceSession> mediaStreamMediaDeviceSessions = new LinkedList<MediaStreamMediaDeviceSession>();
		...
		void addLocalUserAudioLevelListener(SimpleAudioLevelListener l) {
			...
			synchronized (localUserAudioLevelListenersSyncRoot) {
				...
				if (index != -1) {
					 ...
					 */
					localUserAudioLevelListeners = new ArrayList<SimpleAudioLevelListenerWrapper>(localUserAudioLevelListeners);
		 ...
		 * 
		 * @param mediaStreamMediaDeviceSession
		 *            the <tt>MediaStreamMediaDeviceSession</tt> to be added to
		 *            the mix represented by this instance
		 */
		 ...
		 *            the <tt>DataSource</tt> providing remote audio to be added
		 *            to the mix produced by the associated <tt>MediaDevice</tt>
		 */
		@Override
		 ...
		 * 
		 * @param receiveStream
		 *            the <tt>ReceiveStream</tt> to be added to the list of
		 *            <tt>ReceiveStream</tt>s known to this instance to be
		 ...
		 *            the <tt>VolumeControl</tt> which is to be control the
		 *            volume (level) of the audio (to be) played back by this
		 *            instance
		 */
		 ...
		 * 
		 * @param stream
		 *            the stream that <tt>l</tt> would like to register as an
		 *            audio level listener for.
		 * @param listener
		 *            the listener we'd like to register for notifications from
		 *            <tt>stream</tt>.
		 */
		...
		/**
		 * Removes <tt>l</tt> from the list of listeners that are being notified
		 * of local audio levels.If <tt>l</tt> is not in the list, the method
		 * has no effect.
		 * 
		 * @param l
		 *            the listener we'd like to remove.
		 */
		void removeLocalUserAudioLevelListener(SimpleAudioLevelListener l) {
			synchronized (localUserAudioLevelListenersSyncRoot) {
				...
				if (index != -1) {
					SimpleAudioLevelListenerWrapper wrapper = localUserAudioLevelListeners.get(index);
					...
					if (wrapper.referenceCount > 1)
						 ...
						 * and thus reduce the chances for a deadlock (whic was,
						 * otherwise, observed in practice).
						 */
						localUserAudioLevelListeners = new ArrayList<SimpleAudioLevelListenerWrapper>(localUserAudioLevelListeners);
		 ...
		 * 
		 * @param mediaStreamMediaDeviceSession
		 *            the <tt>MediaStreamMediaDeviceSession</tt> to be removed
		 *            from the mix represented by this instance
		 */
		void removeMediaStreamMediaDeviceSession(MediaStreamMediaDeviceSession mediaStreamMediaDeviceSession) {
			if (mediaStreamMediaDeviceSession != null) {
				synchronized (mediaStreamMediaDeviceSessions) {
					if (mediaStreamMediaDeviceSessions.remove(mediaStreamMediaDeviceSession) && mediaStreamMediaDeviceSessions.isEmpty())
						close();
				}
			}
		}
		 ...
		 * 
		 * @param playbackDataSource
		 *            the <tt>DataSource</tt> providing remote audio to be
		 *            removed from the mix produced by the associated
		 *            <tt>AudioMixer</tt>
		 */
		@Override
		 ...
		 * 
		 * @param receiveStream
		 *            the <tt>ReceiveStream</tt> to be removed from the list of
		 *            <tt>ReceiveStream</tt>s known to this instance to be
		 *            contributing audio to the mix produced by its associated
		 *            <tt>AudioMixer</tt>
		 */
	...
	private static class MediaStreamMediaDeviceSession extends AudioMediaDeviceSession implements PropertyChangeListener {
		 ...
		 * that's the case, we hold on to the listener and register it only when
		 * we get the <tt>ReceiveStream</tt>.
		 */
		 ...
		 * to the mix.
		 * 
		 * @param audioMixerMediaDeviceSession
		 *            the <tt>MediaDeviceSession</tt> of the <tt>AudioMixer</tt>
		 *            with its <tt>MediaDevice</tt> which the new instance is to
		 *            delegate to in order to contribute to the mix
		 */
		 ...
		 * 
		 * @param dataSource
		 *            the <tt>DataSource</tt> to create a new <tt>Player</tt>
		 *            for
		 ...
		 * to the media flows that we are sending out. Note that since this is a
		 * pseudo device we would simply be delegating the call to the
		 * corresponding method of the master mixer device session.
		 * 
		 * @return a <tt>long[]</tt> array of SSRC identifiers that are
		 *         currently contributing to the mixer encapsulated by this
		 *         device session.
		 */
		@Override
		 ...
		 * 
		 * @param playbackDataSource
		 *            the <tt>DataSource</tt> which has been added for playback
		 ...
		 * 
		 * @param playbackDataSource
		 *            the <tt>DataSource</tt> which has been removed from
		 ...
		 * 
		 * @param playbackDataSource
		 *            the <tt>DataSource</tt> which has been updated.
		 ...
		 * 
		 * @param evt
		 *            that <tt>PropertyChangeEvent</tt> whose old and new value
		 ...
		 * 
		 * @param receiveStream
		 *            the <tt>ReceiveStream</tt> which has been added to the
		 *            list of playbacks of <tt>ReceiveStream</tt>s and/or
		 *            <tt>DataSource</tt>s performed by respective
		 *            <tt>Player</tt>s on the <tt>MediaDevice</tt> represented
		 *            by this instance
		 */
		@Override
		protected void receiveStreamAdded(ReceiveStream receiveStream) {
			...
			synchronized (streamAudioLevelListenerLock) {
				if (streamAudioLevelListener != null)
					audioMixerMediaDeviceSession.setStreamAudioLevelListener(receiveStream, streamAudioLevelListener);
			}
		...
		/**
		 * Override it here cause we won't register effects to that stream cause
		 * we already have one.
		 * 
		 * @param processor
		 *            the processor.
		 ...
		 * 
		 * @param listener
		 *            the <tt>SimpleAudioLevelListener</tt> that we are to pass
		 *            to the mixer device session or <tt>null</tt> if we are
		 ...
		 * actual media fed from its <tt>CaptureDevice</tt>. If we are muted we
		 * just remove the local level listener from the session.
		 * 
		 * @param mute
	...
	private static class SimpleAudioLevelListenerWrapper {
		 ...
		 * 
		 * @param l
		 *            the listener we'd like to wrap;
		 */
		 ...
		 * 
		 * @param obj
		 *            the wrapper we'd like to compare to this instance
		 * 
		...
		public boolean equals(Object obj) {
			return (obj instanceof SimpleAudioLevelListenerWrapper) && ((SimpleAudioLevelListenerWrapper) obj).listener == listener;
		}
