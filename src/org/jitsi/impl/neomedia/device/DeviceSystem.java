...
public abstract class DeviceSystem extends PropertyChangeNotifier {
	 ...
	 * 
	 * @param deviceList
	 *            the <tt>List</tt> of <tt>CaptureDeviceInfo</tt> which are to
	 *            be filtered based on the specified <tt>MediaLocator</tt>
	 *            protocol
	 * @param locatorProtocol
	 *            the protocol of the <tt>MediaLocator</tt>s of the
	...
	public static DeviceSystem[] getDeviceSystems(RTPMediaType mediaType) {
	...
	public static void initializeDeviceSystems() {
		...
		if (MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.AUDIO)) {
			...
			initializeDeviceSystems(RTPMediaType.AUDIO);
		}
		...
		if (MediaServiceImpl.isMediaTypeSupportEnabled(RTPMediaType.VIDEO)) {
			...
			initializeDeviceSystems(RTPMediaType.VIDEO);
		}
	}
	 ...
	 * 
	 * @param mediaType
	 *            the <tt>MediaType</tt> of the <tt>DeviceSystem</tt>s to be
	 *            initialized
	 */
	public static void initializeDeviceSystems(RTPMediaType mediaType) {
		/*
		...
		switch (mediaType) {
		case AUDIO:
			classNames = new String[] { OSUtils.IS_ANDROID ? ".AudioRecordSystem" : null, OSUtils.IS_ANDROID ? ".OpenSLESSystem" : null, (OSUtils.IS_LINUX || OSUtils.IS_FREEBSD) ? ".PulseAudioSystem" : null, OSUtils.IS_WINDOWS ? ".WASAPISystem" : null, OSUtils.IS_ANDROID ? null : ".PortAudioSystem", ".AudioSilenceSystem", ".NoneAudioSystem" };
			break;
		case VIDEO:
			classNames = new String[] { OSUtils.IS_ANDROID ? ".MediaRecorderSystem" : null, OSUtils.IS_ANDROID ? ".AndroidCameraSystem" : null, (OSUtils.IS_LINUX || OSUtils.IS_FREEBSD) ? ".Video4Linux2System" : null, OSUtils.IS_WINDOWS ? ".DirectShowSystem" : null, ".ImgStreamingSystem" };
			break;
		default:
	 ...
	 * 
	 * @param classNames
	 *            the names of the classes which extend the
	 *            <tt>DeviceSystem</tt> class and instances of which are to be
	 *            initialized
	 */
	 ...
	 * 
	 * @param deviceSystem
	 *            the <tt>DeviceSystem</tt> to invoke <tt>initialize()</tt> on
	 * @throws Exception
	 ...
	 *            the <tt>DeviceSystem</tt> to invoke <tt>initialize()</tt> on
	 * @param asynchronous
	 *            <tt>true</tt> if the invocation is to be performed in a
	 *            separate thread and the method is to return immediately
	 *            without waiting for the invocation to return; otherwise,
	 *            <tt>false</tt>
	 * @throws Exception
	 *             if an error occurs during the initialization of
	 *             <tt>initialize()</tt> on the specified <tt>deviceSystem</tt>
	...
	protected DeviceSystem(RTPMediaType mediaType, String locatorProtocol) throws Exception {
		this(mediaType, locatorProtocol, 0);
	}

	protected DeviceSystem(RTPMediaType mediaType, String locatorProtocol, int features) throws Exception {
		if (mediaType == null)
	 ...
	 *             instance. The initialization of this instance will be
	 *             completed with a subsequent call to <tt>postInitialize()</tt>
	 *             regardless of any <tt>Exception</tt> thrown by
	 *             <tt>doInitialize()</tt>.
	 */
	 ...
	 *         <tt>FEATURE_XXX</tt> constants defined by the
	...
	public final RTPMediaType getMediaType() {
		return mediaType;
	}
	 ...
	 *         interface to render media on a playback or notification device
	 *         associated with this <tt>DeviceSystem</tt> or <tt>null</tt> if no
	 *         <tt>Renderer</tt> instance is to be created by the
	 *         <tt>DeviceSystem</tt> implementation or
	...
	protected void postInitialize() throws Exception {
		try {
			...
			if (format != null) {
				...
				List<CaptureDeviceInfo> postInitializeDevices = new ArrayList<CaptureDeviceInfo>(cdis);
				...
				int postInitializeDeviceCount = (postInitializeDevices == null) ? 0 : postInitializeDevices.size();
