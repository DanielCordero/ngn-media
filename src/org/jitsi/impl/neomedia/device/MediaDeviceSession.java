...
public class MediaDeviceSession extends PropertyChangeNotifier {
	 ...
	 * accesses to the field are to be synchronized using {@link #playbacksLock}
	 * .
	 */
	...
	private final ControllerListener playerControllerListener = new ControllerListener() {
		 ...
		 * <tt>Controller</tt> which it is registered with has generated an
		 * event.
		 * 
		 * @param ev
	 ...
	 * <tt>ControllerClosedEvent</tt> at an unexpected time in its execution. A
	 * value of <tt>false</tt> does not mean that <tt>processor</tt> exists or
	 * that it is not closed, it just means that if <tt>processor</tt> failed to
	 * be initialized or it received a <tt>ControllerClosedEvent</tt>, it was at
	 * an expected time of its execution and that the fact in question was
	...
	/**
	 * Whether output size has changed after latest processor config. Used for
	 * video streams.
	 */
	 ...
	 * 
	 * @param disposePlayerOnClose
	 *            <tt>true</tt> to have this instance dispose of its associated
	 *            player upon closing; otherwise, <tt>false</tt>
	 */
	 ...
	 * 
	 * @param sourceFormat
	 *            the original format to check the size of
	 ...
	 * 
	 * @param device
	 *            the <tt>MediaDevice</tt> to be checked for suitability to
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> to create a new <tt>Player</tt> for
			...
			if (exception != null)
				...
				if (processorControllerListener == null)
					processorControllerListener = new ControllerListener() {
						 ...
						 * 
						 * @param event
						 *            the <tt>ControllerEvent</tt> specifying
						 *            the <tt>Controller</tt> which is the
						 *            source of the event and the very type of
						 *            the event
						 * @see ControllerListener#controllerUpdate(ControllerEvent)
						 */
	 ...
	 * 
	 * @param processor
	 *            the <tt>Processor</tt> of captured media to be sent to the
	 *            remote peer which is to have its <tt>contentDescriptor</tt>
	 *            set to the returned <tt>ContentDescriptor</tt>
	 * @return a <tt>ContentDescriptor</tt> to be set on the specified
	 *         <tt>processor</tt> of captured media to be sent to the remote
	 *         peer
	 */
	 ...
	 * specific <tt>Player</tt> in order to play back the media represented by a
	 * specific <tt>TrackControl</tt>. Allows extenders to override and,
	 * optionally, perform additional configuration of the returned
	 * <tt>Renderer</tt>.
	 * 
	 * @param player
	 *            the <tt>Player</tt> which is to utilize the
	 *            initialized/returned <tt>Renderer</tt>
	 * @param trackControl
	 *            the <tt>TrackControl</tt> which represents the media to be
	 *            played back (and, technically, on which the
	 *            initialized/returned <tt>Renderer</tt> is to be set)
	 ...
	 * 
	 * @param formats
	 *            the array of <tt>Format</tt>s to be searched for a match to
	 *            the specified <tt>format</tt>
	 * @param format
	 *            the <tt>Format</tt> to search for a match in the specified
	 *            <tt>formats</tt>
	 * @return the first element of <tt>formats</tt> which matches
	 *         <tt>format</tt> i.e. is of the same encoding
	 */
	private static Format findFirstMatchingFormat(Format[] formats, RTPMediaFormat format) {
		double formatSampleRate = (format.getFormat() instanceof AudioFormat) ? ((AudioFormat) format.getFormat()).getSampleRate() : Format.NOT_SPECIFIED;

		for (Format match : formats) {
			if (match.getEncoding().equals(format.getFormat().getEncoding())) {
				/*
				...
				else if (match instanceof VideoFormat && match.getRtpFormat() != null) {
					if (!match.getRtpFormat().formatParametersMatch(format.getFormatSettings()))
						continue;
				}
...
//				}
	 ...
	 *         this instance fails to create <tt>captureDevice</tt> or to
	 *         connect to it
	 */
	...
	public Format getProcessorFormat() {
		...
		if ((processor != null) && (this.processor == processor) && !processorIsPrematurelyClosed) {
			...
			for (TrackControl trackControl : processor.getTrackControls()) {
				...
				RTPMediaType type = (jmfFormat instanceof VideoFormat) ? RTPMediaType.VIDEO : RTPMediaType.AUDIO;
	 ...
	 *         this instance as reported by {@link MediaDevice#getMediaType()}
	 *         of its associated <tt>MediaDevice</tt>
	 */
	private RTPMediaType getMediaType() {
	...
	public DataSource getOutputDataSource() {
		...
		if ((processor == null) || ((processor.getState() < Processor.Realized) && !waitForState(processor, Processor.Realized)))
	 ...
	 * 
	 * @param dataSource
	 *            the <tt>DataSource</tt> to get the information related to the
	 *            playback of
	 * @return the information related to the playback of the specified
	 *         <tt>DataSource</tt> on the <tt>MediaDevice</tt> represented by
	 *         this <tt>MediaDeviceSession</tt>
	 */
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> to get the information related to
	 *            the playback of
	 * @return the information related to the playback of the specified
	 *         <tt>ReceiveStream</tt> on the <tt>MediaDevice</tt> represented by
	 *         this <tt>MediaDeviceSession</tt>
	 */
	 ...
	 *         <tt>MediaDevice</tt> of this instance into the format of this
	 *         instance
	 */
	 ...
	 * streams from. In this case (i.e. the case of a device session handling a
	 * single remote party) we would rarely (if ever) have more than a single
	 ...
	 *         capable of capturing media from its associated
	 *         <tt>MediaDevice</tt>
	 */
	public List<RTPMediaFormat> getSupportedFormats() {
		Processor processor = getProcessor();
		...
		List<RTPMediaFormat> supportedMediaFormats = new ArrayList<RTPMediaFormat>(supportedFormats.size());

		for (Format format : supportedFormats)
			supportedMediaFormats.add(RTPMediaFormat.createInstance(format));
		return supportedMediaFormats;
	}

	/**
	 * Determines whether this <tt>MediaDeviceSession</tt> is set to output
	 * "silence" instead of the actual media fed from its <tt>CaptureDevice</tt>
	 * .
	 * 
	 ...
	 * 
	 * @param playbackDataSource
	 *            the <tt>DataSource</tt> which has been added for playback on
	 *            the represented <tt>MediaDevice</tt>
	 */
	 ...
	 * 
	 * @param playbackDataSource
	 *            the <tt>DataSource</tt> which has been removed from playback
	 *            on the represented <tt>MediaDevice</tt>
	 */
	...
	protected void processorControllerUpdate(ControllerEvent ev) {
		if (ev instanceof ConfigureCompleteEvent) {
			...
			for (FormatParametersAwareCodec fpac : getAllTrackControls(FormatParametersAwareCodec.class, processor)) {
				Map<String, String> formatParameters = format == null ? null : format.getFormatSettings();
				if (formatParameters != null)
					fpac.setFormatParameters(formatParameters);
			}
			for (AdvancedAttributesAwareCodec aaac : getAllTrackControls(AdvancedAttributesAwareCodec.class, processor)) {
				Map<String, String> advanceAttrs = format == null ? null : format.getAdvancedSettings();
	 ...
	 * 
	 * @param ssrc
	 *            the SSRC identifier that we'd like to remove from the array of
	 *            <tt>ssrc</tt> identifiers stored by this session.
	 */
	protected void removeSSRC(long ssrc) {
		...
		// simply need to empty it as the only element is the one we are
		// removing
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> which has been added to the list of
	 *            playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> which has been removed from the
	 *            list of playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 *            on the <tt>MediaDevice</tt> represented by this instance
	 */
	...
	protected void setCaptureDeviceFormat(DataSource captureDevice, RTPMediaFormat mediaFormat) {
				...
				if (formatControl != null) {
					...
					if ((supportedFormats != null) && (supportedFormats.length != 0)) {
						if (sampleRate != Format.NOT_SPECIFIED) {
							...
							if ((AudioFormat.G722.equalsIgnoreCase(encoding) || AudioFormat.G722_RTP.equalsIgnoreCase(encoding)) && (sampleRate == 8000)) {
								sampleRate = 16000;
							}
						}
						...
						for (int i = 0; i < supportedFormats.length; i++) {
							...
							if (sf instanceof AudioFormat) {
								...
								if ((Format.NOT_SPECIFIED != channels) && (saf.getChannels() != channels))
									continue;
								if ((Format.NOT_SPECIFIED != sampleRate) && (saf.getSampleRate() != sampleRate))
									continue;
	 ...
	 *            the <tt>ContentDescriptor</tt> which specifies the content
	 *            type in which this <tt>MediaDeviceSession</tt> is to output
	 ...
	 * 
	 * @param format
	 *            the <tt>MediaFormat</tt> in which this
	 *            <tt>MediaDeviceSession</tt> is to output the media captured by
	 *            its <tt>MediaDevice</tt>
	 */
	 ...
	 * 
	 * @param processor
	 *            the <tt>Processor</tt> to set the output
	 *            <tt>MediaFormatImpl</tt> of
	 * @param mediaFormat
	 *            the <tt>MediaFormatImpl</tt> to set on <tt>processor</tt>
	 */
	protected void setProcessorFormat(Processor processor, RTPMediaFormat mediaFormat) {
		...
		for (int trackIndex = 0; trackIndex < trackControls.length; trackIndex++) {
			...
			case AUDIO:
				if (supportedFormats[0] instanceof AudioFormat) {
					supportedFormat = findFirstMatchingFormat(supportedFormats, mediaFormat);
					...
					if (supportedFormat == null)
						supportedFormat = mediaFormat.getFormat();
				}
				break;
			case VIDEO:
	 ...
	 * 
	 * @param trackControl
	 *            the <tt>TrackControl</tt> to set the JMF <tt>Format</tt> of
	 * @param mediaFormat
	 *            the <tt>MediaFormatImpl</tt> to be set on the specified
	 *            <tt>TrackControl</tt>. Though <tt>mediaFormat</tt>
	 *            encapsulates a JMF <tt>Format</tt>, <tt>format</tt> is to be
	 *            set on the specified <tt>trackControl</tt> because it may be
	 *            more specific. In any case, the two JMF <tt>Format</tt>s
	 *            match. The <tt>MediaFormatImpl</tt> is provided anyway because
	 *            it carries additional information such as format parameters.
	 * @param format
	 *            the JMF <tt>Format</tt> to be set on the specified
	 *            <tt>TrackControl</tt>. Though <tt>mediaFormat</tt>
	 *            encapsulates a JMF <tt>Format</tt>, the specified
	 *            <tt>format</tt> is to be set on the specified
	 *            <tt>trackControl</tt> because it may be more specific than the
	 *            JMF <tt>Format</tt> of the <tt>mediaFormat</tt>
	 * @return the JMF <tt>Format</tt> set on <tt>TrackControl</tt> after the
	 *         attempt to set the specified <tt>format</tt> or <tt>null</tt> if
	 *         the specified <tt>format</tt> was found to be incompatible with
	 *         <tt>trackControl</tt>
	 */
	protected Format setProcessorFormat(TrackControl trackControl, RTPMediaFormat mediaFormat, Format format) {
	 ...
	 * 
	 * @param mute
	 *            <tt>true</tt> to set this <tt>MediaDeviceSession</tt> to
	 *            output "silence" instead of the actual media fed from its
	 *            <tt>CaptureDevice</tt>; otherwise, <tt>false</tt>
	 */
	...
	public void addDTMF(DTMFInbandTone tone) {
		InbandDTMFDataSource inbandDTMFDataSource = AbstractControls.queryInterface(captureDevice, InbandDTMFDataSource.class);
	 ...
	 * 
	 * @param playbackDataSource
	 *            the <tt>DataSource</tt> which to be added to the list of
	 *            playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 *            on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 * 
	 * @param playbackDataSource
	 *            the <tt>DataSource</tt> which to be removed from the list of
	 *            playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 *            on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 * 
	 * @param processor
	 *            the JMF <tt>Processor</tt> which is to transcode
	 *            {@link #captureDevice} into the format of this instance
	 */
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> which to be added to the list of
	 *            playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 *            on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> which to be removed from the list
	 *            of playbacks of <tt>ReceiveStream</tt>s and/or
	 *            <tt>DataSource</tt>s performed by respective <tt>Player</tt>s
	 *            on the <tt>MediaDevice</tt> represented by this instance
	 */
	 ...
	 * 
	 * @param newSsrcList
	 *            that SSRC array that we'd like to replace the existing SSRC
	 *            list with.
	 */
	 ...
	 * @param newValue
	 *            the <tt>MediaDirection</tt> which is the value of the
	 *            <tt>startedDirection</tt> property of this instance
	 */
	 ...
	 * 
	 * @param processor
	 *            the JMF <tt>Processor</tt> to wait on
	 * @param state
	...
	private static class Playback {
		 ...
		 * 
		 * @param dataSource
		 *            the <tt>DataSource</tt> the information related to the
		 *            playback of which is to be represented by the new instance
		 */
		 ...
		 * 
		 * @param receiveStream
		 *            the <tt>ReceiveStream</tt> the information related to the
		 *            playback of which is to be represented by the new instance
		 */
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> to use
	 * 
	...
	public TranscodingDataSource getTranscodingDataSource(ReceiveStream receiveStream) {
		...
		if (device instanceof AudioMixerMediaDevice) {
			transcodingDataSource = ((AudioMixerMediaDevice) device).getTranscodingDataSource(receiveStream.getDataSource());
		}
	 ...
	 * Returns a <tt>Set</tt> of instances of class <tt>controlType</tt>, always
	 * non-null.
	 * 
	 * @param controlType
	 ...
	 * 
	 * @param receiveStream
	 *            the <tt>ReceiveStream</tt> whose <tt>Processor</tt>'s
	 ...
	 *            the name of the class to search for.
	 * @param processor
	 *            the <tt>Processor</tt> whose <tt>TrackControls</tt>s will be
	 *            searched.
	 * @return A non-null <tt>Set</tt> of all <tt>controlType</tt>s found.
	 */
