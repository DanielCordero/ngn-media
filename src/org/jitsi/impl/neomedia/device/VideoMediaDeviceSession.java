...
public class VideoMediaDeviceSession extends MediaDeviceSession implements RTCPFeedbackMessageCreateListener {
	 ...
	 * 
	 * @param player
	 *            the <tt>Player</tt> to get the visual <tt>Component</tt> of if
	 *            it has one
	 * @return the visual <tt>Component</tt> of the specified <tt>Player</tt> if
	 *         it has one; <tt>null</tt> if the specified <tt>Player</tt> does
	 *         not have a visual <tt>Component</tt> or the <tt>Player</tt> is
	 *         unrealized
	 */
	 ...
	 * <tt>VideoMediaDeviceSession</tt> and provided to {@link #keyFrameControl}
	 * .
	 */
	 ...
	 * 
	 * It is used to specify a different size (generally lesser ones) than the
	 * capture device provides. Typically one usage can be in desktop
	 * streaming/sharing session when sender desktop is bigger than remote ones.
	 */
	private Dimension outputSize;

	/**
	 * The <tt>SwScale</tt> inserted into the codec chain of the <tt>Player</tt>
	 * rendering the media received from the remote peer and enabling the
	 * explicit setting of the video size.
	 */
	 ...
	 * 
	 * @param device
	 *            the video <tt>MediaDevice</tt> the use of which by a
	 ...
	 * 
	 * @param listener
	 *            the <tt>VideoListener</tt> to be notified when visual/video
	 *            <tt>Component</tt>s are being added or removed in this
	 *            instance
	 */
	...
	protected void checkDevice(AbstractMediaDevice device) {
		if (!RTPMediaType.VIDEO.equals(device.getMediaType()))
	 ...
	 *            which is the source of the event and the very type of the
	 *            event
	 * @param hflip
	 *            <tt>true</tt> if the image displayed in the local visual
			...
			if ((trackControls != null) && (trackControls.length != 0)) {
				try {
					for (TrackControl trackControl : trackControls) {
						trackControl.setCodecChain(hflip ? new Codec[] { new HFlip(), new SwScale() } : new Codec[] { new SwScale() });
						break;
					}
	...
	protected DataSource createCaptureDevice() {
		...
		if (captureDevice != null) {
			...
			if (DeviceSystem.LOCATOR_PROTOCOL_IMGSTREAMING.equals(protocol)) {
				...
				if (outputSize != null && videoSize.height > outputSize.height && videoSize.width > outputSize.width)
	 ...
	 * 
	 * @param captureDevice
	 *            the <tt>DataSource</tt> which is to have its media rendered by
	 *            the new instance as the local visual/video <tt>Component</tt>
	 * @return a new <tt>Player</tt> instance which is to provide the local
	 *         visual/video <tt>Component</tt>
	 */
				...
				if (localPlayer != null) {
					...
					localPlayer.addControllerListener(new ControllerListener() {
						public void controllerUpdate(ControllerEvent ev) {
							controllerUpdateForCreateLocalVisualComponent(ev, hflip);
						}
					});
	 ...
	 *         from the local peer to the remote peer if it was immediately
	 *         created or <tt>null</tt> if it was not immediately created and it
	 *         is to be delivered to the currently registered
	 *         <tt>VideoListener</tt>s in a <tt>VideoEvent</tt> with type
	 *         {@link VideoEvent#VIDEO_ADDED} and origin
	 ...
	 * 
	 * @param component
	 *            the local visual <tt>Component</tt> of the local peer to
	 *            dispose of
	 */
	 ...
	 * 
	 * @param type
	 *            the type of change as defined by <tt>VideoEvent</tt> in the
	 *            availability of the specified visual <tt>Component</tt>
	 *            depicting video
	 * @param visualComponent
	 *            the visual <tt>Component</tt> depicting video which has been
	 *            added or removed in this instance
	 * @param origin
	 *            {@link VideoEvent#LOCAL} if the origin of the video is local
	 *            (e.g. it is being locally captured); {@link VideoEvent#REMOTE}
	 *            if the origin of the video is remote (e.g. a remote peer is
	 *            streaming it)
	 * @param wait
	 *            <tt>true</tt> if the call is to wait till the specified
	 *            <tt>VideoEvent</tt> has been delivered to the
	 *            <tt>VideoListener</tt>s; otherwise, <tt>false</tt>
	 ...
	 *         video is actually being streamed from the local peer to the
	 *         remote peer; otherwise, <tt>null</tt>
	 */
	 ...
	 *         remote peer or <tt>null</tt> if we are not receiving any video or
	 *         the FMJ <tt>Format</tt> of the video we are receiving from the
	 *         remote peer cannot be determined
	 */
	 ...
	 *            the <tt>KeyFrameControl.KeyFrameRequester</tt> on which the
	 *            method is invoked
	...
	private boolean keyFrameRequesterRequestKeyFrame(KeyFrameControl.KeyFrameRequester keyFrameRequester) {
		...
		if (VideoMediaDeviceSession.this.useRTCPFeedbackPLI) {
			try {
				new RTCPFeedbackMessagePacket(RTCPFeedbackMessageEvent.FMT_PLI, RTCPFeedbackMessageEvent.PT_PS, localSSRC, remoteSSRC).writeTo(rtpConnector.getControlOutputStream());
				requested = true;
	...
	public void onRTCPFeedbackMessageCreate(RTCPFeedbackMessageListener rtcpFeedbackMessageListener) {
		if (rtpConnector != null) {
			try {
				((ControlTransformInputStream) rtpConnector.getControlInputStream()).addRTCPFeedbackMessageListener(rtcpFeedbackMessageListener);
			} catch (IOException ioe) {
	...
	protected void playerControllerUpdate(ControllerEvent ev) {
		 ...
		 */
		if ((ev instanceof SizeChangeEvent) && ((playerScaler == null) || (playerScaler.getOutputSize() == null))) {
			SizeChangeEvent scev = (SizeChangeEvent) ev;
	 ...
	 *            the <tt>Player</tt> which is the source of the event
	 * @param origin
	 *            {@link VideoEvent#LOCAL} or {@link VideoEvent#REMOTE} which
	 *            specifies the origin of the visual <tt>Component</tt>
	 *            displaying video which is concerned
	 * @param width
	 *            the width reported in the event
	 * @param height
	 *            the height reported in the event
	 * @see SizeChangeEvent
	 */
	 ...
	 * 
	 * @param player
	 *            the <tt>Player</tt> rendering remote content the visual
	 *            <tt>Component</tt> of which has been resized
	 * @param ev
	 *            a <tt>ComponentEvent</tt> which specifies the resized
	 *            <tt>Component</tt>
	 */
	private void playerVisualComponentResized(Processor player, ComponentEvent ev) {
		...
		if ((outputWidth < SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH) || (outputHeight < SwScale.MIN_SWS_SCALE_HEIGHT_OR_WIDTH))
			return;
	 ...
	 *            visual/video <tt>Component</tt>s are being added or removed in
	 *            this instance
	 */
	...
	/**
	 * Sets the <tt>RTPConnector</tt> that will be used to initialize some codec
	 * for RTCP feedback.
	 * 
	 * @param rtpConnector
	 *            the RTP connector
	...
	public void setFormat(RTPMediaFormat format) {
		if (format.getFormat() instanceof VideoFormat && ((VideoFormat) format.getFormat()).getFrameRate() != -1) {
			FrameRateControl frameRateControl = (FrameRateControl) getCaptureDevice().getControl(FrameRateControl.class.getName());

			if (frameRateControl != null) {
				float frameRate = ((VideoFormat) format.getFormat()).getFrameRate();
	 ...
	 * <tt>VideoMediaDeviceSession</tt> as a means of control over its key
	 * frame-related logic.
	 * 
	 * @param keyFrameControl
	 *            the <tt>KeyFrameControl</tt> to be used by this
	 *            <tt>VideoMediaDeviceSession</tt> as a means of control over
	 ...
	 *         attempt to set the specified <tt>mediaFormat</tt> or
	 *         <tt>null</tt> if the specified <tt>format</tt> was found to be
	 *         incompatible with <tt>trackControl</tt>
	...
	protected Format setProcessorFormat(TrackControl trackControl, RTPMediaFormat mediaFormat, Format format) {
		...
		if (!OSUtils.IS_ANDROID && "h264/rtp".equalsIgnoreCase(format.getEncoding())) {
			...
			// packetization-mode
			{
				Map<String, String> formatParameters = mediaFormat.getFormatSettings();
				String packetizationMode = (formatParameters == null) ? null : formatParameters.get(RTPMediaFormat.H264_PACKETIZATION_MODE_FMTP);
			...
			{
				encoder.setAdditionalCodecSettings(mediaFormat.getCodecSettings());
			}
		...
		if (codecCount != 0) {
			/*
			 * Add our custom SwScale and possibly RTCP aware codec to the codec
			 * chain so that it will be used instead of default.
			 */
			try {
	 ...
	 * 
	 * @param useRTCPFeedbackPLI
	 *            <tt>true</tt> to use PLI; otherwise, <tt>false</tt>
	 */
	 ...
	 * <tt>Component</tt>.
	 * 
	 * @return the <tt>Player</tt> instance which provides the local
	 *         visual/video <tt>Component</tt>.
	 */
	...
	/**
	 * Extends <tt>SwScale</tt> in order to provide scaling with high quality to
	 * a specific <tt>Player</tt> of remote video.
	 */
	private class PlayerScaler extends SwScale {
		 ...
		 * 
		 * @param player
		 *            the <tt>Player</tt> of remote video into the codec chain
		 *            of which the new instance is to be set
		 */
		 ...
		 * <tt>SizeChangeVideoEvent</tt> because <tt>Player</tt> is unable to do
		 * it when this <tt>SwScale</tt> is scaling to a specific
		 * <tt>outputSize</tt>.
		 * 
		 * @param input
		 *            input buffer
		...
		public int process(Buffer input, Buffer output) {
			...
			if (result == BUFFER_PROCESSED_OK) {
				...
				if (inputFormat != null) {
					...
					if ((size != null) && (size.height >= MIN_SWS_SCALE_HEIGHT_OR_WIDTH) && (size.width >= MIN_SWS_SCALE_HEIGHT_OR_WIDTH) && !size.equals(lastSize)) {
						lastSize = size;
