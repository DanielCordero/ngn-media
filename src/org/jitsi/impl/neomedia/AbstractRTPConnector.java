...
public abstract class AbstractRTPConnector implements RTPConnector {
	/**
	 * The <tt>Logger</tt> used by the <tt>AbstractRTPConnector</tt> class and
	 * its instances to print debug information.
	 */
	private static final Logger logger = Logger.getLogger(AbstractRTPConnector.class);
	 ...
	 * 
	 * @param connector
	 *            the pair of datagram sockets for RTP and RTCP traffic the new
	 *            instance is to use
	 */
	 ...
	 * @throws IOException
	 *             if an error occurs during the creation of the RTCP packet
	 *             input stream
	 */
	 ...
	 * @throws IOException
	 *             if an error occurs during the creation of the RTP packet
	 *             input stream
	 */
	 ...
	 * 
	 * @param create
	 *            <tt>true</tt> to create the <tt>PushSourceStream</tt> which
	 *            gives access to the RTCP data received from the remote targets
	 *            if it does not exist yet; otherwise, <tt>false</tt>
	 * @return the <tt>PushBufferStream</tt> which gives access to the RTCP data
	 *         received from the remote targets; <tt>null</tt> if it does not
	 *         exist yet and <tt>create</tt> is <tt>false</tt>
	 * @throws IOException
	 *             if creating the <tt>PushSourceStream</tt> fails
	 */
	 ...
	 * 
	 * @param create
	 *            <tt>true</tt> to create the <tt>OutputDataStream</tt> which is
	 *            to be used to write RTCP data to be sent to the remote targets
	 *            if it does not exist yet; otherwise, <tt>false</tt>
	 * @return the <tt>OutputDataStream</tt> which is used to write RTCP data to
	 *         be sent to the remote targets; <tt>null</tt> if it does not exist
	 *         yet and <tt>create</tt> is <tt>false</tt>
	 * @throws IOException
	 *             if creating the <tt>OutputDataStream</tt> fails
	 */
	 ...
	 * @param create
	 *            <tt>true</tt> to create the <tt>PushSourceStream</tt> which
	 *            gives access to the RTP data received from the remote targets
	 *            if it does not exist yet; otherwise, <tt>false</tt>
	 ...
	 * @param create
	 *            <tt>true</tt> to create the <tt>OutputDataStream</tt> which is
	 *            to be used to write RTP data to be sent to the remote targets
	 *            if it does not exist yet; otherwise, <tt>false</tt>
	 ...
	 * {@link RTPConnector#getReceiveBufferSize()} that always returns
	 * <tt>-1</tt>.
	 */
	...
	/**
	 * Provides a dummy implementation to
	 * {@link RTPConnector#getRTCPBandwidthFraction()} that always returns
	 * <tt>-1</tt>.
	 */
	...
	/**
	 * Provides a dummy implementation to
	 * {@link RTPConnector#getRTCPSenderBandwidthFraction()} that always returns
	 * <tt>-1</tt>.
	 */
	...
	/**
	 * Provides a dummy implementation to
	 * {@link RTPConnector#getSendBufferSize()} that always returns <tt>-1</tt>.
	 */
	 ...
	 * 
	 * @param target
	 *            Destination target to be removed
	 */
	...
	/**
	 * Remove all stream targets. After this operation is done. There will be no
	 * targets receiving data, so no data will be sent.
	 */
	...
	/**
	 * Provides a dummy implementation to
	 * {@link RTPConnector#setReceiveBufferSize(int)}.
	 * 
	 * @param size
	 *            ignored.
	 */
	...
	/**
	 * Provides a dummy implementation to
	 * {@link RTPConnector#setSendBufferSize(int)}.
	 * 
	 * @param size
	 *            ignored.
	 */
	 ...
	 *            <tt>AbstractRTPConnector</tt>. The input stream will be
	 *            enabled or disabled depending on whether <tt>direction</tt>
	 *            allows receiving. The output stream will be enabled or
	 *            disabled depending on whether <tt>direction</tt> allows
	 *            sending.
	 */
	public void setDirection(MediaDirection direction) {
		...
		try {
			...
			logger.error("Failed to " + (receive ? "enable" : "disable") + " data input stream.");
		}
		...
		try {
			...
			logger.error("Failed to " + (send ? "enable" : "disable") + " data output stream.");
