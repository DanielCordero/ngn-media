...
public interface Version extends Comparable<Version> {
	...
	/**
	 * The name of the <tt>System</tt> property the value of which is equal to
	 * the value of {@link #toString()}. Expected to be set by implementers of
	 * the <tt>VersionService</tt> and <tt>Version</tt> interfaces.
	 */
	public static final String PNAME_APPLICATION_VERSION = "sip-communicator.version";

	/**
	 * Returns the version major of the current Jitsi version. In an example
	...
	/**
	 * Returns the version minor of the current Jitsi version. In an example
	 * 2.3.1 version string 3 is the version minor. The version minor number
	 * changes after adding enhancements and possibly new features to a given
	 * Jitsi version.
	 * 
	...
	/**
	 * Indicates if this Jitsi version corresponds to a nightly build of a
	 * repository snapshot or to an official Jitsi release.
	 * 
	...
	/**
	 * Returns the version prerelease ID of the current Jitsi version and null
	 * if this version is not a prerelease. Version pre-release id-s exist only
	 * for pre-releaseversions and are <tt>null<tt/> otherwise. Note
	 ...
	 * respectively an earlier, same, or later version as the one indicated by
	 * the <tt>version</tt> parameter.
	 * 
	 * @param version
	 *            the <tt>Version</tt> instance that we'd like to compare to
	 *            this one.
	 * 
	 * @return a negative integer, zero, or a positive integer as this object
	 *         represents a version that is earlier, same, or more recent than
	 *         the one referenced by the <tt>version</tt> parameter.
	 */
	public int compareTo(Version version);

	/**
	 * Compares the <tt>version</tt> parameter to this version and returns true
	 * if and only if both reference the same Jitsi version and false otherwise.
	 * 
	 * @param version
	 *            the version instance that we'd like to compare with this one.
	 * @return true if and only the version param references the same Jitsi
	 *         version as this Version instance and false otherwise.
	 */
	 ...
	 * like to obtain the version of Jitsi so that you could display it (e.g. in
	 * a Help->About dialog) then all you need is calling this method.
	 * 
	 * @return a major.minor[.build] String containing the complete Jitsi
	 *         version.
	 */
