...
public abstract class AbstractSCAudioClip implements SCAudioClip {
	 ...
	 *         this audio. If negative, this audio will not be played in a loop
	 *         and will be played once only.
	 */
	...
	/**
	 * Stops this audio without setting the isLooping property in the case of a
	 * looping audio. The AudioNotifier uses this method to stop the audio when
	 * setMute(true) is invoked. This allows us to restore all looping audios
	 * when the sound is restored by calling setMute(false).
	 */
					...
					public void run() {
						try {
							synchronized (sync) {
								/*
								 * We have to wait for
								 * play(int,Callable<Boolean>) to let go of sync
								 * i.e. be ready with setting up the whole
								 * AbstractSCAudioClip state; otherwise, this
								 * Runnable will most likely prematurely seize
								 * to exist.
								 */
								if (!equals(command))
									return;
	 ...
	 * 
	 * @param loopCondition
	 *            a <tt>Callback&lt;Boolean&gt;</tt> which represents the
	 *            condition on which this audio will play more than once. If
	 *            <tt>null</tt>, this audio will play once only. If an
	 *            invocation of <tt>loopCondition</tt> throws a
	 *            <tt>Throwable</tt>, this audio will discontinue playing.
	 */
		...
		try {
			...
			while (isStarted()) {
				...
				if (loopCondition == null) {
					/*
					 * The interface contract is that this audio plays once only
					 * if the loopCondition is null.
					 */
					break;
				}
	 ...
	 *         respectively, the method are to be invoked if this audio is to be
	 *         played in a loop; otherwise, <tt>false</tt>. The value reflects
	 *         an implementation-specific loop condition, is not dependent on
	 *         <tt>loopInterval</tt> and <tt>loopCondition</tt> and is combined
	 *         with the latter in order to determine whether there will be a
	 *         subsequent iteration of the playback loop.
	 */
	 ...
	 * 
	 * @param invalid
	 *            <tt>true</tt> to mark this instance invalid or <tt>false</tt>
	 *            to mark it valid
	 */
	 ...
	 * 
	 * @param looping
	 *            <tt>true</tt> to mark this instance that it should play the
	 *            audio it represents in a loop; otherwise, <tt>false</tt>
	 */
	 ...
	 * 
	 * @param loopInterval
	 *            the interval of time in milliseconds between consecutive plays
	 *            of this audio in a loop to be set on this instance
	 */
