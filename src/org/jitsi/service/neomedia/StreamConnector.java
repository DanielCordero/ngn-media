 ...
 * <tt>StreamConnector</tt> is so that they could be shared among media and
 * other libraries that may need to use them like an ICE implementation for
 * example.
 * 
 * @author Emil Ivov
 */
public interface StreamConnector {
	 ...
	 *         data (e.g. RTP) traffic or <tt>null</tt> if this
	 ...
	 * 
	 * @return a reference to the <tt>Socket</tt> that a stream should use for
	 *         control data (e.g. RTCP) or <tt>null</tt> if this
	 *         <tt>StreamConnector</tt> does not handle TCP sockets.
	 */
	 ...
	 * that is, if its data and control sockets share the same local address and
	 * port.
	 * 
	 * @return <tt>true</tt> if this <tt>StreamConnector</tt> uses rtcp-mux,
	 *         that is, if its data and control sockets share the same local
	 *         address and port.
	 */
