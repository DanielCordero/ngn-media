...
public class BasicVolumeControl implements VolumeControl, GainControl {
	 ...
	 * 
	 * @param gainControl
	 *            the <tt>GainControl</tt> which specifies the gain to apply
	 * @param buffer
	 *            the samples of the signal to apply the gain to
	 * @param offset
	 *            the start of the samples of the signal in <tt>buffer</tt>
	 * @param length
	 *            the number of samples of the signal given in <tt>buffer</tt>
	 */
	 ...
	 * 
	 * @param powerLevelRequired
	 *            The power level wished for the signal (corresponds to the
	 *            measured power level).
	 * @param referencePowerLevel
	 *            The reference power level.
	 * 
	...
	/**
	 * Creates volume control instance and initializes initial level value if
	 * stored in the configuration service.
	 * 
	 * @param volumeLevelConfigurationPropertyName
	 *            the name of the configuration property which specifies the
	 *            value of the volume level of the new instance
	 */
	...
	/**
	 * Adds a <tt>VolumeChangeListener</tt> to be informed for any change in the
	 * volume levels.
	 * 
	 * @param listener
	...
	public void addVolumeChangeListener(VolumeChangeListener listener) {
		synchronized (volumeChangeListeners) {
			...
			if (!contains)
				volumeChangeListeners.add(new WeakReference<VolumeChangeListener>(listener));
		}
	}
	...
	private void fireGainChange() {
		if (gainChangeListeners != null) {
			GainChangeEvent ev = new GainChangeEvent(this, mute, db, volumeLevel);
	...
	protected void loadVolume() {
		try {
			...
			if (cfg != null) {
				String volumeLevelString = cfg.getString(volumeLevelConfigurationPropertyName);
	 ...
	 * 
	 * @param listener
	 *            the volume change listener to be removed.
	 */
	...
	/**
	 * Internal implementation combining setting level from JMF and from outside
	 * Media Service.
	 * 
	 * @param value
	 *            the new value, changed if different from current volume
	 *            settings.
	...
	private float setVolumeLevel(float value) {
		...
		if (cfg != null) {
			cfg.setProperty(this.volumeLevelConfigurationPropertyName, String.valueOf(volumeLevel));
		}
