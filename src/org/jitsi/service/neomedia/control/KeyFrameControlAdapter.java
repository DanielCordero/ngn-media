...
public class KeyFrameControlAdapter implements KeyFrameControl {
	 ...
	 */
	private List<KeyFrameRequestee> keyFrameRequestees = new ArrayList<KeyFrameRequestee>(0);

	/**
	 * The <tt>KeyFrameRequester</tt>s made available by this
	 * <tt>KeyFrameControl</tt>.
	 */
	private List<KeyFrameRequester> keyFrameRequesters = new ArrayList<KeyFrameRequester>(0);
	...
	public void addKeyFrameRequestee(int index, KeyFrameRequestee keyFrameRequestee) {
		...
		synchronized (this) {
			if (!keyFrameRequestees.contains(keyFrameRequestee)) {
				List<KeyFrameRequestee> newKeyFrameRequestees = new ArrayList<KeyFrameRequestee>(keyFrameRequestees.size() + 1);
	...
	public void addKeyFrameRequester(int index, KeyFrameRequester keyFrameRequester) {
		...
		synchronized (this) {
			if (!keyFrameRequesters.contains(keyFrameRequester)) {
				List<KeyFrameRequester> newKeyFrameRequesters = new ArrayList<KeyFrameRequester>(keyFrameRequesters.size() + 1);
	...
	public boolean removeKeyFrameRequestee(KeyFrameRequestee keyFrameRequestee) {
		synchronized (this) {
			...
			if (-1 != index) {
				List<KeyFrameRequestee> newKeyFrameRequestees = new ArrayList<KeyFrameRequestee>(keyFrameRequestees);
	...
	public boolean removeKeyFrameRequester(KeyFrameRequester keyFrameRequester) {
		synchronized (this) {
			...
			if (-1 != index) {
				List<KeyFrameRequester> newKeyFrameRequesters = new ArrayList<KeyFrameRequester>(keyFrameRequesters);
