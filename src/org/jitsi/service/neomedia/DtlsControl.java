...
public interface DtlsControl extends SrtpControl {
	/**
	 * The transport protocol (i.e. <tt>&lt;proto&gt;</tt>) to be specified in a
	 * SDP media description (i.e. <tt>m=</tt> line) in order to denote a
	 ...
	 * the signaling path.
	 * 
	 * @param remoteFingerprints
	 *            a <tt>Map</tt> of hash functions to certificate fingerprints
	 *            that have been presented by the remote endpoint via the
	 *            signaling path
	 */
	...
	public enum Setup {
		 ...
		 * 
		 * @param s
		 *            the <tt>String</tt> to parse into a <tt>Setup</tt> enum
		 *            value
		 * @return a <tt>Setup</tt> enum value on which <tt>toString()</tt>
		 *         produces the specified <tt>s</tt>
		 * @throws IllegalArgumentException
		 *             if none of the <tt>Setup</tt> enum values produce the
		 *             specified <tt>s</tt> when <tt>toString()</tt> is invoked
		 *             on them
