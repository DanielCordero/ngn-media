...
/**
 * A base class that manages encoding configurations. It holds information about
 * supported formats.
 * 
...
public abstract class EncodingConfiguration {
	 ...
	 */
	private final Comparator<RTPMediaFormat> encodingComparator = new Comparator<RTPMediaFormat>() {
		public int compare(RTPMediaFormat s1, RTPMediaFormat s2) {
	 ...
	 * priority.
	 */
	private Set<RTPMediaFormat> supportedAudioEncodings;

	/**
	 * The cache of supported <tt>VideoMediaFormat</tt>s ordered by decreasing
	 * priority.
	 */
	private Set<RTPMediaFormat> supportedVideoEncodings;
	 ...
	 */
	private Set<RTPMediaFormat> updateSupportedEncodings(RTPMediaType type) {
		Set<RTPMediaFormat> enabled = new TreeSet<RTPMediaFormat>(encodingComparator);

		for (RTPMediaFormat format : getAllEncodings(type)) {
	 ...
	 * 
	 * @param encoding
	 *            the SDP int of the encoding whose pref we're setting.
	 * @param clockRate
	 *            clock rate
	 * @param pref
	 *            a positive int indicating the preference for that encoding.
	 */
	 ...
	 * 
	 * @param encoding
	 *            the <tt>MediaFormat</tt> specifying the encoding to set the
	 *            priority of
	 * @param priority
	 *            a positive <tt>int</tt> indicating the priority of
	 *            <tt>encoding</tt> to set
	 */
	public void setPriority(RTPMediaFormat encoding, int priority) {
	 ...
	 * 
	 * @param encoding
	 *            the <tt>RTPMediaFormat</tt>
	 * @return the priority
	 */
	public int getPriority(RTPMediaFormat encoding) {
		/*
	 ...
	 * 
	 * @param type
	 *            the <tt>MediaType</tt> we would like to know the available
	 *            encodings of
	 * @return array of <tt>RTPMediaFormat</tt> supported for the
	 *         <tt>MediaType</tt>
	 */
	public abstract RTPMediaFormat[] getAllEncodings(RTPMediaType type);

	/**
	 * Returns the supported <tt>RTPMediaFormat</tt>s i.e. the enabled available
	 * <tt>RTPMediaFormat</tt>s, sorted in decreasing priority. Returns only the
	 * formats of type <tt>type</tt>.
	 * 
	 * @param type
	 *            the <tt>MediaType</tt> of the supported <tt>RTPMediaFormat</tt>s
	 *            to get
	 * @return an array of the supported <tt>RTPMediaFormat</tt>s i.e. the enabled
	 *         available <tt>RTPMediaFormat</tt>s sorted in decreasing priority.
	 *         Returns only the formats of type <tt>type</tt>.
	 */
	public RTPMediaFormat[] getEnabledEncodings(RTPMediaType type) {
		Set<RTPMediaFormat> supportedEncodings;
		...
		case VIDEO:
			...
			break;
		default:
			return new RTPMediaFormat[0];
		}

		return supportedEncodings.toArray(new RTPMediaFormat[supportedEncodings.size()]);
	}
	 ...
	 * 
	 * @param enc1
	 *            the first format to compare for preference.
	 * @param enc2
	 ...
	 */
	protected abstract int compareEncodingPreferences(RTPMediaFormat enc1, RTPMediaFormat enc2);

	/**
	 * Gets the key in {@link #encodingPreferences} which is associated with the
	 * priority of a specific <tt>RTPMediaFormat</tt>.
	 * 
	 * @param encoding
	 *            the <tt>RTPMediaFormat</tt> to get the key in
	 ...
	 */
	protected String getEncodingPreferenceKey(RTPMediaFormat encoding) {
	 ...
	 * using <tt>prefix</tt> as a prefix to the key. Entries in the format
	 * (prefix+formatName, formatPriority) will be added to <tt>properties</tt>,
	 * one for each available format. Note that a "." is not automatically added
	 * to <tt>prefix</tt>.
	 * 
	 * @param properties
	 *            The <tt>Map</tt> where entries will be added.
	 * @param prefix
	 *            The prefix to use.
	 */
	public void storeProperties(Map<String, String> properties, String prefix) {
		for (RTPMediaType mediaType : RTPMediaType.values()) {
			for (RTPMediaFormat mediaFormat : getAllEncodings(mediaType)) {
	 ...
	 * look for an encoding name after the last ".", and interpret the key value
	 * as preference.
	 * 
	 * @param properties
	 *            The <tt>Map</tt> to parse.
	 ...
	 *            the <tt>EncodingConfiguration</tt> to load preferences from.
	 */
	 ...
	 * 
	 * @param mediaType
	 *            The media type, MediaType.AUDIO or MediaType.VIDEO
	 * @return <tt>true</tt> if there is at least one enabled format for media
	 *         type <tt>type</tt>.
	 */
	public boolean hasEnabledFormat(RTPMediaType mediaType) {
