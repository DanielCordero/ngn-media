...
public enum MediaDirection {
	/**
	 * Indicates that the related entity does not support neither input nor
	 * output (i.e. neither send nor receive) operations.
	 */
	INACTIVE("inactive"),

	/**
	 * Represents a direction from the entity that this direction pertains to to
	 * the outside. When applied to a <tt>MediaDevice</tt> the direction
	 * indicates that the device is a read-only one. In the case of a stream a
	 * <tt>SENDONLY</tt> direction indicates that the stream is only sending
	 ...
	 * stream is only receiving data from the remote party without sending any.
	 */
	RECVONLY("recvonly"),
	 ...
	 * 
	 * @param directionName
	 *            the name of the <tt>MediaDirections</tt> we'd like to create.
	 */
	 ...
	 * "sendrecv"). The name returned by this method is meant for use by session
	 * description mechanisms such as SIP/SDP or XMPP/Jingle.
	 * 
	...
	/**
	 * Applies an extra direction constraint to this <tt>MediaDirection</tt> or
	 * in other words performs an <tt>and</tt> operation. This method is
	 * primarily meant for use by the
	 * <tt>getReverseMediaDirection(MediaDirection)</tt> method while working on
	 * Offer/Answer media negotiation..
	 * 
	 * @param direction
	 *            that direction constraint that we'd like to apply to this
	...
	/**
	 * Reverses a direction constraint on this <tt>MediaDirection</tt> or in
	 * other words performs an <tt>or</tt> operation. This method is meant for
	 * use in cases like putting a stream off hold or in other words reversing
	 * the <tt>SENDONLY</tt> constraint.
	 * 
	 * @param direction
	 *            the direction that we'd like to enable (i.e. add) to this
	 *            <tt>MediaDirection</tt>
	 * 
	 ...
	 * party's perspective of this <tt>MediaDirection</tt>. In other words, if I
	 * say I'll be sending only, for you this means that you'll be receiving
	 * only. If however, I say I'll be both sending and receiving (i.e.
	 * <tt>SENDRECV</tt>) then it means you'll be doing the same (i.e. again
	 * <tt>SENDRECV</tt>).
	 * 
	 ...
	 * only receive a particular media type (i.e. <tt>RECVONLY</tt>) and you are
	 * in the same situation then this means that neither of us would be sending
	 * nor receiving and the stream would appear <tt>INACTIVE</tt> to you (and
	 * me for that matter). The method is meant for use during Offer/Answer SDP
	 * negotiation.
	 * 
	 * @param remotePartyDir
	 *            the remote party <tt>MediaDirection</tt> constraint that we'd
	 *            have to consider when trying to obtain a
	 *            <tt>MediaDirection</tt> corresponding to remoteParty's
	 *            constraint.
	 * 
	 * @return the <tt>MediaDirection</tt> value corresponding to a remote
	 *         party's perspective of this <tt>MediaDirection</tt> applying a
	 *         remote party constraint.
	 */
	 ...
	 * 
	 * @param mediaDirectionStr
	 *            the direction <tt>String</tt> that we'd like to parse.
	 * @return a <tt>MediaDirection</tt> value corresponding to the specified
	 *         <tt>mediaDirectionStr</tt>.
	 * 
	 * @throws IllegalArgumentException
	 *             in case <tt>mediaDirectionStr</tt> is not a valid media
	 *             direction.
	 */
