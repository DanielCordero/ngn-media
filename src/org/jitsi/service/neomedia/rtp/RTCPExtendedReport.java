...
public class RTCPExtendedReport extends RTCPPacket {
	...
	public static abstract class ReportBlock {
		 ...
		 *            the <tt>DataOutputStream</tt> into which the binary
		 *            representation of this <tt>ReportBlock</tt> is to be
		 *            serialized/written.
		 * @throws IOException
		 *             if an input/output error occurs during the
		 *             serialization/writing of the binary representation of
		 *             this <tt>ReportBlock</tt>
		 */
	...
	public static class VoIPMetricsReportBlock extends ReportBlock {
		 ...
		 */
		private byte packetLossConcealment = UNSPECIFIED_PACKET_LOSS_CONCEALMENT;
		 ...
		 * 
		 * @param blockLength
		 *            the length of the extended report block to read, including
		 *            the header, in 32-bit words minus one
		 * @param datainputstream
		 *            the binary representation from which the new instance is
		 *            to be initialized. The <tt>datainputstream</tt> is asumed
		 *            to contain type-specific block contents without extended
		 *            report block header i.e. no block type (BT),
		 *            type-specific, and block length fields will be read from
		 *            <tt>datainputstream</tt>.
		 * @throws IOException
		 *             if an input/output error occurs while
		 *             deserializing/reading the new instance from
		 *             <tt>datainputstream</tt> or the binary representation
		 *             does not parse into an <tt>VoIPMetricsReportBlock</tt>
		 *             instance
		 */
		...
		protected void assemble(DataOutputStream dataoutputstream) throws IOException {
			...
			// receiver configuration byte (RX config)
			dataoutputstream.writeByte(((getPacketLossConcealment() & 0x03) << 6) | ((getJitterBufferAdaptive() & 0x03) << 4) | (getJitterBufferRate() & 0x0F));
			// reserved
			dataoutputstream.writeByte(0);
			// jitter buffer nominal delay (JB nominal)
		 ...
		 *            {@link #ADAPTIVE_JITTER_BUFFER_ADAPTIVE},
		 *            {@link #NON_ADAPTIVE_JITTER_BUFFER_ADAPTIVE},
		 *            {@link #RESERVED_JITTER_BUFFER_ADAPTIVE}, or
		 *            {@link #UNKNOWN_JITTER_BUFFER_ADAPTIVE}
		 * @throws IllegalArgumentException
		 *             if the specified <tt>jitterBufferAdapter</tt> is not one
		 *             of the constants <tt>ADAPTIVE_JITTER_BUFFER_ADAPTIVE</tt>
		 ...
		 *            the implementation specific adjustment rate of a jitter
		 *            buffer in adaptive mode
		 */
		 ...
		 *            {@link #UNSPECIFIED_PACKET_LOSS_CONCEALMENT}
		 * @throws IllegalArgumentException
		 *             if the specified <tt>packetLossConcealment</tt> is not
		 ...
		 * 
		 * @param sourceSSRC
		 *            the synchronization source identifier (SSRC) of the RTP
		 *            data packet source being reported upon by this report
		 *            block
		 */
		...
		public String toString() {
			...
			s.append(", SSRC of source ").append(getSourceSSRC() & 0xFFFFFFFFL);
	 ...
	 *            initialized
	 * @param off
	 *            the offset in <tt>buf</tt> at which the binary representation
	 *            starts
	 * @param len
	 *            the number of <tt>byte</tt>s in <tt>buf</tt> starting at
	 *            <tt>off</tt> which comprise the binary representation
	 * @throws IOException
	 *             if an input/output error occurs while deserializing/reading
	 *             the new instance from <tt>buf</tt> or the binary
	 *             representation does not parse into an
	 ...
	 * @throws IOException
	 *             if an input/output error occurs while deserializing/reading
	 *             the new instance from <tt>datainputstream</tt> or the binary
	 *             representation does not parse into an
	 *             <tt>RTCPExtendedReport</tt> instance
	 */
	public RTCPExtendedReport(DataInputStream datainputstream) throws IOException {
		...
		while (length > 0) {
			...
			if (bt == VoIPMetricsReportBlock.VOIP_METRICS_REPORT_BLOCK_TYPE) {
				addReportBlock(new VoIPMetricsReportBlock(blockLength, datainputstream));
			} else {
				/*
	 ...
	 *         this extended report changed because of the method invocation;
	 ...
	 *         <tt>RTCPExtendedReport</tt> has been received or sent by the
	 *         local endpoint
	 */
	 ...
	 * 
	 * @param ssrc
	 *            the synchronization source identifier (SSRC) of the originator
	 *            of this XR packet
	 */
